/*exported PublicProfilePageView */
/*global
PageView,
...
PublicProfilePageView = PageView.extend( {
    ...
    initialize: function( opts ) {
        // Cheers Customization - Celebrating you wip-62128 - 
        // This PublicProfilePageView has many additions right from the beginning
        // Added new model PublicProfileModel
        // Added comments, likes and upload related events and functions. Kindly make sure while upgrading compare the original version with this client cuustomization updates
        var that = this;
        ...
        this.PublicProfileModel = new PublicProfileModel( opts.json );
        this.paxId = this.PublicProfileModel.get( 'recipient' ).id;
        ...
        this.PublicProfileModel.fetchComments( );
     },
    ...
    events: {
        ...
        'click .careerMomentsCommentsWrapper .dropdown-menu li a': 'doContribCommentSpellCheck',
    ...
    setupEvents: function(){
        ...
        // fetch comment list
        this.PublicProfileModel.on('success:fetchnewEmployeeAbout', this.fetchnewEmployeeAbout, this);
      
        // fetch comment list
        this.PublicProfileModel.on('start:fetchComments', this.startFetchComments, this);
        this.PublicProfileModel.on('end:fetchComments', this.endFetchComments, this);
        this.PublicProfileModel.on('success:fetchComments', this.successFetchComments, this);
       
        // attachments
        this.PublicProfileModel.on('change:attachMode', this.updateContribComment, this);
        this.PublicProfileModel.on('change:attachMode', this.updateAttachMedia, this);
        ...
        // save comment
        this.PublicProfileModel.on('start:saveComment', this.startSaveComment, this);
        this.PublicProfileModel.on('end:saveComment', this.endSaveComment, this);
        this.PublicProfileModel.on('success:saveComment', this.successSaveComment, this);

        //translate text
        this.PublicProfileModel.on('translated', function(){this.updateTranslatedText();},this);   

        //when model is liked
        this.PublicProfileModel.on('liked', function(numLikes, id) {this.changeDomToLiked(numLikes, id); }, this);

        //when Iam containers is liked
        this.PublicProfileModel.on('iamliked', function(numLikes, id) {this.changeIamDomToLiked(numLikes, id); }, this);
    

        //when model has a comment added
        this.PublicProfileModel.on( 'levelOneCommentAdded', function ( comment ) {
    ...
    attachCheersPopover: function( e ) {
        ...
        if ( !$tar.data( 'cheersPopover' ) ) {
            $tar.cheersPopover( { containerEl: this.$el, recipientId: $paxId, promotionId : $promoId  } ).qtip( 'show' );
        }
        e.preventDefault();
    },
    ...
    doAttachMode: function( e ) {
        ...
        e.preventDefault();
        this.PublicProfileModel.set( 'attachMode', mode );
    },
    doCloseAttachMode: function( e ) {
        ...
        this.PublicProfileModel.set( 'attachMode', 'select' );
    },
    doRemoveCommentMedia: function( e ) {
        ...
        this.PublicProfileModel.set( 'commentImage', null );
        this.PublicProfileModel.set( 'commentVideo', null );
    ...
    doAttachVideoUrl: function( e ) {
        ...
        // set video in model
        this.PublicProfileModel.set( 'commentVideo', $url.val() );

        // clear input
        $url.val( '' );

        // this will trigger, and the view will listen and update
        this.PublicProfileModel.set( 'attachMode', 'display' );
    },

    // update the attach photo, video view
    updateAttachMedia: function() {
        var mode = this.PublicProfileModel.get( 'attachMode' ),
            $upBtn = this.$el.find( '.uploadBtn' ),
    ...
    updateAttachMedia_display: function() {
        var //$attSec = this.$el.find( '.attachmentSection' ),
            ...
            $upBtn = this.$el.find( '.uploadBtn' ),
            imgObj = this.PublicProfileModel.get( 'commentImage' ),
            vidObj = this.PublicProfileModel.get( 'commentVideo' ),
    ...
    doSubmitContribComment: function( e ) {
        ...
        this.PublicProfileModel.saveComment( cmt );
    },
    ...
    doLike: function ( e ) {
        ...
        if ( $tar.hasClass( 'liked' ) ) {
            this.PublicProfileModel.saveLike( conId, that.paxId );
        } else {
            this.PublicProfileModel.saveUnlike( conId, that.paxId );
        }
    },
    ...
    doIamLike: function ( e ) {
        ...
        if ( $tar.hasClass( 'liked' ) ) {
            this.PublicProfileModel.saveIamLike( iamId, that.paxId );
        } else {
            this.PublicProfileModel.unsaveIamUnlike( iamId, that.paxId );
        }
    },
    ...
    addLevelOneCommentToDom: function ( comment, type ) {
        ...
        TemplateManager.get( 'levelOneCommentItem', function ( tpl ) {
            that.$el.find( '#commentCounts_' + comment.commentId ).text( '(' + comment.numComments + ')');
            if ( type == 'single' ) {
    ...
    changeIamDomToLiked: function ( numLikes, id ) {
        var $likeBtn = this.$el.find( '#iamCommentsLike_'+id ),            
            ...
            m = this.PublicProfileModel.toJSON();
    ...
    doCommentKeydown: function ( e ) {
        ...
        if ( e.which === 13 ) {
            ...
            if ( $inp.val().length > 0 ) {
                ...
                this.PublicProfileModel.saveLevelOneComment( params, $inp.val(), $commentId, jsonUrl, function ( cmt ) {
                    // console.log(e);
    ...
    updateImInfo: function() {        
        ...
        immodel = this.PublicProfileModel.get('iaminfo');
    ...
    updateContribCommentsList: function( isLatestOnly ) {
        ...
        var that = this,
            ...
            comments = this.PublicProfileModel.get( 'comments' );//,
            //toAddAt;
    ...
    updateContribComment: function() {
        ...
        var $inp = this.$el.find( '.contribCommentInp' ),
            ...
            //hasMedia = this.PublicProfileModel.get( 'commentImage' ) || this.PublicProfileModel.get( 'commentVideo' ),
            ...
            beforeSend: function() {
                ...
                if( msg.isSuccess ) {
                    if( msg.vidURL ) {
                        ...
                        that.PublicProfileModel.set( 'commentVideo', {
                            url: msg.vidURL,
                        ...
                        that.PublicProfileModel.set( 'commentImage', {
                            url: msg.picURL,
                            thumbUrl: msg.thumbNailURL
                        } );
                    }
                    ...
                    that.PublicProfileModel.set( 'attachMode', 'display' );
                } else {

...
SSIPayoutChartView = SSISharedHelpersView.extend( {
    ...
    render: function () {
        ...
        var progressWidth = ( Math.min( 100, this.model.get( 'payoutPercentProgress' ) ) + '%' );

        if ( this.model.get( 'includeBonus' ) ) {
            this.$el.addClass( 'includeBonus' );
            this.$payout.css( 'left', ( this.model.get( 'payoutCap' ) / this.model.get( 'payoutBonusCap' ) * 100 ) + '%'  );
        } else {
            this.$el.addClass( 'noBonus' );
        }
        ...
        if ( this.$progress.length ) {
            ...
            this.$progress.addClass( 'top' ).removeClass( 'bottom' );
            ...
            // make the tooltip stay on screen
            this.$progress.removeClass( 'left right' );

            if ( this.$progress.offset().left < 0 ) {
                this.$progress.addClass( 'left' );
            }
            if ( this.$progress.offset().left + this.$progress.outerWidth() > $( 'body' ).innerWidth() ) {
                this.$progress.addClass( 'right' );
            }
        }

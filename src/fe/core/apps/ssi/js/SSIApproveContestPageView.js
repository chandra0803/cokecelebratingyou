/*exported SSIApproveContestPageView */
/*global
_,
$,
G5,
...
SSIApproveContestPageView = PageView.extend( {
    ...
    initialize: function ( opts ) {
        ...
        //var detailTableJSON; // commented out for lint
        ...
        this.model.billCodes = _.sortBy( this.model.billCodes, function ( billCode ) {return billCode.index;} );
    ...
    renderDetailsTable: function( tarSect ) {
        ...
        this.detailsTableModel.on( 'loadDataFinished', function() {
            ...
            TemplateManager.get( detailTableTpl, function( tpl, vars, subTpls ) {
                ...
                if ( tarSect == 'Levels' ) {
                    that.$el.find( '.table th.level' ).each( function() {
                        if ( $( this ).data( 'sortOn' ) == that.detailsTableModel.attributes.sortedOn ) {
                            $( this ).addClass( 'sorted ' + that.detailsTableModel.attributes.sortedBy );
                            $( this ).data( 'sortBy', that.detailsTableModel.attributes.sortedBy == 'asc' ? 'desc' : 'asc' );
                        } else {
                            $( this ).addClass( 'asc' );
    ...
    renderInviteesTables: function() {
        ...
        this.superViewersCollectionView = new SSIApproveContestMembersCollectionView( {
            el: this.$el.find( '#ssiSuperViewersTableTpl' ),
            set: 'superViewers',
            clientState: this.model.clientState
        } );
    },
    ...
    attachPopover: function( $trig, cont, myPos, atPos ) {
        $trig.qtip( {
            ...
            style: {
                classes: 'ui-tooltip-shadow ui-tooltip-light ssiApproveContestPagePopover',
                tip: {
                    corner: true,
                    width: 20,
                    height: 10

/* exported SSIParticipantContestView */
/*global
_,
$,
console,
...
SSIParticipantContestView = PageView.extend( {
    ...
    initialize: function ( opts ) {
        ...
        this.on( 'compiled', this.showTooltip, this );
    ...
    loadStackRank: function ( e ) {
        ...
        if ( this.parentView.model.get( 'isManager' ) || this.parentView.model.get( 'isSuperViewer' ) ) {
            data.filter = 'team';
        }
    ...
    getTemplateName: function () {
        ...
        var isModule  = ( this.parentView instanceof SSIModuleView ),
            ...
            isAdmin   = isManager || isCreator || isSuperViewer,
            subset    = [ 'tpl' ],
            res;
        ...
        // Jeff added as comment... Needed?
        // if (isSuperViewer) {
        //     subset.unshift( isModule ? 'superViewerModuleTpl' : 'superViewerTpl');
        // }
    ...
    render: function () {
        ...
        function compile( tpls ) {
            var uniqueTpl  = tpls[ tpl ],
                ...
                extendedData,
                isAdmin   = data.isManager || data.isCreator || data.isSuperViewer;
    ...
    initReadMore: function() {
        ...
        if( description.length > 200 ) {
            $activityPartialDesc.html( $activityPartialDesc.html().replace( /^\s\s*/, '' ).replace( /\s\s*$/, '' ).substring( 0, 200 ) )
                .addClass( 'shown' )
                .show();
            textString = $activityPartialDesc.text();
            textString += '...';
            $activityPartialDesc.text( textString );

/*exported SSIContestPageEditView */
/*global
console,
$,
_,
...
SSIContestPageEditView = PageView.extend( {
    ...
    events: {
        ...
        //bill codes
        'change .ssiBillCodes': 'setBillTo',
        'keyup .custombillcode': 'updateCustomBillCode',
		// remove default option
        'focus .contestApproversLists': 'removeDefault'
    },
    ...
    adjustPageAfterPos: function( $el, $bl ) {
        var top = $( '#contents' ).css( 'position' ) === 'fixed' ? $el.position().top : $el.offset().top,
    ...
    unlockNav: function( key ) {
        var toUnlock = this.navLockKeys[ key ]; // array of btns to unlock for key

        delete this.navLockKeys[ key ]; // clear the key

       // lockKeys = _.keys(this.navLockKeys); // get keys

        _.each( this.navLockKeys, function( v ) {
            _.each( v, function( b ) {
    ...
    setNavLockState: function( isLock, btns ) {
        ...
        _.each( btns, function( b ) {
            ...
            if ( bSel ) {
               if ( isLock ) {
                    $cntCnts.find( bSel ).attr( 'disabled', 'disabled' );
                } else {
                    $cntCnts.find( bSel ).removeAttr( 'disabled' );
                }
            }
        } );
    },
    ...
    handleTabClick: function( e, fromTab, toTab ) {
        e.preventDefault();
    ...
    onNavErrorTip: function( msgClass, $target ) {
        ...
        var posMy = isBtn ? 'bottom center' : 'top center';
        var posAt = isBtn ? 'top center' : 'bottom center';
    ...
    updateBillTo: function() {
        var $root = this.$el.find( '.billToRoot:visible' ),
            $wrap = $root.find( '.billToWrapper' ),
            cm = this.contestModel,
            $billCodeSelect = this.$el.find( '.ssiBillCodes' ),
            billCodes = cm.get( 'billCodes' ),
        ...
        if ( !$wrap.length && !this._renderingBillTo ) {
            ...
            TemplateManager.get( 'billTo', function( tpl ) {
                $root.append( tpl( _.extend( { _initVisible: ptIsPts && bcReq }, this.contestModel.toJSON() ) ) );
                this._renderingBillTo = false;
                this.updateBillTo();
            }.bind( this ) );
        } else {
            //update each select to hide pre-selected values
             _.each( billCodes, function( bc ) {
                _.each( $billCodeSelect, function( bcs ) {
                    if( bc.index === $( bcs ).data( 'index' ) ) {
                        // Triggers setBillTo to select option at matching index
                        $( bcs ).find( 'option[value="default"]' ).attr( 'selected', false );
                        $( bcs ).find( 'option[value="' + bc.billCode + '"]' ).attr( 'selected', true ).change();
                    }
    ...
    updateCustomBillCode: function( e ) {
        var $tar = $ ( e.currentTarget ),
            $value = $tar.val(),
            position = parseInt( $tar.data( 'position' ) );
            this.contestModel.setCustomBillCodes( $value, position );
    },

    setBillTo: function( e ) {
        var $tar = $( e.currentTarget ),
            $value = $tar.val(),
            $billcodeSelects = this.$el.find( '.ssiBillCodes' ),
            $billCodeSelect = $billcodeSelects.not( $tar ),
            prevVal = $tar.data( 'stored-value' ),
            index = parseInt( $tar.data( 'index' ) ); // default index = null for $tar to initialize with default option selected
        	//Checking for the custom attribute from selected option
        	$tar.find ( 'option:selected' ).data( 'customValue' ) ? $tar.next().show() : $tar.next().hide();

            if( $value !== 'default' ) {

                $billCodeSelect.find( 'option[value="' + $value + '"]' ).attr( 'disabled', 'disabled' ).hide();
            }

            $billCodeSelect.find( 'option[value="default"]' ).attr( 'disabled', false ).show();

            if ( prevVal && prevVal !== $value ) {
                $billCodeSelect.find( 'option[value="' + prevVal + '"]' ).attr( 'disabled', false ).show();
            }

            $tar.data( 'stored-value', $value ); 

            if ( $value ) {
                this.contestModel.setBillCodes( $value, prevVal, index );
            }

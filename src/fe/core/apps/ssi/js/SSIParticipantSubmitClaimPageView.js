/* exported SSIParticipantSubmitClaimPageView */
/*global
$,
_,
G5,
...
SSIParticipantSubmitClaimPageView = PageView.extend( {
    ...
    events: {
        ...
        'click .ssiFileUploadWrap .icon-info': 'docInfoPopover',
    ...
    updateAttachment: function ( url, name ) {
        ...
        if( !url.currentTarget ) {
            ...
            $hiddenInp.val( url );
            $attachCont.show();
            $removeLink.css( 'display', 'inline-block' ); // .show() is insisting on display:inline
            $browseBtn.hide();
        } else {
            $attach.html( '' );
            $attachCont.hide();

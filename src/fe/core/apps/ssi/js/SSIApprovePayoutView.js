...
SSIApprovePayoutView = PageView.extend( {
    ...
    initialize: function( opts ) {
        ...
        helpers = new SSISharedHelpersView();
            ...
            ExtendedParticipantPaginatedView = ParticipantPaginatedView.extend( {
                ...
                validateSingleField: function ( $f ) {
                    ...
                    if ( $f.data( 'model-key' ) === 'payoutDescription' ) {
                        ...
                        sib = this.getTextInputForPaxAndField( $f.data( 'modelId' ), 'payoutValue' );
                        ...
                        $desc = this.getTextInputForPaxAndField( $f.data( 'modelId' ), 'payoutDescription' );
    ...
    */
    updateOtherPageTotal: function( e ) {
        ...
        if ( e ) {
            newTotal = ( this.otherPageTotal + sum ).toString().replace( /\B(?=(\d{3})+(?!\d))/g, ',' );
            this.$el.find( '.totalDisp span' ).html( newTotal );

        } else {
            this.otherPageTotal = parseInt( $payoutTotal, 10 ) - sum;

            newTotal = ( this.otherPageTotal + sum ).toString().replace( /\B(?=(\d{3})+(?!\d))/g, ',' );
            this.$el.find( '.totalDisp span' ).html( newTotal );

            return this.otherPageTotal;
        }

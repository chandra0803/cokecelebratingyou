/*exported SSIApproveClaimsPageView */
/*global
_,
Backbone,
$,
...
SSIApproveClaimsPageView = PageView.extend( {
    ...
    createHistoryTable: function () {
        ...
        modelData = _.pick( this.model.toJSON(), [ 'contestId', 'sortedOn', 'sortedBy', 'canApprove', 'claimsPendingCount' ] );
        ...
        this.historyTable.on( 'tableRendered', function() {
            ...
            that.$el.find( '.noDataMessages span' ).addClass( 'hidden' );
            ...
            if ( this.collection.length ) {
                ...
                if ( filterVal === 'waiting_for_approval' || anyWaiting ) {
                    ...
                    that.$el.find( '.ssiApproveAllClaimsBtn, .approveDenyBtns' ).removeClass( 'hidden' );
                } else {
                    that.$el.find( '.ssiApproveAllClaimsBtn, .approveDenyBtns' ).addClass( 'hidden' );
                }
    ...
    attachPopover: function( $trig, cont, myPos, atPos, hideCallback ) {
        $trig.qtip( {
            ...
            style: {
                classes: 'ui-tooltip-shadow ui-tooltip-light ssiApproveClaimsPagePopover',
                tip: {
                    corner: true,
                    width: 20,
                    height: 10

/* exported SSIDTGTActivityModelView*/
/*global
console,
$,
_,
...
SSIDTGTActivityModelView = Backbone.View.extend( {
    ...
    */
    setEstimates: function( isGoalSacred ) {
        ...
        pnts = Math.round( (  pct * m( 'maxPayout' ) )  / 100 );
        // don't allow fractional estimated points
        if( this.pt === 'points' ) { pnts = Math.round( pnts ); }
        this.model.set( 'estMaxPayout', pnts );
    ...
    updatePayoutType: function() {
        ...
        this.$el.find( '.activityDetailValueDesc' )[ this.pt == 'other' ? 'show' : 'hide' ]();
        this.$el.find( '.otherPayoutTypeMsg' ).css( 'display', this.pt == 'other' ? 'inline-block' : 'none' );
        this.$el.find( '.pointsPayoutTypeMsg' ).css( 'display', this.pt == 'other' ? 'none' : 'inline-block' );
    },

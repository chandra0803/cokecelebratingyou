/*exported SSIContestEditTabPayoutsView_siu */
/*global
$,
_,
Backbone,
...
SSIContestEditTabPayoutsView_siu = Backbone.View.extend( {
    ...
    events: {
        ...
        'keydown .levelDetailPayout input': 'noDecimal',
        'keydown .bonusPayoutInput': 'noDecimal',
        'keydown .bonusPayoutCapInput': 'noDecimal',
        'keydown .participant-item .paxDatTextInput': 'noDecimal',
    ...
    ***************************************************** */
    ...
    updateCurrencyDisp: function() {
        var  mt = this.contMod.get( 'measureType' ), // act - points
            mob = this.contMod.get( 'measureOverBaseline' ),
        ...
        if ( !this.levelCollectionView || !this.pt || !mt ) {
            return;
        }
    ...
    updateEstimatedTotals: function( goalIsSacred ) {
        ...
        if( this.pt === 'points' ) { estPay = Math.round( estPay ); }
    ...
    updateEstimators: function() {
        ...
        if ( estTpl ) {
            ...
            $estLst.find( '.estimateSlider' ).each( function( i ) {
                ...
                $t.on( 'slideStart', function() {
                    $estLst.find( '.estimateSlider' ).each( function() {

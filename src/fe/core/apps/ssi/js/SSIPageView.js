...
SSIPageView = PageView.extend( {
    ...
    initialize: function ( opts ) {
        ...
        /**
         * updated for G6 to use the global router
         * (see handleRoute below)
         *
        ...
        if ( opts.contestData ) {
            ...
            // backbone apparently needs to reset the path to trigger routing
            this.navigate( '', { trigger: false } )
                .navigate( G5.props.CURRENT_ROUTE.length ? G5.props.CURRENT_ROUTE : [ 'index' ] );
        }
    ...
    /**
     * replaces the SSI-specific router defined (and preserved in comments) above
     */
    handleRoute: function( event, route ) {
        var path = route.join( '/' ),
            param = null;

        // replaces the route:* handler above
        // functionality to be applied every time a route changes
        this.nav.setActive( path );
        this.unloadContest( this._prevContestPath );
        this._prevContestPath = param || path;
     ...
     * Updated for G6 to use the global router
     *
     * @param {string} path - array of the hashed URL
    ...
    loadChildView: function ( id, JSONURL ) {
        ...
        if ( !!currentContest.length ) {
            ...
            this.getContestData( id, JSONURL )
                ...
                .then( _.bind( function ( childView ) {
                    this.$el.find( '#ssiContestView' ).after( childView.$el );
                    // debugger
                    this.trigger( 'loadChildView', id, childView );
        ...
        if( id === 'all' || id === 'archived' || id === 'denied' ) {
            this.$el.find( '#ssiContestView' ).show();
        }
        else {
            this.$el.find( '#ssiContestView' ).hide();
        }
    ...
    createContestView: function ( data ) {
        ...
        var contestMap = {
            ...
            isAdmin = this.model.get( 'isManager' ) || this.model.get( 'isCreator' ) || this.model.get( 'isSuperViewer' ),
            _data;

/* exported SSIParticipantStepItUpView */
/*global
console,
$,
_,
...
SSIParticipantStepItUpView = SSIParticipantContestView.extend( {
    ...
    postRender: function ( chart ) {
        ...
        var isCreator = this.parentView.model.get( 'isCreator' ),
            ...
            isSuperViewer = this.parentView.model.get( 'isSuperViewer' ),
        ...
        if ( !chartType ) {
            chartType = ( isManager || isCreator || isSuperViewer ) ? 'bar' : 'circle';
        ...
        this.$el.find( '.levelData' ).responsiveTable();

        this.resizeText();

        this.resizeChart();
        G5._globalEvents.on( 'windowResized', this.resizeChart, this );
    ...
    */
    ...
    proccessModelJSON: function ( json ) {
        ...
        var isCreator = this.parentView.model.get( 'isCreator' ),
            isManager = this.parentView.model.get( 'isManager' ),
            isSuperViewer = this.parentView.model.get( 'isSuperViewer' ),
            roleFn    = ( isCreator || isManager || isSuperViewer ) ? 'proccessAdminModelJSON' : 'proccessParticipantModelJSON',
            participants,
            levels;
    ...
    resizeChart: function() {
        ...
        if( this.model.get( 'chartType' ) != 'circle' ) {
            return false;
        }

        if( this._weDoneCalculatedTheCharts !== true ) {
            this._resizeChart = {
                $wrap: this.$el.find( '.levelChartWrapper' ),
                // $track  : this.$el.find('.levelChartTrack'),
                // $cols   : this.$el.find('.levelCircles .levelCol'),
                levels: this.$el.find( '.levelCircles .levelCol' ).length,
                // $curCol : this.$el.find('.levelCircles .currentLevelCol'),
                curLev: this.$el.find( '.levelCircles .currentLevelCol' ).index() + 1, // starting at 1
                minW: parseInt( this.$el.find( '.levelChartTrack' ).css( 'min-width' ), 10 ) //,
                // maxW    : parseInt(this.$el.find('.levelChartTrack').css('max-width'), 10)
            };

            this._weDoneCalculatedTheCharts = true;
        }
        refs = this._resizeChart;

        // do stuff
        // console.log('SIU chart resize watcher', this._resizeChart);

        classes.push( 'curLev' + refs.curLev );
        classes.push( refs.curLev == 1 ? 'curIsFirst' : null );
        classes.push( refs.curLev == refs.levels ? 'curIsLast' : null );

        if( refs.$wrap.width() < refs.minW ) {
            // console.log('SIU chart too wide!');

            refs.$wrap.addClass( classes.join( ' ' ) );
        }
        else {
            refs.$wrap.removeClass( classes.join( ' ' ) );
        }
    }
} );

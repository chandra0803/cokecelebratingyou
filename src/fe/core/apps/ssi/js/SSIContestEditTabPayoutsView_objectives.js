/*exported SSIContestEditTabPayoutsView_objectives*/
/*global
$,
_,
Backbone,
...
SSIContestEditTabPayoutsView_objectives = Backbone.View.extend( {
    ...
    events: {
        ...
        'keyup .textInputCell .paxDatTextInput': 'doShowSameForAllTip',
        'blur .textInputCell .paxDatTextInput': 'doHideSameForAllTip',
        'keydown .textInputCell .paxDatObjPayout': 'noDecimal',
    ...
    ***************************************************** */
        ...
        this.paxPaginatedView = new ParticipantPaginatedView( {
            ...
            getJsonForTemplate: function() {
                ...
                return {
                    ...
                    payoutCurrencyLabel: this.pt || false
                };
            }
    ...
    noDecimal: function( e ) {

        var keyCode = e.which,
            keyVal = $( e.target ).val(),
            noDecimal = this.pt === 'points' ? true : false;
    ...
    update: function() {
        ...
        this.$el.find( '[name=measureType][value=' + this.mt + ']' ).prop( 'checked', true );

        // set the payout type
        this.$el.find( '[name=payoutType][value=' + this.pt + ']' ).prop( 'checked', true );
    ...
    updateLockPageBottom: function() {
        ...
        if ( !this.contMod.get( 'measureType' ) || !this.pt ) {
    ...
    updateCurrTypeSel: function() {
        var ct = this.contMod.get( 'currencyTypeId' ),
            $ctw = this.$el.find( '.currencyTypeWrapper' ),
            $cts = this.$el.find( '#currencyTypeSelect' );

        this.mt = this.contMod.get( 'measureType' );
    ...
    updateOtherPayoutTypeSel: function() {
        var opt = this.contMod.get( 'otherPayoutTypeId' ),
            $optw = this.$el.find( '.otherPayoutTypeWrapper' ),
            $opts = this.$el.find( '#otherPayoutTypeSelect' );
        this.mt = this.contMod.get( 'measureType' );
        this.pt = this.contMod.get( 'payoutType' );
        // show/hide the currency type dropdown
        if ( this.mt === 'currency' || this.pt !== 'other' ) {
            $optw.slideUp();
        } else {
            $optw.slideDown();
        }
    ...
    updateIncludeBonus: function() {
        ...
        this.$el.find( '.includeBonusWrapper' )[ this.pt === 'other' ? 'hide' : 'show' ]();
    ...
    ***************************************************** */
    ...
    handleFetchCalcSuccess: function() {
        ...
        if ( !this.contMod.get( 'contestGoal' ) ) {
            ...
            this.contMod.setObjectiveEstimatedAchievmentPercent( parseInt( this.$el.find( '#estimateSlider' ).val() ) );
        } else {
    ...
    ***************************************************** */
    ...
    fmt: function( num, key ) {
        ...
        else if( this.pt === 'points' && key !== 'objectiveAmountTotal' ) {
            num = Math.round( num );
        }

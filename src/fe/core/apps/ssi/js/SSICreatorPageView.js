...
SSICreatorPageView = SSIPageView.extend( {
    ...
    createNewContestOverlay: function ( event ) {
        ...
        this.overlay = new SSIContestCreateModalView( {
            parentView: this,
            // target: $(event.target), // used for the qtip approach
            target: this.$el.find( '.ssiContestCreateModal' ),
    ...
    setValueFromQtip: function ( event ) {
        ...
        var contestId = this.overlay.$el.find( '.tabNavContent.active' ).data( 'id' );

        this.overlay.destroyView();

        this.$el.find( '.contestType [value="' + contestId + '"]' ).prop( 'selected', true );

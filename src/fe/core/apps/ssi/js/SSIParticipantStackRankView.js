/* exported SSIParticipantStackRankView */
/*global
console,
_,
$,
...
SSIParticipantStackRankView = SSIParticipantContestView.extend( {
        ...
        var $thisEl = this.$el,
            ...
            selfRender  = _.bind( function ( key, el ) {
                ...
                if ( this.isModule ) {
                    ...
                    new SSIEventBus( descendants );
                    if( bus ) {
                        // we have a bus linted - removing lint error for now
                    }
    ...
    proccessModelJSON: function ( json ) {
        ...
        if ( json.stackRankParticipants ) {
            json.stackRankParticipants = _.map( json.stackRankParticipants, function ( p, index, list ) {
                p.classes = [];
                if( index > 0 && p.score == list[ index - 1 ].score ) {
                    p.rank = list[ index - 1 ].rank;
                    p.tied = true;
                    p.classes.push( 'tied' );
                }
                else {

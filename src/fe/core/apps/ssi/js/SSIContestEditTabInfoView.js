/*exported SSIContestEditTabInfoView */
/*global
TemplateManager,
BadgesSelectorView,
...
SSIContestEditTabInfoView = Backbone.View.extend( {
    ...
    events: {
        ...
        'click .showRecurrancePeriod': 'updateRecurrancePeriod',
    ...
    handleFetchTranslationsDocsEnd: function() {
        ...
        $w.find( '.spinWrap' ).slideUp( G5.props.ANIMATION_DURATION );

        $w.find( '.documentsTranslationsEditor' ).slideDown( G5.props.ANIMATION_DURATION );
    ...
    doDocsHide: function( e ) {
        ...
        $w.find( '.documentsTranslationsEditor' ).slideUp( G5.props.ANIMATION_DURATION );
        $w.find( '.documentsTranslationsSummary' ).slideDown( G5.props.ANIMATION_DURATION );
    ...
    handleFetchTranslationsNamesEnd: function() {
        ...
        $w.find( '.nameTranslationsEditor' ).slideDown( G5.props.ANIMATION_DURATION );
    ...
    doNamesHide: function( e ) {
        ...
        $w.find( '.nameTranslationsEditor' ).slideUp( G5.props.ANIMATION_DURATION );
        $w.find( '.nameTranslationsSummary' ).slideDown( G5.props.ANIMATION_DURATION );
    ...
    handleFetchTranslationsDescriptionsEnd: function() {
        ...
        $w.find( '.ajaxSpinner' ).spin( false ).hide();
        $w.find( '.spinWrap' ).slideUp( G5.props.ANIMATION_DURATION );
        $w.find( '.descriptionTranslationsEditor' ).slideDown( G5.props.ANIMATION_DURATION );
    ...
    doDescriptionsHide: function( e ) {
        ...
        $w.find( '.descriptionTranslationsEditor' ).slideUp( G5.props.ANIMATION_DURATION );
        $w.find( '.descriptionTranslationsSummary' ).slideDown( G5.props.ANIMATION_DURATION );
    ...
    // show hide message stuff based on checkbox
    updateRecurrancePeriod: function( e ) {
        var $visCheck = this.$el.find( '.showRecurrancePeriod' ),
            $guts = this.$el.find( '.recurranceGuts' ),
            $txt = this.$el.find( '.defaultMessageInput' ),
            incMess = this.contMod.get( 'includeMessage' );
    ...
    handleFetchTranslationsMessagesEnd: function() {
        ...
        $w.find( '.messageTranslationsEditor' ).slideDown( G5.props.ANIMATION_DURATION );
    ...
    doMessagesHide: function( e ) {
        ...
        $w.find( '.messageTranslationsEditor' ).slideUp( G5.props.ANIMATION_DURATION );
        $w.find( '.messageTranslationsSummary' ).slideDown( G5.props.ANIMATION_DURATION );
        ...
        TemplateManager.get( 'messageItems', function( tpl ) {
            ...
            $w.find( '.richtext' ).each( function() {
                ...
                this.jhtmlareaObject.editor.body.onkeyup = function() {
                    that.doMessageTransSetText( { target: that } ); //that was thisRichText;
                };
            } );
        } );
    },
    ...
    updateAwardThemNow: function() {
        ...
        // Check for badgesSelectorView availability and call disable - Bug #70609
        if ( this.badgesSelectorView ) {
			this.badgesSelectorView.disable();
		}
    ...
    validate: function() {
        ...
        if ( cm.get( 'status' ) !== cm.STATUSES.LIVE && !this.isAtnEdit() ) {
            ...
            if ( this.isAtn() ) {
                if ( startDate > todayDate ) {
                    this.$el.find( '.startDateTrigger' ).parents( '.validateme' ).addClass( 'error' );
                    return { msgClass: 'msgStartDateTooLate', valid: false };
                }
            } else { // normal start date rule ( must be after or on today )
                // bz62404
                // if ( startDate < todayDate ) {
        ...
        if ( !this.isAtn() ) {
            if ( endDate <= startDate ) {
                 this.$el.find( '.endDateTrigger' ).parents( '.validateme' ).addClass( 'error' );
        ...
        if ( !this.isAtn() ) {
            if ( tileStartDate > startDate ) {
                this.$el.find( '.tileStartDateTrigger' ).parents( '.validateme' ).addClass( 'error' );

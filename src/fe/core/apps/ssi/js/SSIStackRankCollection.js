/* exported SSIStackRankCollection */
/*global
console,
$,
_,
...
SSIStackRankCollection = Backbone.Collection.extend( {
    ...
    toProcessedJSON: function ( opts ) {
        ...
        _.each( json, function ( participant, index, list ) {
            var classes = [];
            // even or odd
            ...
            if( index > 0 && participant.score == list[ index - 1 ].score ) {
                participant.rank = list[ index - 1 ].rank;
                participant.tied = true;
                participant.classes.push( 'tied' );
            }
            else {
                participant.rank = participant.rank || index + 1;
            }
        } );

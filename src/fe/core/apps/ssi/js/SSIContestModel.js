...
SSIContestModel = Backbone.Model.extend( {
    ...
    initialize: function() {
        ...
        // superViewer search results
        this.superViewerSearchResults = new Backbone.Collection();
        // model for the superViewer search table sorting state
        this.superViewerSearchTableModel = new Backbone.Model( { by: 'lastName', asc: true } );
        this.superViewerSearchTableSort( 'lastName', true );
    ...
    }, // sort the superViewer search table
    superViewerSearchTableSort: function( by, isAsc ) {
        var stm = this.superViewerSearchTableModel;
        ...
        this.superViewerSearchResults.comparator = function( a, b ) {
            var by = stm.get( 'by' ),
                comp = a.get( by ) > b.get( by ) ? 1 : -1;

            return stm.get( 'asc' ) ? comp : -comp;
        };
        this.superViewerSearchResults.sort();
        this.trigger( 'sort:superViewerSearchResults' );
    },
    ...
    getManagerFromSearchById: function( id ) { // Where is this triggered?
        var m = this.managersSearchResults.get( id );
        return m;
    },
    
    setCustomBillCodes: function( val, index ) {
        var billCodes = this.get( 'billCodes' );            
        _.each( billCodes, function( item, idx ) {
            if ( idx == index ) {
        ...
        //console.log( 'Updated Billcodes' + this.get( 'billCodes' ) );           
    },

    setBillCodes: function( val, previous, index ) {
        var billCodes = this.get( 'billCodes' ),
            updatedCodes = _.where( billCodes, { billCode: val } ),
            prevCode = _.where( billCodes, { billCode: previous } );
        if( prevCode.length ) {
                        prevCode[ 0 ].index = null;
                    }
        if ( val !== 'default' ) {
           updatedCodes[ 0 ].index = index;
        }
    },
    ...
    // fetch list of managers for given set of pax
    fetchSuperViewerSearch: function() {
        var that = this,
            url = G5.props.URL_JSON_CONTEST_SUPERVIEWERS_SEARCH,
            request;

        this.trigger( 'start:fetchSuperViewerSearch' );
        ...
        request.done( function( serverResp ) {
            ...
            if ( err ) {
                that.trigger( 'error:fetchSuperViewerSearch', err.text );
                that.trigger( 'error:genericAjax', err );
            } else {

                if ( serverResp.data.participants ) {
                    that.superViewerSearchResults.reset( serverResp.data.participants );
                }

                that.trigger( 'success:fetchSuperViewerSearch', serverResp.data );
            }
        } );

        request.fail( function( jqXHR, textStatus, err ) {
            var errors;
            console.error( '[ERROR] SSIContestModel: ajax fetch super viewers search fail', jqXHR, textStatus, err );
            // struts returns full HTML for FORM VALIDATION - BOOO!
        ...
        request.always( function() {
            ...
            that.trigger( 'end:fetchSuperViewerSearch' );
        } );
    },
    ...
    // call the server to remove a super viewer
    ajaxRemoveSuperViewer: function( paxId ) {
        var url = G5.props.URL_JSON_CONTEST_SUPERVIEWER_REM,
            params = { paxIds: [ paxId ] };

        this.fetchGeneric( 'ajaxRemoveSuperViewer', url, params, false );
    },

    // call the server to add a super viewer
    ajaxAddSuperViewer: function( paxId ) {
        var url = G5.props.URL_JSON_CONTEST_SUPERVIEWER_ADD,
            params = {};

        params.paxIds = _.isArray( paxId ) ? paxId : [ paxId ];

        this.fetchGeneric( 'ajaxAddSuperViewer', url, params, false );
    },
    ...
    validateBillTo: function() {
        ...
        if ( payoutTypePoints ) {
            _.each( this.get( 'billCodes' ), function( bc ) {

                bcCount = bc.index !== null ? bcCount + 1 : bcCount;
            } );

            valid = valid && bcCount > 0;
...
SSIContestModel.reformatDecimalStrings = function( obj ) {
    ...
    function fmt( string ) {
        ...
        isNaN( string ) ? n = parseNum( string ) : n = string;

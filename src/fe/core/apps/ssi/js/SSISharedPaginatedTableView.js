/* exported SSISharedPaginatedTableView */
/*global
_,
$,
G5,
...
SSISharedPaginatedTableView = SSISharedHelpersView.extend( {
    ...
    handleSort: function ( event ) {
        ...
        var $i = $( event.target ).closest( '.sortHeader' ),
            sortOn = $i.data( 'sort' ),
    ...
    setSortArrows: function () {
        ...
        if( $target.length ) {
            $all.removeClass( _.values( this.CLASSES ).join( ' ' ) ).addClass( 'unsorted' );
        }

        if ( sortBy && $target.length ) {
            $target.addClass( this.CLASSES[ sortBy.toUpperCase() ] ).removeClass( 'unsorted' );
        }

...
/*exported GoalquestPageEditView */
/*global
PageView,
WizardTabsView,
...
GoalquestPageEditView = PageView.extend( {
    ...
    initialize: function() {
        ...
        if ( !this.formModel.get( 'showPartnerStep' ) ) {
            this.options.tabsJson = _.reject( this.options.tabsJson, function( tab ) { return tab.name === 'stepPartner'; } );
            this.$el.find( '.wizardTabsContent .stepPartnerContent' ).remove();
        }

        // if no UA step, remove its tab and tab contents BEFORE wiz tabs init
        if ( !this.formModel.get( 'showUAStep' ) ) {
            this.options.tabsJson = _.reject( this.options.tabsJson, function( tab ) { return tab.name === 'stepUAConnect'; } );
            this.$el.find( '.wizardTabsContent .uaConnectContent' ).remove();
        }
        ...
        // init UA step (doesn't really do anything yet)
        this.initUAStep();
        ...
        // initial update
        this.wizTabs.on( 'tabsInitialized', this.updateView, this );
    },
    ...
    events: {
        ...
        'click .uaConnectContent .nextBtn': 'goToNextStep',
    ...
    validate_stepPartner: function() {
        var isChecked = this.$el.find( '.selectPartnerYesInput:checked' ).length > 0,
            ...
            maxPartners = parseInt( this.formModel.get( 'maxPartners' ) );
    ...
    doSelectPartnerYesChange: function() {
    ...
    initPartnerStep: function() {
        ...
        this.partnerView = new ParticipantCollectionView( {
            ...
            model: collection, //new Backbone.Collection(this.formModel.get('partners')),
        ...
        this.partnerSearchView = new PaxSearchStartView( {
         ...
         this.partnerSearchView.setAjaxParam( 'promotionId', this.formModel.get( 'promotionId' ) );
         this.partnerSearchView.setAjaxParam( 'goalId', this.formModel.get( 'goalId' ) );
        collection.reset( this.formModel.get( 'partners' ) );

    },
    // init UA stuff
    initUAStep: function() {
        // exit if this is not shown
        if ( !this.formModel.get( 'showUAStep' ) ) { return; }

        if ( this.formModel.get( 'showUAStep' ) ) {
            //UA connect dot tooltip
            $( '.ua-dot' ).each( function() {
                $( this ).tooltip( {
                   title: $( this ).data( 'titleConnectStatus' )
                } );
            } );

            // console.log( 'initUAStep complete' );
        }
    ...
    updateUAStep: function() {
        //console.log( 'update UA' );
    },
    ...
    updateSubmitStep: function() {
        ...
        if ( selPartYes && this.partnerView.model.length > 0 ) {
            ...
            _.each( that.partnerView.model.models, function( p ) {
                $partners.append( '<span class="label label-inverse">' + p.get( 'firstName' ) + ' ' + p.get( 'lastName' ) + '</span> ' );
            } );
        }
    ...
    extractFormPartners: function( formArray, prefix ) {
        ...
        _.each( formArray, function( inp ) {
            ...
            if ( inp.name.indexOf( prefix ) === 0 ) {
                ...
                if ( inp.name === prefix + 'Count' ) {
                    ...
                    num = brkt.length === 2 ? parseInt( brkt[ 1 ] ) : false; //get the number
        ...
        if ( arry.length !== count ) {
            console.error( '[ERROR] GoalquestPageEditView - extractFormPartners() [' + prefix + '] counts do not match ' + arry.length + ' != ' + count );
        }

...
QuizModel = Backbone.Model.extend( {
    ...
    initialize: function( opts ) {
        ...
        this.participants = new PaxSelectedPaxCollection( this.get( 'participants' ) || [] );//new Backbone.Collection(this.get('participants')||[]);
    ...
    toJSON: function() {
        ...
        if( json.createAsParticipant ) {
            if( typeof json.createAsParticipant.toJSON === 'function' ) {
               json.createAsParticipant = json.createAsParticipant.toJSON();
            }
        }
        return json;
    },
    serializeForStruts: function( opts ) {
        ...
        var paxLength = dat.participants.length;
        var participants = dat.participants;
        for ( var i = 0; i < paxLength; i++ ) {
            if ( participants[ i ].hasOwnProperty( 'promotions' ) ) {
                delete participants[ i ].allNodeIds;
                delete participants[ i ].promotions;
            }
        }
        ...
        //replacing empty [] as the regex isn't handling that correctly
        dat = dat.replace( '%5B%5D', '' );

...
QuizEditTabIntroView = Backbone.View.extend( {
    ...
    initPaxSearch: function() {
        ...
        this.participantSearchView = new PaxSearchStartView( {
                el: this.$el.find( '.paxSearchStartView2' ),
                multiSelect: true,
    ...
    updateCreateAsPaxSearch: function() {
        ...
        if( allowed && !qm.get( 'id' ) && !this.paxCreateAsView ) {
            this.paxCreateCollection = new PaxSelectedPaxCollection();
            this.paxCreateAsView = new PaxSearchStartView( {
                el: this.$el.find( '.paxSearchStartView' ),
                multiSelect: false,
                selectedPaxCollection: this.paxCreateCollection,
                disableSelect: false,
                addSelectedPaxView: true,
                follow: false
            } );
            this.paxCreateCollection.on( 'add', function( pax ) {
        ...
        if( pax ) {
            _.each( pax.attributes, function( v, k ) {

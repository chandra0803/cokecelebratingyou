...
ReportsPageView = PageView.extend( {
    ...
    initialize: function( opts ) {
        ...
        this.model.on( 'dataLoaded', function() {
            if( G5.props.CURRENT_ROUTE.length ) {
        ...
        this.model.on( 'activeReportSet', function() {
            ...
            // Check the model for Celebration activity history URL
            var celebrationActivityReport = thisView.model.getAwardUrl();
           if ( celebrationActivityReport ) {
               //Condition based child rendering
                ( celebrationActivityReport.indexOf( 'myawardsite.com' ) > -1 )  ? thisView.renderChildView( celebrationActivityReport ) : thisView.renderChildView();
           } else {
    ...
    renderChildView: function( awardUrl ) {
        var thisView = this,
            activeReportId = this.model.getActiveReportId(),
            ajaxdata = { clearForm: true };
            if( awardUrl ) {
                ajaxdata = $.extend( {}, ajaxdata, { renderChild: false, url: awardUrl } );
            }
    ...
    handleRoute: function( event, route ) {
        if( route ) {
            ...
            if( id !== this.model.getActiveReportId() ) {
                this.model.setActiveReportById( id );
            }

            if( this.reportsPageDetailView._isRendered === true ) {
                if( chartId ) {
                    this.reportsPageDetailView.model.setActiveChartById( chartId );
                }
                else {
                    this.reportsPageDetailView.model.setActiveChartByIndex( 0 );
                }
            }
            else if( chartId ) {
                this.reportsPageDetailView.on( 'renderDone', function() {
                    this.reportsPageDetailView.model.setActiveChartById( chartId );
                }, this );
            }
        }

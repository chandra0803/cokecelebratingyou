/*exported ReportsFavoritesPopoverView*/
...
ReportsFavoritesPopoverView = Backbone.View.extend( {
    ...
    initialize: function( opts ) {
        ...
        this.$trigger.qtip( {
            ...
            style: {
                classes: 'ui-tooltip-shadow ui-tooltip-light reportsFavoritesPopover modal',
                tip: opts.tip
            },
            events: {
    ...
    doShown: function() {
        ...
        $( '.reportsFavoritesPopover' ).before( '<div class="modal-backdrop fauxdrop"></div>' );
        ...
        .find( '.remove' ).click( function() {
            ...
            $.ajax( {
              ...
              success: function() {
                ...
                if ( thatView.favoritesJson.favorites.length >= G5.props.REPORTS_MAX_FAVORITES ) {
    ...
    favoritesFullVersusStartedFull: function() {
        ...
        if ( that.favoritesJson.favorites.length >= G5.props.REPORTS_MAX_FAVORITES ) {
    ...
    updateFromNumberOfFavorites: function() {
        ...
        if( this.options.triggeredFromModuleView || this.favoritesJson.favorites.length < G5.props.REPORTS_MAX_FAVORITES ) {
            $( '#pleaseRemoveAlert' ).hide();
            // only 1 favorite
        ...
        // otherwise, we can assume it was triggered by the page view and there are (G5.props.REPORTS_MAX_FAVORITES) favorites
        else {
    ...
    addChartInFocus: function() {
        ...
        // if thatView.favoritesJson.favorites.length < (G5.props.REPORTS_MAX_FAVORITES) add chartInFocus
        if ( this.favoritesJson.favorites.length < G5.props.REPORTS_MAX_FAVORITES ) {

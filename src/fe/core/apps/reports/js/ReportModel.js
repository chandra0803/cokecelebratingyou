...
ReportModel = Backbone.Model.extend( {
    ...
    loadReportsLandingPage: function( url ) {
            window.open( url, '_blank' );
            var partialUrl = 'reports/allReports.do';
            G5.util.jsRoute( partialUrl ); 
    },

    loadFullData: function( opts ) {
    	if( opts.data){
	        if( opts.data.renderChild === false ) {
	            this.loadReportsLandingPage( opts.data.url );           
	            return; 
	        }
    	}
    ...
    setActiveChartById: function( id ) {
        var chart = _.find( this.get( 'chartSet' ), function( set ) { return set.id == id; } );
        if( chart ) {
            this.setActiveChart( chart );
        }
        else {
            console.log( 'ReportModel:setActiveChartById: no chart found for id ' + id + '. Setting chart with index 0.' );
            this.setActiveChartByIndex( 0 );
        }
        // this.updateUrlParams({ _activeChartId : this.getActiveChartId() });
    },
    ...
    getChartById: function( id ) {
        return _.find( this.get( 'chartSet' ), function( chart ) { return chart.id == id; } );
    },

    getChartIndexById: function( id ) {
        return _.indexOf( _.pluck( this.get( 'chartSet' ), 'id' ), id );
    },

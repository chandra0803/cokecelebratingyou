/*exported ReportsPageDetailView*/
...
ReportsPageDetailView = Backbone.View.extend( {
    ...
    initialize: function( opts ) {
        ...
        colors = window.getComputedStyle( document.querySelector( this.$el.selector ), ':before' ).getPropertyValue( 'content' ).replace( /\"/g, '' ).replace( / /g, '' );
        ...
        this.model.on( 'activeChartSet', function() {
            ...
            if( thisView.charts.selectedIndex < 0 ) {
                return false;
            }
    ...
    events: {
        ...
        'click .chartThumbs li': 'handleChartSelect',
    ...
    chartsActivate: function() {
        var $chartDisplaysTop = this.$el.find( '.chartDisplaysTop' ),
            chartDisplaysTop = this.charts.selected && this.charts.selected.chartDisplaysTop || $chartDisplaysTop.data( 'default' );

        this.charts.$root.find( '.chartThumbs .activeSlide' ).removeClass( 'activeSlide' );
        this.charts.$root.find( '.chartThumbs li' ).eq( this.charts.selectedIndex ).addClass( 'activeSlide' );

        this.charts.$root.find( '#chartInFocus .active' ).removeClass( 'active' );
        this.charts.$root.find( '#chartInFocus .reportsChart-moveable-item' ).eq( this.charts.selectedIndex ).addClass( 'active' );
        ...
        G5._globalEvents.trigger(
            'navigate',
            {},
            [ this.model.get( 'id' ), this.model.getActiveChartId() ],
            {
                trigger: false,
                replace: G5.props.CURRENT_ROUTE.length <= 1 ? true : false
            }
        );
    },
        ...
        $.ajax( {
            ...
            success: function( servResp ) {
                ...
                workingChart.data.chart = $.extend(
                    ...
                    thisView.chartStyle                     // finally, override with computed chart styles
                );
    ...
    chartsGenerateFC: function( chart ) {
        ...
        // if it doesn't exist yet, render it from the start
            ...
            this.charts.FC[ chart.id ] = new FusionCharts( {
                type: chart.chartType.replace( /3D/i, '2d' ).toLowerCase(),
                id: chart.id,
                width: '100%',
                height: '100%',
    ...
    chartsRender: function() {
        var thisView = this,
            ...
            currentChartId = G5.props.CURRENT_ROUTE && G5.props.CURRENT_ROUTE[ 1 ];

        this.charts = {
            $root: this.$el.find( '#chartRow' ),
            set: modData,
            FC: {},
            selected: this.model.getChartById( currentChartId ) || this.model.getActiveChart(),
            selectedIndex: Math.max( this.model.getChartIndexById( currentChartId ), this.model.getActiveChartIndex(), 0 )
        };
        ...
        this.model.setActiveChartByIndex( this.charts.selectedIndex );
    },
    ...
    handleChartSelect: function( e ) {
        e.preventDefault();

        var id = $( e.target ).closest( 'li' ).find( 'a' ).data( 'chartId' );

        this.model.setActiveChartById( id );
    },
    ...
    renderColumnDefinitionsPopover: function() {
        var $dataContainer = this.$el.find( '#reportDefinitionListContents dl' ).children();
        ...
        var $tar = this.$el.find( '.columnDefinitions-popover' );

        $tar.popover( {
            title: null,
            content: this.$el.find( '#reportDefinitionListContents' ).html(),
            html: true,
            trigger: 'hover',
            container: '#contents',
            placement: 'top'

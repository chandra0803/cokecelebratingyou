...
RecognitionEzView = Backbone.View.extend( {
    ...
    events: {
        ...
        'click   #ezRecognitionCancelBtn': 'closeModule',
        'click   #ezRecChangeRecipLink': 'closeModule',
        'click   #ezRecModalCloseBtn': 'closeModule',
    ...
    initialize: function( opts ) {
        ...
        this.promotions = opts.promotions;
    ...
    loadRecipientInfo: function() {
        ...
        // this IF is a way to preload the recipient and promotion information to avoid having to make the second server call
        // nothing actually makes use of this right now, but PaxSearchCollectionView has a framework set up to do so
        // ...eventually
        if( this.recipient && this.promotions ) {
            requestedData = {
            ...
            this.renderRecipientRecognitionForm( requestedData );
            return;
        }
    ...
    renderRecipientRecognitionForm: function( info ) {
        var self            = this,
            ...
            data = {
                recipientNodeId: recipientInfo.nodes[ 0 ].id,
        ...
        TemplateManager.get( tplName, function( tpl ) {
            ...
            if( self.ismodal ) {
                // remove class???
                self.$el.find( '.card-profile' ).removeClass( 'card' );
            }
            self.trigger( 'templateReady' ); // broadcast status

            self.$el.find( '#ezRecognitionCommentBox, .ezRecFormBtnsWrapper:not(.empty)' ).hide();
            self.$el.find( '.ezRecFormBtnsWrapper:not(.empty)' ).show()
                    .find( '.btn' ).hide()
                    .filter( '#ezRecognitionCardBtn' ).show();

            // if we have a promotionID but no promotion object
            if( self.promotionID && !self.promotion && data.hasPromotions && data.promotions.length ) {
                self.promotion = _.find( data.promotions, function( p ) { return p.id === self.promotionID; } );
                self.promotion.promotionID = self.promotion.id;
            }

            // check to see if we have a promotion - if so set it
            if( self.promotion ) {
                self.$el.find( '#ezRecognitionPromotionSelect option[value=' + self.promotion.promotionID + ']' ).prop( 'selected', 'selected' );
                self.$el.find( '#ezRecognitionPromotionSelect' ).change();
            }
            // check to see how many promotions there are. If just one, select it
            else if( data.hasPromotions && data.promotions.length === 1 ) {
    ...
    onChangePromotion: function( event ) {
        ...
        var $selected  = $( event.currentTarget ).find( ':selected' ),
            ...
            $formBtnsOnly = $formParts.filter( '.ezRecFormBtnsWrapper' );
        ...
        if ( $selected.val() !== '' ) {
            ...
            $formParts.hide();
            $formBtnsOnly.show().find( '.btn' ).hide();
            this.$el.find( '#ezRecognitionCardBtn' ).show();
        }
    },
    ...
    postRecognition: function( event ) {
        ...
        dataSent = $.ajax( {
            ...
            success: function( serverResp ) {
                ...
                self.trigger( 'ezRecognitionSent' );
                self.closeModule();
    ...
    validateForm: function() {
        ...
        if ( !activePromotion.attributes.commentsActive ) {
            $validate = $.grep( $validate, function( el ) {
                return ( $( el ).find( '#comments' ).length !== 0 ) ? false : true;
            } );
    ...
    */
    allowOverflow: function( allow ) {
         ...
         *
         */
        var staticDiv = $( '.formContainer' ).height();
        var absoluteDiv = $( '.langs' ).outerHeight() - 150;
        var myHeight;
        ...
        if ( !allow ) {
            ...
            myHeight = staticDiv + absoluteDiv;
        }
        $( '.formContainer' ).css( 'height', myHeight );
    },

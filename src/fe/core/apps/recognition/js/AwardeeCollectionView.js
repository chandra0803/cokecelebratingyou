/*exported AwardeeCollectionView*/
/*global
TemplateManager,
...
AwardeeCollectionView = ParticipantCollectionView.extend( {
    initialize: function( opts ) {
        ...
        this.$wrapper.on( 'click', '.sameForAllTip', function( e ) {that.doSameForAllClick( e );} );
    ...
    events: {
        ...
        'change .awardPointsInp': 'doPointsKeyup',
    ...
    handlePromoNodeChange: function() {
        var promo = this.parentView.getPromoSetup(),
            ...
            $awdCashTh = this.$wrapper.find('th.awardCash'),
            $currTh = this.$wrapper.find('th.currency'),
            $budDec =  this.$wrapper.find('.budgetDeduction'),
        ...
        if( at === 'none' ) {
            ...
            $dedTh.hide();
        } else if (at === 'cash') { //Custom for Coca-cola based on wip#42701 starts
             // hide the deduction TH$currTh.show();
                $awdCashTh.css("display","table-cell");
                $currTh.css("display","table-cell");
                $awdTh.hide();
                $dedTh.hide(); //Custom for Coca-cola based on wip#42701 ends
        } 
        ...
        //set the budget owner if there is one.
        var budgetOwner = node.budgetOwner;
        var useParentBudget = node.utilizeParentBudget;
        if(budgetOwner != null && budgetOwner != undefined && useParentBudget){
            this.$wrapper.find('.budgetDeduction .budgetOwner').show().text("Budget Owner: "+budgetOwner);
        }else{
            this.$wrapper.find('.budgetDeduction .budgetOwner').hide()
        }
    ...
    addParticipant: function( participant, index ) {
        ...
        // only adding the eligible participant
        if( participant.models ) {
            _.each( participant.models, function( model ) {
               //console.log(model);
               if( !model.get( 'invalid' ) ) {that.addParticipant( model );}
            } );
            return;
        ...
        //Custom for Coca-cola based on wip#42701 starts
        json._isAwardCash = awardType === 'cash';
        //Custom for Coca-cola based on wip#42701 ends
        ...
        // Custom for Coca-cola based on wip#42701 checked awardcash type as well
        json._isShowCalcCol = !( json._isAwardNone || json._isAwardLevels || json._isAwardCash )  && this.parentView.hasBudget(); // and has a budget
        ...
        //Custom for Coca-cola based on wip#42701 starts
        if(json._isAwardCash){
            // this is silent because there are updates always after attachment
            // (see Tpl Manager get callback() )
            participant.set( {currency:promo.currency}, {silent:true} );
        }   
        //Custom for Coca-cola based on wip#42701 ends
    ...
    updateBudget: function() {
        var that = this,
            ...
            budgetToolTipClasses = 'ui-tooltip-dark remBudgTip ui-tooltip-tip',
            calcBudgRem,
            // Custom for Coca-cola based on wip#42701 added awType cash check
            isShowBudg = this.parentView.hasBudget() && awType != 'none' && awType!= 'cash', // showing the budget info?
        ...
        this.$el.find( '.participant-item[data-participant-cid]' ).each( function() {
            var $pax = $( this ),
                ...
                awd = that.getAwardQuantityFor( pax ),
        ...
        isTotalRoundingDiscrepancy = Math.round( totalDedPrecise ) !== totalDed || budgetAmount - Math.round( totalDedPrecise ) !== Math.round( budgetAmount - totalDedPrecise );
        ...
        if( isShowBudg ) {
            ...
            if( !$bar.data( 'qtip' ) ) { // lazy build the budg qtip
                ...
                setTimeout( function() {$bar.qtip( 'reposition' );}, 400 );
            // adjust qtip budget
                        ...
                        $bar.qtip().elements.content.animateNumber( calcBudgRem, 400, {},
                            function() {
                                ...
                                if( $bar.is( ':visible' ) ) {
                                    ...
                                    $bar.qtip( 'option', 'style.classes', budgetToolTipClasses +  ( calcBudgRem < 0 ? ' ui-tooltip-red' : '' ) );
                                    $bar.qtip( 'redraw' );
                                    $bar.qtip( 'reposition' );
                                }
                            }
                        );
                        //setting calcBudgRem in qtip again since value not displaying properly when server side error throws. 
                       $bar.qtip('option', 'content.text', calcBudgRem);
                    }
                    //console.log( 'slowed-down qtip' );
                }, 500 );
            }
    ...
    getBudgetRemaining: function() {
      var awType = this.parentView.getPromoSetup().awardType,
          isShowBudg = this.parentView.hasBudget() && awType != 'none';
    ...
    validateRangeOf: function( $el, emptyOk ) {
        ...
        var castNum = function( x ) { // convert strings to nums
            ...
            cashPromoType = this.parentView.getPromoSetup().promoType,
            cashMin =  pax.get( 'awardMin' ),
            cashMax =  pax.get( 'awardMax' ),
            $button = $el.parents( 'form' ).find( '#recognitionButtonSubmit' ) || $el.parents( 'form' ).find( '#recognitionButtonPreview' );
        ...
        if(cashPromoType == 'cash'){
            ...
            return false;
        } else if( this.budgetRemaining < 0 && this.parentView.activeNodeSetup.amount !== 0 ) {
            this.errorTip( $el, errTxt );
            $button.addClass( 'disabled' );
    ...
    setAllAwardRanges: function( val ) {
        ...
        this.$el.find( 'input.awardPointsInp' ).each( function() {
            var $this = $( this );
            if( $this.val() !== val && !$this.hasClass( 'optOut' ) ) {
                $this.val( val );
    ...
    showSameForAllTip: function( e ) {
        var $el = $( e.target );
        // killing this on touch at the moment as you cannot click on toolip when select loses focus
        if( $( 'html' ).hasClass( 'touch' ) && $el.hasClass( 'awardLevelSel' ) ) {
        ...
        $el.qtip( {
            ...
            position: { my: 'left center', at: 'right center', container: this.$wrapper, viewport: $( 'body' ) },
            show: { ready: true, event: false },
            hide: false,
            //only show the qtip once
            events: { hide: function() {$el.qtip( 'destroy' );} },
            style: {
    ...
    updateMerchModal: function( levelId ) {
        ...
        TemplateManager.get( 'plateauAwardsPage', function( html ) {
            ...
            plateauAwardsPageView = new PlateauAwardsPageView( {
...
//rpsv.recipientView.setAwardPointsForCalc($('.participant-item .participant'),rpsv.getCalcSetup())
    ...
    errorTip: function( $el, msg, isDestroyOnly ) {
        ...
        $el.qtip( {
            content: msg + ' <i class="icon icon-warning-circle"></i>',
            position: { my: 'left center', at: 'right center', container: this.$wrapper, viewport: $( 'body' ), adjust: { method: 'shift' } },
            show: { ready: true, delay: false },
            hide: false,
            //only show the qtip once
            events: {
    ...
    getAwardQuantityFor: function( pax ) {
        var awType = this.parentView.getPromoSetup().awardType;
        if( awType === 'levels' ) {
            return this.getValueOfLevelFor( pax );
        }else{
            if( pax.get( 'optOutAwards' ) ) {
                return 0;
            }else{
                return pax.get( 'awardQuantity' ) || 0;
            }
        }
    },

...
/*exported ProfilePageActivityHistoryTabView */
/*global
console,
$,
Backbone,
...
ProfilePageActivityHistoryTabView = Backbone.View.extend( {
    el: '#ProfilePageActivityHistoryTabView',
    initialize: function ( opts ) {
        ...
        this.on( 'templateloaded', function() {
            this.animateTabs();
            var $tar = this.$el.find( '.sa-activity-history' );        
        $tar.popover( {
            content: this.$el.find( '#saHistoryNotification' ).html(),
            html: true,
            trigger: 'click',
            container: '#contents',
            placement: 'right'
    ...
    headerSortClick: function ( hrefValue, target ) {
        ...
        $.ajax( {
            ...
            success: function ( data, status ) {
                var $newContents = $( data ),
                    $container  = $( target ).closest( '.tab-pane' ).parent();
    ...
    render: function () {
        ...
        if( this.$el.html().length === 0 ) {
            ...
            TemplateManager.get( this.tplName,
                function ( tpl ) {
                    ...
                    if( that._params && that._params.promotionId ) {
        ...
        // otherwise, just trigger the completion event
            ...
            // set any parameters that were passed to the view
            if( this._params && this._params.promotionId ) {

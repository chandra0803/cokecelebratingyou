/*exported ProfilePageBadgesTabView */
/*global
...
ProfilePageBadgesTabView = Backbone.View.extend( {
    ...
    activate: function () {
        ...
        // if we've already loaded data, bail
        // the chances of the user receiving a new badge in the meantime is too small to worry about
    ...
    postRender: function() {
        ...
        if ( this._params && this._params.length ) {
            ...
            if ( this.profilePageView.globalSidebar.isExpanded === true ) {
                this.profilePageView.globalSidebar.collapseSidebar();
            }

            $.scrollTo( this.$el.find( '#' + that._params[ 0 ] ), G5.props.ANIMATION_DURATION, {
                axis: 'y',
                onAfter: function() {
                    that.$el.find( '#' + that._params[ 0 ] ).addClass( 'background-flash' );
                    setTimeout( function() {
                        that.$el.find( '#' + that._params[ 0 ] ).removeClass( 'background-flash' );
                        G5.util.animBg( that.$el.find( '#' + that._params[ 0 ] ), 'background-flash' );
                    }, 1000 );
                }
            } );
        }

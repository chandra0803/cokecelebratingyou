/*exported ProfilePageAlertsAndMessagesTabView */
/*global
TemplateManager,
...
ProfilePageAlertsAndMessagesTabView = Backbone.View.extend( {
    ...
    activate: function () {
        'use strict';
        // Adding condition to avoid double rendering of module which triggers two API calls unneccesarily
    ...
    render: function () {
        ...
        TemplateManager.get( this.tplName,
            function ( tpl, vars, subTpls ) {
                ...
                that.$alertsTable.closest( '.row-fluid' )
                ...
                that.profilePageAlertsTabAlertsView.profilePageAlertsTabAlertsCollection.on( 'alertDataLoaded', function () {
                    ...
                    // otherwise, find the error message and show it
                        that.$alertsTable.nextAll( '.alert-info' ).show();
                    }
                } );
                that.profilePageAlertsTabMessagesView.profilePageAlertsTabMessagesCollection.on( 'messageDataLoaded', function () {
                    ...
                    // otherwise, find the error message and show it
                        that.$messagesTable.nextAll( '.alert-info' ).show();
                    }
    ...
    attachResponsiveTableToAlerts: function() {
        ...
        if ( this.$alertsTable.find( 'tbody tr' ).length === this.profilePageAlertsTabAlertsView.profilePageAlertsTabAlertsCollection.length ) {
            ...
            if ( this.$alertsTable.data( 'rT' ) && this.$alertsTable.data( 'rT' ).state === 'responsive' ) {
this.$alertsTable.data( 'rT_api' ).makeResponsive();
            } else { this.$alertsTable.responsiveTable( { pinLeft: [ 0 ] } ); }
    ...
    sortAlertsOrMessages: function( event ) {
        ...
        var $th,
                ...
                $table;
        if ( event ) {
            $th = $( '#' + event.currentTarget.id ).closest( 'th' );
            selector = $th.index();
            ...
            if ( which === 'messages' ) {
                ...
                //this.attachResponsiveTableToAlerts();
        ...
        if ( this.maxRows > this.messagesQueryString.resultsPerPage ) {
            ...
            if ( !this.tabularPagination ) {
                ...
                this.tabularPagination.on( 'goToPage', function( page ) {
                    that.messagesQueryString.pageNumber = page;
                    G5.util.showSpin( that.$messagesTable.closest( '.span12' ), { cover: true } );
                    that.datasLoaded--;
                    that.profilePageAlertsTabMessagesView.profilePageAlertsTabMessagesCollection.loadMessages( that.messagesQueryString );
                } );

                this.profilePageAlertsTabMessagesView.profilePageAlertsTabMessagesCollection.on( 'messageDataLoaded', function () {
                    that.tabularPagination.setProperties( {
                        rendered: false,
                        pages: Math.ceil( that.maxRows / that.messagesQueryString.resultsPerPage ),
                        current: that.messagesQueryString.pageNumber
                    } );
                } );
            } else {
    ...
    removeAlert: function( e, alertId ) {
        ...
        $.ajax( {
            ...
            success: function (  ) {
                ...
                that.profilePageAlertsTabAlertsView.profilePageAlertsTabAlertsCollection.remove( alertId );
                that.$el.find( '#profilePageAlertsTabAlerts tr td a[data-alertid=' + alertId + ']' ).parent().parent().parent().remove();
            ...
            if ( messageId !== undefined ) {
                ...
                that.$detailModal.one( 'shown', function() {
                    ...
                    $iframe
                        ...
                        .height( that.$detailModal.find( '.modal-body' ).height() - 10 );

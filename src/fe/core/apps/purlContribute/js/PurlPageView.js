...
PurlPageView = PageView.extend( {
    ...
    events: {
		...
		'click .translateTextLink': 'doTranslateComments',
    ...
    setupEvents: function() {
		...
		this.purlModel.on( 'translatedComments', function() {this.updateTranslatedTextComments();}, this );
    ...
    doSubmitContributor: function( e ) {
        ...
        // check if the have an img - if they do clean it
        if( $img.length ) {
            contribToSave.avatarUrl = this.purlModel.cleanImgUrl( $img.attr( 'src' ) );
        }
    ...
    doShowInviteTipBtnClick: function( e ) {
        ...
        e.preventDefault();
		/*if ( $('.row-fluid.commentItemWrapper ').length < 3){ 
			$('.purlPage#purlPageView').css('margin-bottom','280px');
		}*/
    ...
    doCancelInviteContribs: function( e ) {
		...
		this.purlModel.pendingInviteesErrorDomainEmails = null;
        this.purlModel.pendingInvitees.reset( [] );
        this.$el.find( '.inviteContributorsBtn' ).qtip( 'hide' );
		//$('.purlPage#purlPageView').css('margin-bottom','0px');
    },
    doAddEmailsToInviteList: function( e ) {
        var $emTxt = this.$el.find( '.inviteContribsEmailsInp' ),
            //Custom for TCCC - WIP-61261 - Checking for Allowed Domains to send mail invite starts 
			allowedEmailDomains = this.purlModel.attributes.purlAllowOutsideDomains ? this.purlModel.attributes.purlAllowOutsideDomains : null,
            parsedEmails = G5.util.parseEmails( $emTxt.val(), allowedEmailDomains );
        ...
        //Custom for TCCC - WIP-61261 - Checking for Allowed Domains to send mail invite starts
        this.purlModel.pendingInviteesErrorDomainEmails = parsedEmails.errorDomainEmails || null;
...
updateView: function() {
        ...
        if( m.get( 'isRecipient' ) || m.get( 'awardProcessed' ) ) {
            ...
            this.$el.find( '.lowerControls' ).hide();
        }
    ...
    updateView: function() {
        ...
        if( m.get( 'allowContribution' ) ) {
            ...
            this.$el.find( '.contributeToPurlSection' ).remove(); // remove contribute sections
        }

        // show recipient section and controls?
        if( m.get( 'isRecipient' ) || m.get( 'awardProcessed' ) ) {
            this.$el.find( '.recipientSection' ).show();
            ...
            if( m.get( 'awardProcessed' ) ) {
                ...
                this.$el.find( '.share-print' ).hide();
            }
        } else if( !m.get( 'awardProcessed' )  && !m.get( 'allowContribution' ) ) {
            this.$el.find( '.recipientSection' ).show();
            this.$el.find( '.share-print' ).hide();

        } else {
            this.$el.find( '.recipientSection' ).hide();
            // this.$el.find( '.purlDetailsSection' ).hide(); // hide recipient section
             this.$el.find( '.purlDetailsRecipientControls' ).hide();
             this.$el.find( '.purlDetailsViewerControls' ).hide();
             this.$el.find( '.lowerControls' ).hide();
    ...
    updateContribCommentsList: function( isLatestOnly ) {
        ...
        TemplateManager.get( 'commentItem', function( tpl ) {
            ...
            if( isLatestOnly ) {
                ...
                if( $cont.find( '#purlContribVideo' + single.activityId ) ) { that.initVideoJs( 'purlContribVideo' + single.activityId ); }
            }
            // full list
            else {
    ...
    updateAttachMedia_display: function() {
        var //$attSec = this.$el.find( '.attachmentSection' ),
            ...
            vidObj = this.purlModel.get( 'commentVideo' ),
        ...
        if( vidObj ) {
            $cont.append( '<div class="cmtImgWrap responsiveVideoContainer"><i class="btn btn-mini btn-icon btn-danger icon-trash rmBtn"></i>' +
                '<video id="purlContribVideo' + randId + '" class="video-js vjs-default-skin vjs-16-9 vjs-big-play-centered" controls preload="auto" data-setup="{}" poster="' + vidObj.thumbUrl + '">' +
                    '<source src="' + vidObj.url + '" type="video/' + vidObj.fileType + '">' +
                '</video></div>' );

            this.initVideoJs( 'purlContribVideo' + randId );
        }
        /*
         * this was for the old approach when "uploading" a video meant pasting in a url
        if(videoUrl) {
            // get embed
            ...
            if(videoHtml) {
                ...
                videoHtml = '<div class="videoWrapper"><i class="btn btn-mini btn-icon btn-danger icon-trash rmBtn"></i>'+
                    '<div class="responsiveVideoContainer">'+videoHtml+'</div></div>';
            }
            // no generated embedding -- let's poop out just a link then
            else {
                videoHtml = '<i class="btn btn-mini btn-icon btn-danger icon-close rmBtn"></i> <a target="_blank" href="'+videoUrl+'">'+videoUrl+'</a>';
            }
    ...
    updateInviteTip: function() {
        var that = this,
			...
			pendingDomainErrorEmails = this.purlModel.pendingInviteesErrorDomainEmails,
            invitees = this.purlModel.get( 'invitees' ),
            namedInvitees = null,
        ...
        if( pendingErrorCode ) {
            ...
            //Custom for TCCC - WIP-61261 - Checking for Allowed Domains to send mail invite starts
			if( pendingDomainErrorEmails.length > 0 ) {
                $pendingErrEms.empty();
                _.each( pendingDomainErrorEmails, function( ee ) {
                    $pendingErrEms.append( '<div>' + ee.email + '</div>' );
                } );
            }
			//Custom for TCCC - WIP-61261 - Checking for Allowed Domains to send mail invite ends
    ...
    videoBackground: function() {
        var video = document.getElementById( 'video' ),
                path = window.getComputedStyle( document.querySelector( '.purlmain-vid-container' ), ':before' ).getPropertyValue( 'content' ).replace( /"/g, '' ),
                extension = '.mp4';

            if( path.length ) {
                if( Modernizr.video && Modernizr.video.webmd ) {
                    extension = '.webm';
                ...
                video.outerHTML = '';
                delete document.getElementById( 'video' );
    ...
    // toggleMobileUploadBtn: function () {
    //     this.$el.find('.uploadInput')
    //         .toggleClass('ready');
    //     this.$el.find('.uploadBtn-mobile')
    //         .toggleClass('ready');
    // },
	...
	//events- translate text
    doTranslateComments: function( e ) {
        var $tar = $( e.target ),
            $attr = $tar.attr( 'data-id' );
        this.purlModel.translateDataComments( $attr );
        e.preventDefault();
        $tar.replaceWith( '<div class="translateLinkDisabled">' + $tar.text() + '</div>' );
    },
    updateTranslatedTextComments: function() {
        var isTranslated = this.purlModel.get( 'translatedText' ),
            self = this,
			c = this.purlModel.get( 'attr' );
        ...
        $el.find( '.uploaderFileInput' ).fileupload( {
            ...
            beforeSend: startSpin,
                ...
                if( msg.isSuccess ) {
                    ...
                    that.setImageIn( msg.picURL, $el.find( '.avatarwrap' ) );
                } else {
            ...
            beforeSend: function() {
                ...
                if( msg.isSuccess ) {
                    if( msg.vidURL ) {
                        ...
                        that.purlModel.set( 'commentVideo', {
                            url: msg.vidURL,
    ...
    setImageIn: function( url, $el ) {
        ...
        var frameUrl = url + '?ts=' + ts;
        var $img = $( '<img src="' + frameUrl + '">' );
        $el.empty().append( $img );
    }

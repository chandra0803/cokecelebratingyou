/*exported PurlContributePurlListPageView*/
    ...
    sortRows: function( event ) {
        ...
        var $table = $( '#PURLContributeListTable' ),
            ...
            sortAlpha = function() {
                $newRows.sort( function( a, b ) {
                    ...
                    if ( srtDirection === 'down' ) {
                        $target.attr( 'data-sort-direction', 'up' ).closest( 'th' ).removeClass( 'unsorted' ).addClass( 'sorted ascending' );
                        return ( aText < bText ) ? -1 : ( aText > bText ) ? 1 : 0;
                    }else{
                        $target.attr( 'data-sort-direction', 'down' ).closest( 'th' ).removeClass( 'unsorted' ).addClass( 'sorted descending' );
                        return ( aText > bText ) ? -1 : ( aText < bText ) ? 1 : 0;
                    }
                } );
            },
            ...
            clearSortClasses = function() {
                $tableHeader.find( '.sorted' ).removeClass( 'sorted' ).addClass( 'unsorted' );
                $tableHeader.find( '.sortHeader' ).removeClass( 'ascending descending' );
            };

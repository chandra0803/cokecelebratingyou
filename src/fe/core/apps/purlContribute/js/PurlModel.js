...
PurlModel = Backbone.Model.extend( {
	...
	translateDataComments: function( attr ) {
        var self = this,
            data = {
        ...
        $.ajax( {
            ...
            success: function( serverResp ) {
				...
				self.set( 'attr', attr );
                self.set( data );
                self.trigger( 'translatedComments' );
    ...
    saveComment: function( comment ) {
            ...
            data = [
                {
                    ...
                    activityId: Math.round( Math.random() * 1000000000 ),
                    userInfo: [
                        {
        ...
        $.ajax( {
            ...
            success: function( serverResp ) {
                ...
                if( msg.isSuccess ) {
                    //Updating item photo img with new img service returned img srcurl and img thumb url
                    if( item && item.media ) {
                        if( item.media[ 0 ].photo ) {
                            item.media[ 0 ].photo[ 0 ].src = msg.imageServiceUrlSrc;
                            item.media[ 0 ].photo[ 0 ].thumbSrc = msg.imageServiceUrlThumb;
                        }                        
                    }

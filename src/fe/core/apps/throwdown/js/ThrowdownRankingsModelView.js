...
/*exported ThrowdownRankingsModelView*/
/*global
console,
$,
_,
...
ThrowdownRankingsModelView = Backbone.View.extend( {
    ...
    bindRuleLink: function() {
        ...
        $( '.viewrules' ).off().click( function( e ) {
            that.displayRules( e );
        } );
    },
    ...
    findAndAddCols: function( boardJson ) {
        ...
        if( boardJson.leaders.length >= breakNum ) {
    ...
    addTopRanked: function( boardJson ) {
        var isModule = $( '.throwdownRankingsModule' ).length > 0;
    ...
    renderPagination: function() {
        var thisView = this,
            isModule = $( '.throwdownRankingsModule' ).length > 0;

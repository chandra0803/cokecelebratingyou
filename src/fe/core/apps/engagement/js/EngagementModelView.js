...
EngagementModelView = Backbone.View.extend( {
    ...
    initialize: function() {
        ...
        this.model.on( 'loadDataStarted', function() {
            this.unrenderErrors();
            G5.util.showSpin( this.$el.parent() );
        }, this );
        this.model.on( 'loadDataDone', function() {
            G5.util.hideSpin( this.$el.parent() );
            this.render();
        }, this );
    ...
    handleTabActivation: function( $tab, view ) {
        ...
        if ( detailView === this.activeDetailView ) {
            return;
        }
    ...
    loadData: function( params, url ) {
        ...
        G5.util.hideSpin( this.$el.parent() );
        G5.util.showSpin( this.$el.parent(), { cover: true } );
    }
} );

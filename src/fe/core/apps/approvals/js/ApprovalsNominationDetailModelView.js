/*exported ApprovalsNominationDetailModelView */
/*global
TemplateManager,
PageView,
ApprovalsCollection,
...
ApprovalsNominationDetailModelView = PageView.extend( {
    ...
    initialize: function( opts ) {
        ...
        // Custom for Coco cola for the wip#58122 starts
        if( $( '.levelSelectDetail').length ) {
            $( '.levelSelectDetail' ).attr( 'disabled', 'disabled' );
        }
        // Custom for Coco cola for the wip#58122 ends
        ...
        this.setLevelData(); // Custom for Coco cola for the wip#58122 
    ...
    events: {
        ...
        "change .approvalNominationStatus": "changeApprovalStatus", // Custom for Coco cola for the wip#56492
        "change .levelSelect" : "updateAward" // Custom for coco cola - wip#58122
    },
    ...
    bindStatusDropDown: function() {
        ...
        if ( dropDown.hasClass( 'nominationDetial' ) ) {
            ...
            selectedVal = dropDown.children( ':selected' ).attr( 'value' );
            if ( selectedVal === 'deny' ) {
    ...
    bindAwardRangeInput: function() {
        ...
        var maxAward = parseInt( $theInput.attr( 'data-range-max' ) );
        var minAward = parseInt( $theInput.attr( 'data-range-min' ) );
    ...
    doSendCalculatorRatings: function( $theSelects, theQtip ) {
        ...
        var theData = {
            promotionId: parseInt( $( '#promotionId' ).val() ),
    ...
    doSaveCalculatedAward: function( data, theQtip ) {
        ...
        $thisRow.find( '.calcDeduction' ).text( Math.round( parseInt( theCalcAwardObj.awardedAmount * conversionRate ) ) + ' Points' );
    ...
    setAwardPointsForCalc: function( $el, calcData ) {
        ...
        for ( i = 0; i < critLength; i++ ) {
            thisCriteria = calcData.criteria[ i ];

            $paxPar.append( "<input type='hidden' name='calculatorResultBeans[" + [ i ] + "].criteriaId' value='" + thisCriteria.criteriaId + " '>" + "<input type='hidden' name='calculatorResultBeans[" + [ i ] + "].ratingId' value='" + thisCriteria.criteriaRatingId  + " '>" + "<input type='hidden' name='calculatorResultBeans[" + [ i ] + "].criteriaRating' value='" + parseInt( thisCriteria.criteriaRating ) + "'>" );
        }
    ...
    doSameForAllCalc: function( $original, $theToolTip ) {
        ...
        $theRows.each( function() {
            ...
            $this.find( '.calcDeduction' ).text( Math.round( parseInt( that.savedCalcObj.awardedAmount * conversionRate ) ) + ' Points' );
    ...
    showRecogCalcPayout: function( anchor ) {
        ...
        populateGrid = function( $theQtip ) {
            ...
            for ( i = 0; i < that.calcObj.payTable.rows.length; i++ ) {
    ...
    updateBudgetData: function( event, fromCalc ) {
        ...
        var availableBudget = parseInt( $( '#availableBudget' ).text() ); //if no table exists, returns NaN
        ...
        if ( isNaN( availableBudget ) || availableBudget === null ) {
            ...
            if ( !fromCalc ) {
                ...
                $( '#awardPointsInput' ).each( function() {
                   ...
                   convertionRatio = parseFloat($this.parent().find( '#conversionRate' ).val()),
                   ...
                   if ( !isNaN( theVal )  ) {
                            budgetDeduction += Math.round( theCalulatedVal * convertionRatio ) ;
                   } else {
                        budgetDeduction += 0;
                   }
                ...
                theVal = parseInt( this.savedCalcObj.awardedAmount );
            ...
            remainingBudget = availableBudget - budgetDeduction;
    ...
    // Custom for Coco cola for the wip#56492 starts
    changeApprovalStatus: function() {
        var $theDropdown = $( '.approvalNominationStatus' ),
            $levelSelectDetail = $( '.levelSelectDetail' );
        if ( $levelSelectDetail.length ) {
            if ( $theDropdown.val() === 'winner' ) {
                $levelSelectDetail.removeAttr( 'disabled' );
            } else {
                $levelSelectDetail.attr( 'disabled', 'disabled' );
            }
        }
    },
    // Custom for Coco cola for the wip#56492 ends  
    ...
    // Custom for coco cola - wip#58122 and wip#56492 starts
    ...
    updateAward: function( e ) {
        var that = this,
            selectBox = $( e.target ),
            maxCapPerPax = this.levelData.capperpax,
            levelAwardValue = that.levelData.levels[ selectBox.val() ],
            teamSizeCount = $( '.teamSizeValue' ).val(),
            updateValue = ( levelAwardValue / teamSizeCount ),
            cappedUpdateValue = Math.floor( Math.min( updateValue, maxCapPerPax ) );

        $( '#awardPointsInput' ).val( cappedUpdateValue );
    }
    // Custom for coco cola - wip#58122 and wip#56492 ends
} );
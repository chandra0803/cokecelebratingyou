...
ApprovalsCollection = Backbone.Collection.extend( {
        ...
        var that = this,
            ...
            dataReturned = $.ajax( {
                ...
                success: function ( serverResp ) {
                    ...
                    //notify listener
                    that.trigger( 'loadDataFinished', approvals );
                }
            } );
    ...
    loadClaimDetailData: function () {
        ...
        $.ajax( {
            ...
            success: function ( serverResp ) {
                ...
                //notify listener
                if ( claims.length > 0 ) {
                    that.trigger( 'loadClaimDetailDataFinished', claims );
                } else {
    ...
    bindSelectAllLinks: function() {
        var that = this,
            $selectAll = $( '.approvalSearchWrapper .table-striped th a' );
    ...
    validateApprovals: function( $tr ) {
        ...
        $radio = $tr.find( "input[name*='approvalStatusType']:checked" );
        $awardInput = $tr.find( "[name*='awardQuantity']" );
        $dateBtn = $tr.find( '.awardDateIcon' );
        $levelSelect = $tr.find( '.levelSelect' ), // Custom for Coco cola for the wip#56492
        ...
        if ( $radio.val() === 'pend' || $radio.val() === 'non_winner' ) {
            ...
            $levelSelect.attr( 'disabled', 'disabled' ); // Custom for Coco cola for the wip#56492
        } else {
            $awardInput.removeAttr( 'disabled' );
            $levelSelect.removeAttr( 'disabled' ); // Custom for Coco cola for the wip#56492
    ...
    bindViewAwardLevels: function() {
        var $theLinks = $( '.viewAwardLevels' );

        $theLinks.click( function( e ) {
            ...
            G5.util.doSheetOverlay( false, href, title );
        } );
    }
} );
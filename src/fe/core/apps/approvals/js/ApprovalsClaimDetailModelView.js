/*exported ApprovalsClaimDetailModelView */
/*global
PageView,
    ...
    */
    ...
    bindApprovalStatusSelects: function () {
        ...
        if ($theSelects.size() > 1) {
            $theSelects.qtip({
                content : {
                    text: that.$el.find("#sameForAllTipTpl").html()
                },
                position : {
                    ...
                    render: function (event, api) {
                        ...
                        theQtiLink.off().click(function (event) {
                            ...
                            $theSelects.each(function (index) {
                                var $that = $(this),
                                    newStatusOption = $that.find("option[value='" + theStatusForAll + "']").val();

                                if (newStatusOption !== null && newStatusOption != 'undefined') {
                                    $that.val(theStatusForAll).trigger("change", false).qtip("hide");
                                } else {
                                    $that.val('pend').trigger("change").qtip("hide");
                                }
                            });
                        });
                    }
        ...
        */
        ...
        $theSelects.change( function ( e, showtip ) {
            ...
            if ( $this.closest( '.validateme' ).hasClass( 'error' ) ) {
                ...
                that.$claimForm.find( '.validateme select' ).qtip( 'reposition' );
                $this.focus();
            }

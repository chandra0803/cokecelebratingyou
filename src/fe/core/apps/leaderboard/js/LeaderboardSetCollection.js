/*exported LeaderboardSetCollection */
/*global
LeaderboardSetModel,
...
LeaderboardSetCollection = Backbone.Collection.extend( {
    ...
    getBoardById: function( boardId ) {
        ...
        _.each( that.models, function( set ) {
        ...
        $.ajax( {
            ...
            success: function( serverResp ) {
                ...
                _.each( data.leaderboardSets, function( s ) {
                    ...
                    that.mergeBoardSets( s, params.nameId );
                } );

                // now we can guarantee this is present
                that._chosenSet = that.get( nameId ) || that.at( 0 );
                that._chosenSet.leaderboards.setChosenBoardById( params.boardId || that._chosenSet.leaderboards.at( 0 ) );

                //console.log('[INFO] LeaderboardSetCollection - RETRIEVED leaderboard set data using params { nameId : '+nameId+', boardId : '+boardId+' }');
                // console.log('[INFO] LeaderboardSetCollection - SET _chosenSet set to ['+that._chosenSet.get('id')+']');
                // console.log('[INFO] LeaderboardSetCollection - SET _chosenBoard leaderboard to ['+that._chosenSet.leaderboards._chosenBoard.get('id')+']');

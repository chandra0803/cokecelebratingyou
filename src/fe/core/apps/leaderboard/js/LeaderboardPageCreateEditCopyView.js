/*exported LeaderboardPageCreateEditCopyView */
/*global
PageView,
...
LeaderboardPageCreateEditCopyView = PageView.extend( {
    ...
    render: function() {
        ...
        if ( this.options.notifyParticipantsOpen || this.$el.find( '#notifyPaxChecked' ).val() ) {
            // check the box - setting both of these as there is old jquery
            this.$el.find( '#notifyParticipants' ).attr( 'checked', 'checked' );
            this.$el.find( '#notifyParticipants' ).val( true );
            // trigger the toggle function
            ...
            $notifyMessage = this.$el.find( '#notifyParticipants' );
         ...
         this.setNotifyPaxChecked();
        // initialize any participant-popover links
        this.$el.find( '.participant-popover' ).participantPopover( { containerEl: this.$el } );
    ...
    formActions: function( e ) {
        ...
        if ( ( button === 'leaderboardButtonPreviewLeaderboard' ) || ( button === 'leaderboardButtonSaveDraft' ) || ( button === 'leaderboardButtonSubmit' ) || ( button === 'leaderboardButtonEdit' ) || ( ( button === 'leaderboardButtonCancel' ) && ( this.areYouSure ) ) ) {
    ...
    initializeParticipantSearch: function() {
        ...
        this.participantsView = new ParticipantCollectionView( {
            ...
            model: collection//new Backbone.Collection(participants)
        } );
    ...
    selectivelyDisableNotifyParticipants: function( hide ) {
        ...
        if ( ( !this.participantsView ) || ( this.participantsView.model.length === 0 ) ) {
            ...
            if ( this.$el.find( '#notifyParticipants' ).prop( 'checked' ) ) {
    ...
    setNotifyPaxChecked: function() {
        this.$el.find( '#notifyParticipants' ).val( this.$el.find( '#notifyParticipants' ).prop( 'checked' ) );
    },
    toggleNotifyParticipants: function( e ) {
        ...
        if ( $( e.target ).prop( 'checked' ) ) {
            ...
            // enable the textarea
            $notifyMessage.attr( 'disabled', false );
            $notifyMessage.parents( '.controls' ).slideDown( G5.props.ANIMATION_DURATION );
        } else {
            // if the trigger checkbox has been unchecked
            // disable the textarea
            $notifyMessage.attr( 'disabled', 'disabled' );
            $notifyMessage.parents( '.controls' ).slideUp( G5.props.ANIMATION_DURATION );
        }
        this.setNotifyPaxChecked();
    ...
    scrubOnBlur: function( event ) {
        ...
        if ( ( value === '' ) || ( value === '.' ) || ( parseInt( value ) === 0 ) ) {
            ...
            if ( _.indexOf( valueAsArray, '.' ) === valueAsArray.length - 1 ) {
                ...
                numericValue = parseInt( valueAsString );

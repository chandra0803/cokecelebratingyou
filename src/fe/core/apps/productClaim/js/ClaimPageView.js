...
ClaimPageView = PageView.extend( {
    ...
    updateProductData: function( $form, product ) {
        ...
        _.each( $customInputs, function( input ) { // [2]
            var $input         = $( input ),
                id             = $input.data( 'id' ),
                value          = ( $input.is( ':checkbox' ) ) ? $input.is( ':checked' ) : $input.val(),
    ...
    setCurrentPromotion: function() {
        //var self = this;
        this.participantSearchView.setAjaxParam( 'promotionId', this.activePromotion.id );
    ...
    getById: function( arr, id ) {
        ...
        return ( !_.isUndefined( stringId ) ) ? stringId : numId;
    },
    ...
    setActivePromotion: function( id ) {
        ...
        var self      = this,
            ...
            toShow    = '#claimNav, #promotionInfo, #claimFormSubmit, ' + '#claimAddProducts, #addTeamMemberSearch';
        ...
        if( $( '#addTeamMemberSearch' ).not( ':visible' ) ) {
            $( 'input#teamMembersCount' ).parent().removeClass( 'validateme' );
        }
        ...
        if ( !!self.participantSearchView ) {
            ...
            self.participantSearchView.setAjaxParam( 'promotionId', promo.id );
            self.participantSearchView.setAjaxParam( 'nodeId', $( '#orgUnit option:selected' ).val() );
    ...
    changePromotion: function( event ) {
        ...
        //reset participants
        this.participantsView.model.reset();
    ...
    initializeProductForm: function( productsArr ) {
        ...
        this.products.on( 'add remove', function() {
    ...
    validateClaimForm: function( event ) {
        ...
        var self      = this,
            ...
            validTest = function() { // [1]
                ...
                if( $( '#teamMembersCount' ).parent().hasClass( 'validateme' ) ) {
                ...
                if ( !!this.activePromotion && !!this.participantsView ) {
                    if ( !!this.activePromotion.teamActive  && this.participantsView.model.length <= 0 ) { // [1d]
                        allValid = false;
                    }
                    ...
                    if ( this.usableArray( this.activePromotion.customSections ) ) { // [e]
                       ...
                       var $validateContainer = self.$el.find( '#' + section ),
                          $validate = $validateContainer.find( '.validateme' );
    ...
    initializeParticipantSearch: function( participantsArr ) {
        ...
        this.participantsView = new ParticipantCollectionView( {
            ...
            model: collection//new Backbone.Collection(this.options.leaders)
        } );
        ...
        collection.reset( this.options.leaders );
    ...
    */
    ...
    removeSpinner: function( $trigger ) {
        ...
        $trigger.removeClass( 'disabled' ).removeAttr( 'disabled' ).spin( false );

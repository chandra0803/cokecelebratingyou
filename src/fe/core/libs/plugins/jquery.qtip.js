...
PLUGINS = QTIP.plugins = {
	...
	offset: function( elem, container ) {
		...
		// you can add 'ignorePageOffset' class to any tooltip trigger to ignore overrides
		// if the body has a fix body subtract the body scroll height
		if( ( $( 'body' ).hasClass( 'fix-body' ) ||  elem.parents( '.modal' ).length ) && !elem.hasClass( 'ignorePageOffset' ) ) {
			// check if we are not in the sidebar
			if( !elem.parents( '#sidebar' ).length ) {
		...
		if( $( 'body' ).hasClass( 'fix-sidebar'  && !elem.hasClass( 'ignorePageOffset' ) ) ) {
	...
	function calculate( side, otherSide, type, adjust, side1, side2, lengthName, targetLength, elemLength ) {
		...
		// flip/flipinvert
			...
			if( overflow1 > 0 && ( mySide !== side1 || overflow2 > 0 ) ) {
				...
				if( newMy ) {newMy[ 'invert' + side ]( side1 );}
			}

			// Check for overflow on the bottom/right
			else if( overflow2 > 0 && ( mySide !== side2 || overflow1 > 0 )  ) {
				position[ side1 ] -= ( mySide === CENTER ? -offset : offset ) + adjust;
				if( newMy ) {newMy[ 'invert' + side ]( side2 );}
			}

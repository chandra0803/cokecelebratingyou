...
GlobalFooterView = Backbone.View.extend( {
    ...
    initialize: function( opts ) {
        ...
        // if the user isn't logged in or there's no sidebar, we need to show the footer
        if( opts.loggedIn === false || this.options.noSidebar === true ) {
            $( 'body' ).addClass( 'has-footer' );
            this.render();
        } else {
            // otherwise remove the footer from the DOM
            this.$el.remove();
            G5.views.globalFooter = 'hidden';
        }

...
ModuleView = Backbone.View.extend( {
    ...
    initialize: function( opts ) {
        ...
        //console.log(filters);
        var that = this;
        // write the order the divs per filter
        _.each( filters, function( filterObj, filterName ) {
            if( filterObj.order == 'hidden' ) {filterObj.order = 10000;} // setting to a far off number so sorting works cleaner
            that.$el.data( filterName, filterObj.order );
            //that.$el.attr('data-'+filterName, filterObj.order);
        } );
    },

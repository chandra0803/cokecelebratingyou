...
WizardTabsView = Backbone.View.extend( {
    ...
    initialize: function( opts ) {
        ...
        this.on( 'beforeTabActivate', this.doBeforeTabActivate, this );
    ...
    buildTabs: function() {
        console.log( this );
        if( this.tabsTpl && this.options.tabsJson ) {
            // console.log(this.tabsTpl, this.tabsJson);
            this.$el.prepend( this.tabsTpl( this.options.tabsJson ) );
            this.setTabMinWidths();
    ...
    updateTabs: function() {
        _.each( this.tabs.models, function( t ) {
            ...
            t.$tab.attr( 'class', t.$tab.attr( 'class' ).replace( /state[A-Z][a-z]+/, '' ) );// clear state classes
    ...
    doBeforeTabActivate: function() {
    },
    doAfterTabActivate: function() {
        var that = this;

        // if a pageView has been passed into WizardTabsView, unfreeze it before scrolling
        // if a pageView has not been passed in, the individual implementations of wizardTabs will need to do the freeze checking
        if( this.options.scrollOnTabActivate ) {
            if( this.options.pageView && this.options.pageView.bodyIsFrozen === true ) {
                this.options.pageView.unfreezeBody();
            }

            $.scrollTo( this.$el, { axis: 'y', duration: G5.props.ANIMATION_DURATION / 2, onAfter: function() {
                // if a pageView has been passed into WizardTabsView, freeze it after scrolling
                if( that.options.pageView ) {
                    that.options.pageView.freezeBodyCheck();
                }
            } } );
        }
    },
    ...
    updateWidths: function() {
        ...
        this.$tabs.css( 'max-width', '' );

        if( this.$el.data( 'threshold-compact' ) >= this.$el.width() ) {
            this.$el.addClass( 'compactMode' );

            if( this.$el.data( 'threshold-supercompact' ) >= this.$el.width() ) {
                this.$el.addClass( 'superCompactMode' );
                this.$el.css( 'min-width', this.$tabs.length * this.$tabs.width() );
            }
        }

        while( this.sumTabWidths().current >= this.$el.width() && mw > 0 ) {
            this.$tabs.css( 'max-width', mw + '%' );
            mw--;
        }
    ...
    sumTabWidths: function() {
        var cur = 0,
            min = 0;

        this.$tabs.each( function() {
            cur += $( this ).width();
            min += $( this ).data( 'min-width' );
        } );

        return { current: cur, minimum: min };
    },
    setTabMinWidths: function() {
        var widestTab = 0,
            compactTab = 0;

        this.$tabs.each( function() {
            var $tab = $( this ),
                $name = $tab.find( '.wtName' ),
                width = 0;

            $tab.width( '0' );
            width = $name.outerWidth();

            $tab.css( 'min-width', width ).data( 'min-width', width );
            $tab.width( '' );

            widestTab = Math.max( widestTab, width );
        } );

        this.$el.addClass( 'compactMode' );
        compactTab = this.$tabs.not( '.active' ).width();
        this.$el.removeClass( 'compactMode' );

        this.$el.data( 'threshold-compact', this.sumTabWidths().minimum );
        this.$el.data( 'threshold-supercompact', widestTab + ( this.$tabs.length - 1 ) * compactTab );
    },
    /*
    ...
    */
    updateHeights: function() {
        ...
        $liners.css( 'height', maxH ).css( 'line-height', maxH + 'px' );

        this.$el.css( 'height', '' ).height( this.$el.height() ); // freeze height
    },
    clearHeights: function() {
        this.$el.css( 'height', '' );
        this.$el.find( '.wtTabLiner,.wtName' ).css( { height: '', 'line-height': '', 'margin-top': '' } );

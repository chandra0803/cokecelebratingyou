/*exported LoginPageFirstTimeView */
/*global
PageView,
...
LoginPageFirstTimeView = PageView.extend( {
    ...
    events: {
        ...
        'click #confirmDeclination': 'doDecline',
		...
		'change .TermsAndConditionsRadios': 'acceptTermsAndConditions'
    },
	...
	acceptTermsAndConditions: function( ) {
        $( '.nextandClearSectionBtn' ).removeAttr( 'disabled' );
    },
    ...
    scrollandClearNext: function( event ) {
        ...
        if ( $( '[name=TermsAndConditionsRadios]:checked' ).val() === 'accept' ) {
            ...
            that.checkPhoneInputs();
            that.checkEmailAddress();
        } else {
            $( '#termsAndConditions div.validateme' ).removeClass( 'validateme' ).addClass( 'validatemeoff' );
    ...
    doDecline: function() {
        this.$el.find( 'form' ).attr( 'action', G5.props.URL_DECLINE_URL );
    ...
    initPersonalInfo: function() {
        var that = this,
            ...
            $placeholder = $photoInput.siblings( '.placeholder' ),
        ...
        $photoInput.fileupload( {
            ...
            beforeSend: function() {
                ...
                if ( data.result.data.properties.isSuccess === true ) {
                    ...
                    var thumbNail = G5.util.generateTimeStamp ( data.result.data.properties.avatarUrl );

                    $( '#personalInfo' ).spin( false ); //stop the spinner

                    $imgElm.attr( 'src', thumbNail ).show();
                    $placeholder.hide();
    ...
    checkPhoneInputs: function() {
        ...
        if ( that.pageType == 'selfEnrollment' ) {
            that.$countryPhoneCode = that.$el.find( '#countryPhoneCode :selected' );
            that.$phoneNumberElm = that.$el.find( '#phoneNumber' );
            if ( that.$primaryPhoneType.val() !== null && that.$primaryPhoneType.val() !== undefined && that.$primaryPhoneType.val() !== '' ) {
                that.$countryPhoneOptions.prop( 'disabled', false );
                if ( that.$countryPhoneCode.val() !== null && that.$countryPhoneCode.val() !== undefined && that.$countryPhoneCode.val() !== '' ) {
                    that.$phoneNumberElm.prop( 'disabled', false );
                } else {
                    that.$phoneNumberElm.val( '' ).prop( 'disabled', true );
                }
            } else {
                that.$countryPhoneOptions.prop( 'disabled', true );
                that.$phoneNumberElm.val( '' ).prop( 'disabled', true );
            }
        }
    },
    checkEmailAddress: function() {
        var that = this;
        that.$emailType = that.$el.find( '#emailType :selected' );
        that.$emailAddressElm = that.$el.find( '#emailAddress' );
        //Check form type, if user is on selfEnrollment page, the phoneNumber has diffrent validation rules than on the firstTimeLogin page.
        if ( that.pageType == 'selfEnrollment' ) {
            if ( that.$emailType.val() !== null && that.$emailType.val() !== undefined && that.$emailType.val() !== '' ) {
                that.$emailAddressElm.prop( 'disabled', false );
            } else {
                that.$emailAddressElm.prop( 'disabled', true );
            }
        }
    },
    checkIE: function() {
    ...
    showUploadedPhoto: function() {
        ...
        if ( $photoUrl !== undefined && $photoUrl !== '' ) {
            ...
            $thumbnail.show();
            this.$el.find( '#personalInfo span.avatar').hide();
        }
    },

/*exported ParticipantPaginatedView */
/*global
PaxCollection,
TemplateManager,
Backbone,
...
ParticipantPaginatedView = Backbone.View.extend( {
    initialize: function( opts ) {
        ...
        this.model.paxes = opts.participants || new PaxCollection();
    ...
    updateSortControls: function() {
        var $sc = this.$el.find( '.sortHeader' ),
            that = this;
        ...
        $sc.each( function() {
            ...
            // clear styles
            $t.closest( '.sortHeader' ).removeClass( 'sorted' ).addClass( 'unsorted' );
            $t.removeClass( 'asc desc' );
            // add styles to apropo
            if( s === that.model.get( 'sortedOn' ) ) {
                $t.closest( '.sortHeader' ).addClass( 'sorted' ).removeClass( 'unsorted' );
                $t.addClass( that.model.get( 'sortedBy' ) === 'asc' ? 'asc' : 'desc' );
            }
        } );
    },
    ...
    doSort: function( e ) {
        ...
        var $tar = $( e.target ),
            $s = $tar.hasClass( 'sortHeader' ) ? $tar : $tar.closest( '.sortHeader' ),
            on = $s.data( 'sort' ),
    ...
    //     this.model.set( 'recordsTotal', this.model.paxes.length );
    // },

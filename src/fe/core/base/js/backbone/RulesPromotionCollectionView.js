/*exported RulesPromotionCollectionView */
/*global
TemplateManager,
...
RulesPromotionCollectionView = Backbone.View.extend( {
    ...
    updateRules: function( e ) {
        ...
        this.displayRulesById( selectedLink );

        $( '#col-left' ).find( 'li' ).removeClass( 'active' );
        $( '#rules' + selectedLink ).parent().addClass( 'active' );
    },

    displayRulesById: function( id ) {
        this.$el.find( '#rulesTitle' ).html( this.collection.get( id ).get( 'name' ) );
        this.$el.find( '#rulesContent' ).html( this.collection.get( id ).get( 'content' ) );
    }

...
AwardeeCollectionView = ParticipantCollectionView.extend({
    ...
    handlePromoNodeChange: function(){
        ...
        } else if (at === 'pointsFixed') { //Custom for Coca-cola
            $awdCashTh.hide();
            $currTh.hide();
            // display 'award' TH
        } else if (at === 'levels') {
    ...
    addParticipant:function(participant, index){
        ...
        json._OptOut = json.isOptOut;       
        
        // match data coming in @ top of form
        json._paxName = this.parentView.recipFormNamePrefix;
        //Custom for Coca-cola
        json._isShowCalcCol =
            !(json._isAwardNone||json._isAwardLevels||json._isAwardCash) // not none or level awtype
    ...
    updateBudget: function(){
        var that = this,
            ...
            calcBudgRem,
            isShowBudg = this.parentView.hasBudget()&&awType!='none'&&awType!='cash', // showing the budget info? //Custom for Coca-cola
    ...
    doSameForAllCalc: function($anchor, $theToolTip, theCalcAwardObj) {
        ...
        if ($theRows.length > 0 ){
            ...
            $theRows.each(function() {
                ...
                if ($thisAnchor.length > 0) {  // WIP-40443 checking the calclink button length and proceed further  

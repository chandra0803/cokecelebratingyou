...
RecognitionPageSendView = PageView.extend({
    ...
    initialize:function(opts){
        ...
        this.uploadFormNamePrefix = 'claimUploadFormBeans';
        ...
        this.initForm();
		// START Document Uploads CUSTOM
		
       
        if (that.usableArray(this.formSettings.upload)) { // if uploads are set
             that.initializeDocumentUploads(this.formSettings.upload);
        } else {
              that.initializeDocumentUploads();
         }
	  
		
	  // END Document Uploads CUSTOM
    ...
    initPromotionSection: function($section){
        ...
        // Add sorting for dropdown items excluding the first options - for Coke
        defaultOption = $promo.find('option:first');
        promoOptions = $promo.find ('option:not(:first)').sort(function(a, b) {
           return a.text == b.text ? 0 : a.text < b.text ? -1 : 1
        });

        $promo.html(promoOptions).prepend(defaultOption); 
    ...
    initTeamSection: function($section){
    	/// Custom for Celebrating you - added nominationCopyBlock dom for wip#59418
        var $tn = $section.find('#teamName');
        var $copyBlock = $section.find('.nominationCopyBlock strong');
        if(this.activePromotionSetup&&this.activePromotionSetup.teamActive){
        	var $copyContent = this.activePromotionSetup.teamNameCopyBlock;
            ...
            $copyBlock.empty().html($copyContent);
        } else {
            //hide and detach
            this.detachSection($section);
        }
    },
	initUploadSection: function($section) {			
		var $up = $section.find('#recognitionFieldsetUpload');
		if(this.activePromotionSetup&&this.activePromotionSetup.uploadDocument){
            this.attachSection($section);
			this.$el.find('#recognitionFieldsetUpload').show();
			this.setDocumentUploadValidation(this.activePromotionSetup);
            if(this.formSettings.uploadDocument) { 
                $up.val(this.formSettings.uploadDocument);
                delete this.formSettings.uploadDocument;
            }
        } else {
            //hide and detach
            this.detachSection($section);
    ...
    initCustomSection: function($sec) {
        if(this.activePromotionSetup&&this.activePromotionSetup[$sec.attr('id')+'Active']) {
            ...
            // Custom for Coke hiding the more infoprmation header if the hideCommentsOnNomination flag is set to true wip#55713
            if ( this.activePromotionSetup.hideCommentsOnNomination ) {
            	$sec.find('h2').hide();
            }
            else {
            	$sec.find('h2').show();
    ...
    promoChanged: function(e){
        var pVal = this.$promo.val(),
            ...
            customAttr = this.$promo.find(':selected').attr('custom-url');

        // view level access to active form setup (promo object)
        this.activePromotionSetup = this.getFormSetupByPromoId(pVal);
        
        // Redirect to REcognition anyone page incase custom dropdown value wip-51838
        if ( customAttr ) {
            window.location.replace ( customAttr );                     
        }
		...
		// Custom for NCR - Add additional form attribute if the promotion is nomination

        // do stuff dependent on node and promo
        this.nodeOrPromoChanged();
		
        this.setDocumentUploadValidation();
        
		// Custom for NCR - Add additional form attribute if the promotion is nomination
		var $form = $('#sendForm');	
		if(this.isNomination()) {			
			$form.attr('enctype',$('#encodeType').val());
		}
		else {
    ...
    validateForm: function(e) {
		...
		// BEGIN CUSTOM
         ...
         if( !G5.util.formValidate(this.$el.find('#uploadedDocumentsCount')) ) {
            allValid = false;
          }
    ...
    parseForm: function($f){
        ...
        json.upload = this.parseObjectsFromForm(arry, that.uploadFormNamePrefix);

        return json;
    },

    /**
     * Sets properties on the hidden validation elements to ensure
     * the proper messages for min/max Document uploads are displayed.
     */
    setDocumentUploadValidation: function () {
        "use strict";
        var $uploadedDocumentsCount = $("#uploadedDocumentsCount"),
            $validationWrapper = $uploadedDocumentsCount.parent(),
            flags = [],
            failMessages = {},
            minDocumentRequired = $("#minDocumentRequired").val(),
            maxDocumentRequired = $("#maxDocumentRequired").val(),
            minMessage = $validationWrapper.attr("data-message-template-minvalue"),
            maxMessage = $validationWrapper.attr("data-message-template-maxvalue"),
		...
		if($('input[name*=claimUploadFormBeansCount]').val() == 0) {
			$uploadedDocumentsCount.val("0");		
		}
		else {
			$uploadedDocumentsCount.val($('input[name*=claimUploadFormBeansCount]').val());
		}
        

        // Remove existing validation
        $validationWrapper.removeAttr("data-validate-flags");
        $validationWrapper.removeAttr("data-validate-min-value");
        $validationWrapper.removeAttr("data-validate-max-value");
        $validationWrapper.removeAttr("data-validate-fail-msgs");
		
		if(this.activePromotionSetup.maxDocumentAllowed == $('input[name*=claimUploadFormBeansCount]').val()) {
			$(".upload-button").css("display", "none");
		}
		

       // Set min validation
       if (jQuery.isNumeric(minDocumentRequired)) {
            flags.push("minvalue");
            failMessages.minvalue = format(minMessage, minDocumentRequired);
            $validationWrapper.attr("data-validate-min-value", minDocumentRequired);
        }

        // Set max validation
        if (jQuery.isNumeric(maxDocumentRequired)) {
            flags.push("maxvalue");
            failMessages.maxvalue = format(maxMessage, maxDocumentRequired);
            $validationWrapper.attr("data-validate-max-value", maxDocumentRequired);
        }


        // Apply validation and messages
        if (flags.length > 0) {
            $validationWrapper.attr("data-validate-flags", flags.join(","));
            $validationWrapper.data("validateFailMsgs", failMessages);
        }
    },

    /**
     * Creates the document uploading view
     * @param {Array} uploads Existing uploaded files and their descriptions
     */
    initializeDocumentUploads: function (uploads) { 
        'use strict';
        var that = this;

        $(".upload-button input").fileupload({
            url: G5.props.URL_JSON_NOMINATION_FILE_UPLOAD,
            dataType: 'g5json',			
            add: function(e, data) {
                'use strict';
                var uploadErrors = [],
                    /* CUSTOM STARTS HERE
                    acceptFileTypes = /(\.|\/)(gif|jpe?g|png|pdf|ppt|pptx|psa|doc|xls|xlsx)$/i,
                    */
                    acceptFileTypes = new RegExp(G5.props.NCR_DOCUMENT_UPLOAD_FILE_TYPES, 'i'),
                    /* CUSTOM ENDS HERE */
                    extension = "." + data.originalFiles[0].name.split(".").pop();
                if (extension.length && !acceptFileTypes.test(extension)) {
                    uploadErrors.push('Not an accepted file type');
                }
                /* CUSTOM STARTS HERE
                if (typeof(data.originalFiles[0]['size']) !== 'undefined' && data.originalFiles[0]['size'] > 3000000) { // 3000000 = 3MB
                */
                if (typeof(data.originalFiles[0]['size']) !== 'undefined' && data.originalFiles[0]['size'] > G5.props.SYSTEM_DOCUMENT_UPLOAD_SIZE_LIMIT) {
                /* CUSTOM ENDS HERE */
                ...
                if (uploadErrors.length > 0) {
					...
					$(this).parent().prevAll('.progress').css('display', 'block');
                    // Upload the file
                    data.submit();
                }
            },
            beforeSend: function (e, data) {console.log("[file upload] beforeSend");},
            success: function (e, data) {console.log("[file upload] success");},
            fail: function (e, data) {console.log("[file upload] fail", e, data);},
            progress: function (e, data) {
                var progress = parseInt(data.loaded / data.total * 100, 10);
                $(this).parent().prevAll('.progress').find('.progress-bar').css(
                    'width',
                    progress + '%'
                );
            },
                ...
                var result = data.result.data.properties || data.result.data;
                ...
                $(this).parent().prevAll('.progress').css('display', 'none');

                }
                // Display uploaded files
                that.setUploadedFiles(result.files);
        ...
        if (uploads) {			
            // Load the map of file urls to descriptions			
            that.uploadedFileDescriptions = that.uploadedFileDescriptions || {};			
            $.each(uploads, function (index, upload) {
                that.uploadedFileDescriptions[upload.url] = upload.description;
            });
            // Create view
            that.setUploadedFiles(uploads.map(function (upload) { return upload.url; }));			
        }
        else {
    ...
    /**
     * Updates the file upload section, keeping any entered file descriptions.
     * @param fileUrlArray
     */
    setUploadedFiles: function (fileUrlArray) {
        'use strict';
        var that = this,
            $uploadedFileList = $(".uploaded-file-list"),
            $mapElements = $uploadedFileList.find(".uploaded-file-map"),
            $descriptions = $uploadedFileList.find(".uploaded-file-description"),
            maxFiles = $("#uploadedDocumentsCount").parent().attr("data-validate-max-value");

        // Set the number of uploaded files so that the form validation can check it when the form is submitted
        $("#uploadedDocumentsCount").val(fileUrlArray.length);

        // Keep a map of file url to descriptions
        that.uploadedFileDescriptions = that.uploadedFileDescriptions || {};
        $.each($mapElements, function (index, mapElement) {
            that.uploadedFileDescriptions[$(mapElement).val()] = $($descriptions.get(index)).val();
        })

        // Empty out the uploaded file list in the DOM and rebuild
        $uploadedFileList.empty();
        $.each(fileUrlArray, function (index, fileUrl) {
            // Get any existing description for the file
            var description = that.uploadedFileDescriptions[fileUrl] || "";
            // Create the display for the file
            that.createUploadedFileView.call(that, fileUrl, index, description).appendTo($uploadedFileList);
        });

        // Hide the Browse button if the promotion does not allow anymore uploads
        if ($.isNumeric(maxFiles) && fileUrlArray.length >= parseInt(maxFiles, 10)) {
            $(".upload-button").css("display", "none");
        }
        else {
    ...
    /**
     * Creates the elements that display an uploaded file, including the description field.
     * @param fileUrl Url of the uploaded file
     * @param index Used for unique field names
     * @param description Any existing description associated with the file
     * @returns {*|jQuery|HTMLElement}
     */
    createUploadedFileView: function (fileUrl, index, description) {
        'use strict';
        var that = this,
            $temp = $("<div/>"),
            $el,
        ...
        TemplateManager.get("uploadedFileTemplate", function(tpl) {
            $temp.html(tpl({
        ...
        $el = $($temp[0].children[0]);

        $deleteButton = $el.find(".remove-uploaded-file");
        $deleteButton.click({"fileUrl": fileUrl, "uploadedFileViewElement": $el}, function (e) {
            that.requestUploadedFileRemoval.call(that, e.data.fileUrl, e.data.uploadedFileViewElement);
    ...
    /**
     * Send a request to the server to remove the uploaded file
     * @param fileUrl File to remove
     * @param $uploadedFileViewElement jQuery reference to the element displaying the file in the page
     */
    requestUploadedFileRemoval: function (fileUrl, $uploadedFileViewElement) {
        'use strict';
        var that = this;
        $.ajax({
            "url": G5.props.URL_JSON_NOMINATION_FILE_REMOVE,
            "dataType": "g5json",
            "method": "POST",
            "data": {
                "fileUrl": fileUrl
            ...
            "success": function (data, textStatus, jqXhr) {
                'use strict';
                var uploadedFileViewIndex = $uploadedFileViewElement.index(),
                    result = data.data.properties || data.data;

                //error validation
                if ( !result.isSuccess ) {
                    alert("Document removal failed");
                }
                else {
                    ...
                    // Set focus
                    $uploadedFileViewElement = $(".uploaded-file-list").find(".uploaded-file-view:eq(" + uploadedFileViewIndex + ")");
                    if ($uploadedFileViewElement.length === 0) {
                        $(".upload-button").focus();
                    }
                    else {
                        $uploadedFileViewElement.find("button").focus();
                    }
                }
            },
            "error": function (jqXhr, textStatus, errorThrown) {
                console.log("[ERROR] ClaimPageView.requestUploadedFileRemoval: ", jqXhr, textStatus, errorThrown);
            }
        });
    },
	// END NCR Document Uploads CUSTOM
	...
	// Custom for NCR - Adding addition property 
	isNomination: function(){
        return this.activePromotionSetup && this.activePromotionSetup.promoType === 'nomination';
    },

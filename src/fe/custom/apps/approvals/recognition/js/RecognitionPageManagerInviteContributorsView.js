...
RecognitionPageManagerInviteContributorsView = PageView.extend({
    ...
    initialize: function(opts) {
		...
		this.recipFormNamePrefix = 'claimRecipientFormBeans'; //Custom for TCCC - WIP-26532
	...
	//Custom for TCCC - WIP-26532 - Setting all values to model
	getRecipientView: function() {
        var rv = {};
        rv.model = new Backbone.Collection();
        if (this.getFormSettings().recipients) {
            rv.model.add(this.getFormSettings().recipients);
        }
        return rv;
    },
    ...
    parseForm: function($f) {
        ...
        json.recipients = this.parseObjectsFromForm(arry, that.recipFormNamePrefix); //Custom for TCCC - WIP-26532 - Appending recipients objects to formSettings

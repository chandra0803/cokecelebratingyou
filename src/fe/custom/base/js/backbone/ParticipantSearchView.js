...
ParticipantSearchView = Backbone.View.extend({
    ...
    events:{
        ...
        "click #recognitionPageSendView .participantSearchResultRow":"doParticipantSelectt",		
    ...
    doParticipantSelect: function(e){
        var $tar = $(e.target).closest('tr').find('.participantSelectControl'),
			...
			//custom
			toRender = this.model.getParticipants().models,
			checkCurrency = _.where(toRender, {id: pid}),
			currency = $('.recipientsSection th').hasClass("currency"),
			currencyError = $('.recipientsSection th.currency').data('msgCurrency');
        ...
        //else we are selecting
			...
			//custom
			if($('.recipientsSection th.currency').is(":visible")){
				if(checkCurrency[0].attributes.currency !='' && checkCurrency[0].attributes.currency !=undefined){
					//this.trigger('doParticipantSelectNoError', $tar);
					...
					this.errorTip($tar, currencyError);
    ...
    errorTip: function($el,msg,isDestroyOnly){
        ...
        $el.qtip({
            ...
            show:{ready:true,delay:false,event:true},
            hide: { event : 'unfocus',
                    fixed : true, delay : 200 },

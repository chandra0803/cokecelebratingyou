 ...
 *
 */
public class StackRankListController extends BaseController
{
  ...
  public void onExecute( ComponentContext tileContext, HttpServletRequest request, HttpServletResponse response, ServletContext servletContext ) throws Exception
  {
    ...
    // create the promotion query constraint
    ...
    constraint.setPromotionStatusTypesIncluded( new PromotionStatusType[] { PromotionStatusType.lookup( PromotionStatusType.LIVE ) } );
    ...
    // now get the count (of StackRank obj, by passing state(StackRankState),promotionId using
    // StackRankQueryConstraint
    // for above promotion list
    for ( Iterator iter = promotionStackRankList.iterator(); iter.hasNext(); )
    {
      ...
      // get the count now,
      int createCount = getStackRankService().getStackRankListCount( queryConstraint );

      // set the state - approve
      ...
      // get the count now,
      int toBeApproveCount = getStackRankService().getStackRankListCount( queryConstraint );
      ...
      bean.setDisplayCreate( createCount == 0 );
      bean.setDisplayApprove( toBeApproveCount > 0 );
      bean.setDisplayHistory( historyCount > 0 );

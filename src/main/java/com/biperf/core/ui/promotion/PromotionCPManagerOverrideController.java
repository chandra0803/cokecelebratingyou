 ...
 * $Source: /usr/local/ndscvsroot/products/penta-g/src/javaui/com/biperf/core/ui/promotion/PromotionCPManagerOverrideController.java,v $
 */
...
/**
 * Implements the controller for the Manager override page for Challengepoint.
 * <p>
 ...
 *
 */
public class PromotionCPManagerOverrideController extends BaseController
{
  /**
  ...
  public void onExecute( ComponentContext context, HttpServletRequest request, HttpServletResponse response, ServletContext servletContext ) throws Exception
  {
    ...
    ChallengePointPromotion promotion = (ChallengePointPromotion)getPromotionService().getPromotionByIdWithAssociations( promoManagerOverrideForm.getPromotionId(), promoAssociationRequestCollection );
    ...
    // List overrideStructureList = CPOverrideStructure.getList();
    ...
    {
      List newOverrideStructureList = new ArrayList( overrideStructureList.size() - 1 );
      for ( Iterator overrideStructureIter = overrideStructureList.iterator(); overrideStructureIter.hasNext(); )
      {
        ManagerOverrideStructure overrideStructure = (ManagerOverrideStructure)overrideStructureIter.next();
        if ( !overrideStructure.getCode().equals( ManagerOverrideStructure.OVERRIDE_PERCENT ) && !overrideStructure.getCode().equals( ManagerOverrideStructure.AWARD_PER_ACHIEVER ) )
        {
          newOverrideStructureList.add( overrideStructure );
        }
      }
      request.setAttribute( "overrideStructureList", newOverrideStructureList );
    }
    else
    {
    ...
    if ( ObjectUtils.equals( promoManagerOverrideForm.getPromotionStatus(), PromotionStatusType.EXPIRED )
        || ObjectUtils.equals( promoManagerOverrideForm.getPromotionStatus(), PromotionStatusType.LIVE ) && promotion.getGoalCollectionStartDate().before( new Date() ) )
    {
  ...
  private void populateSaveTilesAttribute( ComponentContext context, String pageType, ChallengePointPromotion promotion )
  {
    if ( pageType != null )
    {

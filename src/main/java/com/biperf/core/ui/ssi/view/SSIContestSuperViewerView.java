...
public class SSIContestSuperViewerView
{
  ...
  public SSIContestSuperViewerView()
  {
    super();
  }

  public SSIContestSuperViewerView( WebErrorMessage message )
  {
    this.messages = new ArrayList<WebErrorMessage>();
    this.messages.add( message );
  }

  public SSIContestSuperViewerView( List<SSIContestParticipantValueBean> participants, int recordsCount, String sortedOn, String sortedBy )
  {
    this.currentPage = 1;
  ...
  public SSIContestSuperViewerView( List<SSIContestSuperViewer> contestSuperviewers, Integer superViewersCount, String sortedBy, String sortedOn, Integer currentPage, int recordsPerPage )
  {
    if ( contestSuperviewers != null )
    {
      this.recordsTotal = contestSuperviewers.size();
      ...
      for ( SSIContestSuperViewer contestSuperviewer : contestSuperviewers )
      {
        ...
        DynaPickListType departmentItem = DynaPickListType.lookup( DepartmentType.PICKLIST_ASSET, contestSuperviewer.getSuperViewer().getDepartmentType() );
        DynaPickListType jobPositionItem = DynaPickListType.lookup( PositionType.PICKLIST_ASSET, contestSuperviewer.getSuperViewer().getPositionType() );
        ...
        participant.setFirstName( contestSuperviewer.getSuperViewer().getFirstName() );
        participant.setLastName( contestSuperviewer.getSuperViewer().getLastName() );
        participant.setOrgName( contestSuperviewer.getSuperViewer().getPrimaryUserNode().getNode().getName() );
        participant.setOrgType( contestSuperviewer.getSuperViewer().getPrimaryUserNode().getNode().getNodeType().getName() );
        participant.setId( contestSuperviewer.getSuperViewer().getId() );
        participant.setUserId( contestSuperviewer.getSuperViewer().getId() );
        if ( Hibernate.isInitialized( contestSuperviewer.getSuperViewer().getUserAddresses() ) )
        {
          participant.setCountryName( contestSuperviewer.getSuperViewer().getPrimaryCountryName() );
          participant.setCountryCode( contestSuperviewer.getSuperViewer().getPrimaryCountryCode() );
        }
    ...
    this.recordsTotal = superViewersCount;

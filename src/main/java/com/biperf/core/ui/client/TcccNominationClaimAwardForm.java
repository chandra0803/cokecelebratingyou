...
public class TcccNominationClaimAwardForm extends BaseForm
{
  ...
  private String award_type_selected;
  ...
  private boolean levelPaxClaim;
  private boolean optTraining;
  private Long awdQuantity;
  ...
  public ActionErrors validate( ActionMapping actionMapping, HttpServletRequest request )
  {
    ...
    if ( StringUtil.isEmpty( this.award_type_selected ) )
    {
  ...
  public void load( AbstractRecognitionClaim claim,NominationClaim nc )
  {
    ...
    for ( ClaimRecipient claimParticipant : claim.getClaimRecipients() )    
    {
    	if( null != claimParticipant.getRecipient() && null != claimParticipant.getRecipient().getId()
    			&& claimParticipant.getRecipient().getId().equals(UserManager.getUserId()))
    	{
      //this.claimItemId = claimRecipient.getId();
      /*this.currencyCode = claimRecipient.getCashCurrencyCode();
      this.currencyFullAmt = claimRecipient.getCashAwardQuantity();
      this.currencyFullAmtString = NumberFormatUtil.getLocaleBasedNumberFormat( claimRecipient.getCashAwardQuantity() );

      this.currencyHalfAmt = TcccClientUtils.convertToPercentCash( getSystemVariableService(), claimRecipient.getCashAwardQuantity() );
      this.currencyHalfAmtString = NumberFormatUtil.getLocaleBasedNumberFormat( this.currencyHalfAmt );

      Long usdCashAmt = TcccClientUtils.convertToUSD( getCokeClientService(), claimRecipient.getCashAwardQuantity(), claimRecipient.getCashCurrencyCode() );
...
*/
      this.recipientId = claimParticipant.getRecipient().getId();
      this.recipientName = claimParticipant.getRecipient().getNameLFMWithComma();
      this.recipientAvatar = claimParticipant.getRecipient().getAvatarSmallFullPath();
      this.recipientCountryCode = claimParticipant.getRecipient().getPrimaryCountryCode();
      this.recipientCountryName = claimParticipant.getRecipient().getPrimaryCountryName();
      this.recipientDeptName = claimParticipant.getRecipient().getPaxDeptName();
      this.recipientJobName = claimParticipant.getRecipient().getPaxJobName();
      this.awdQuantity = claimParticipant.getAwardQuantity(); 
...
public boolean isLevelPaxClaim() {
	return levelPaxClaim;
}

public void setLevelPaxClaim(boolean levelPaxClaim) {
	this.levelPaxClaim = levelPaxClaim;
}
...
public Long getAwdQuantity() {
	return awdQuantity;
}

public void setAwdQuantity(Long awdQuantity) {
	this.awdQuantity = awdQuantity;
}

public String getAward_type_selected() {
	return award_type_selected;
}

public void setAward_type_selected(String award_type_selected) {
	this.award_type_selected = award_type_selected;
}

...
import com.biperf.core.domain.throwdownnews.ThrowdownNewsDetailsView;
import com.biperf.core.domain.throwdownnews.ThrowdownNewsView;
...
public class NewsAction extends BaseDispatchAction
{
  ...
  public ActionForward communicationsList( ActionMapping mapping, ActionForm form, HttpServletRequest request, HttpServletResponse response ) throws Exception
  {
    ...
    List<ThrowdownNewsDetailsView> communications = new ArrayList<ThrowdownNewsDetailsView>();
    communications = buildCommunicationJson( communicationList, baseUri );
    ThrowdownNewsView commView = new ThrowdownNewsView();
  ...
  public ActionForward communicationsDetail( ActionMapping mapping, ActionForm form, HttpServletRequest request, HttpServletResponse response ) throws Exception
  {
    ...
    List<Content> activeCommunicationsList = (List<Content>)ContentReaderManager.getContentReader().getContent( "home.throwdown.news" );
    String baseUri = RequestUtils.getBaseURI( request );
  ...
  @SuppressWarnings( "unchecked" )
  public List<ThrowdownNewsDetailsView> buildCommunicationJson( List<Content> communicationList, String baseUri )
  {
    List<ThrowdownNewsDetailsView> communications = new ArrayList<ThrowdownNewsDetailsView>();
    ...
    for ( Content commContent : communicationList )
    {
      ...
      String storySlug = baseUri + "/throwdownNewsDetail.do";
      ...
      ThrowdownNewsDetailsView commDetails = new ThrowdownNewsDetailsView();
    ...
    if ( communications.size() > 5 )
    {
      return communications.subList( 0, 5 );
    }
    else
    {
  ...
  {
    List<Content> contents = (List)ContentReaderManager.getContentReader().getContent( "home.throwdown.news" );

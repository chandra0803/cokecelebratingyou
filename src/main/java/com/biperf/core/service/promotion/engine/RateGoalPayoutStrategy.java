 ...
 * $Source: /usr/local/ndscvsroot/products/penta-g/src/java/com/biperf/core/service/promotion/engine/RateGoalPayoutStrategy.java,v $
 */
 ...
 *
 */
public class RateGoalPayoutStrategy extends AbstractGoalPayoutStrategy
{
  ...
  protected GoalCalculationResult processGoalInternal( PaxGoal paxGoal )
  {
    ...
    if ( paxGoal != null && paxGoal.getGoalQuestPromotion() != null && paxGoal.getGoalLevel() != null && ( (GoalLevel)paxGoal.getGoalLevel() ).getMinimumQualifier() != null )
    {
      ...
      {
        amountToAchieve = goalLevel.getMinimumQualifier();
        amountToAchieve = amountToAchieve.add( goalLevel.getIncrementalQuantity() );
        ...
        if ( meetsAchievement( overriddenValue, amountToAchieve, promotion.getAchievementPrecision(), promotion.getRoundingMethod() ) )
        {
          ...
          calculateFixedAmountForRateAndBoth( overriddenValue, paxGoal, goalCalculationResult, true );
        }
      }
      else if ( AchievementRuleType.lookup( AchievementRuleType.PERCENT_OF_BASE ).equals( promotion.getAchievementRule() ) )
      {
        if ( paxGoal.getBaseQuantity() != null )
        {
          BigDecimal calculatedMinimumQualifier = goalLevel.getMinimumQualifier().movePointLeft( 2 ).multiply( paxGoal.getBaseQuantity() );
          BigDecimal calculatedIncremental = goalLevel.getIncrementalQuantity().movePointLeft( 2 ).multiply( calculatedMinimumQualifier );
          goalCalculationResult.setCalculatedIncremental( calculatedIncremental );
          amountToAchieve = calculatedMinimumQualifier.add( calculatedIncremental );
          ...
          {
            goalCalculationResult.setAchieved( true );
            calculatePercentBaseForRateAndBoth( overriddenValue, paxGoal, goalCalculationResult, true );
          }
        }
      }
    }

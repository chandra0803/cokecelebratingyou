 ...
 * $Source: /usr/local/ndscvsroot/products/penta-g/src/java/com/biperf/core/service/promotion/engine/ChallengePointAchievementBasePlusFixedStrategy.java,v $
 */
 ...
 *
 */
public class ChallengePointAchievementBasePlusFixedStrategy extends AbstractChallengePointAchievementStrategy
{
  ...
  protected ChallengePointCalculationResult processChallengePointInternal( PaxGoal paxGoal )
  {
    ...
    if ( paxGoal != null && paxGoal.getGoalQuestPromotion() != null && paxGoal.getGoalLevel() != null && ( (GoalLevel)paxGoal.getGoalLevel() ).getAchievementAmount() != null )
    {
      ...
      if ( AchievementRuleType.lookup( AchievementRuleType.BASE_PLUS_FIXED ).equals( promotion.getAchievementRule() ) )
      {
        if ( promotion.getAchievementPrecision().getCode().equals( "two" ) && goalLevel.getAchievementAmount() != null )
        {
          ...
          {
            amountToAchieve = amountToAchieve.add( paxGoal.getBaseQuantity().setScale( 2 ) );
          }
        }
        else
        {

...
public class SSIObjContestUsingPrcImportStrategy extends ImportStrategy
{
  private SSIContestService ssiContestService;
  private static final Log logger = LogFactory.getLog( SSIObjContestUsingPrcImportStrategy.class );
  ...
  public void importImportFile( ImportFile importFile, List records ) throws ServiceErrorException
  {
    ...
    Map<String, Object> result = ssiContestService.importImportFile( importFile.getId(), importFile.getContestId(), SSIContestType.OBJECTIVES );
  ...
  {
    Map<String, Object> result = ssiContestService.verifyImportFile( importFile.getId(), importFile.getContestId(), SSIContestType.OBJECTIVES );
    int errorCount = ( (BigDecimal)result.get( "p_total_error_rec" ) ).intValue();

...
public class SSISiuContestUsingPrcImportStrategy extends ImportStrategy
{
  private SSIContestService ssiContestService;
  private static final Log logger = LogFactory.getLog( SSISiuContestUsingPrcImportStrategy.class );
  ...
  public void importImportFile( ImportFile importFile, List records ) throws ServiceErrorException
  {
    ...
    Map<String, Object> result = ssiContestService.importImportFile( importFile.getId(), importFile.getContestId(), SSIContestType.STEP_IT_UP );
  ...
  {
    Map<String, Object> result = ssiContestService.verifyImportFile( importFile.getId(), importFile.getContestId(), SSIContestType.STEP_IT_UP );
    int errorCount = ( (BigDecimal)result.get( "p_total_error_rec" ) ).intValue();

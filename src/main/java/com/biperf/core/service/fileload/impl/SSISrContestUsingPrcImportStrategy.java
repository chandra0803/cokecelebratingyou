...
public class SSISrContestUsingPrcImportStrategy extends ImportStrategy
{
  private SSIContestService ssiContestService;
  private static final Log logger = LogFactory.getLog( SSISrContestUsingPrcImportStrategy.class );
  ...
  public void importImportFile( ImportFile importFile, List records ) throws ServiceErrorException
  {
    ...
    Map<String, Object> result = ssiContestService.importImportFile( importFile.getId(), importFile.getContestId(), SSIContestType.STACK_RANK );
  ...
  {
    Map<String, Object> result = ssiContestService.verifyImportFile( importFile.getId(), importFile.getContestId(), SSIContestType.STACK_RANK );
    int errorCount = ( (BigDecimal)result.get( "p_total_error_rec" ) ).intValue();

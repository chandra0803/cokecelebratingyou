...
public class SSIDtgtContestUsingPrcImportStrategy extends ImportStrategy
{
  private SSIContestService ssiContestService;
  private static final Log logger = LogFactory.getLog( SSIDtgtContestUsingPrcImportStrategy.class );
  ...
  public void importImportFile( ImportFile importFile, List records ) throws ServiceErrorException
  {
    ...
    Map<String, Object> result = ssiContestService.importImportFile( importFile.getId(), importFile.getContestId(), SSIContestType.DO_THIS_GET_THAT );
  ...
  {
    Map<String, Object> result = ssiContestService.verifyImportFile( importFile.getId(), importFile.getContestId(), SSIContestType.DO_THIS_GET_THAT );
    int errorCount = ( (BigDecimal)result.get( "p_total_error_rec" ) ).intValue();

...
public class GenerateBudgetFileWorkdayProcess extends BaseProcessImpl
{

	public static final String BEAN_NAME = "generateBudgetFileWorkdayProcess";

	public static final String MESSAGE_NAME = "Generate Budget File Work day Process";
	...
	/**
	   ...
	   */
	private static final Log logger = LogFactory.getLog( GenerateBudgetFileWorkdayProcess.class );
	...
	{	      
		logger.info( "processName: " + BEAN_NAME + " - Calling Generate Budget File Work Day Procedure" );
	    addComment( "processName: " + BEAN_NAME + " - Calling Generate Budget File Work Day Procedure" );

	    // Call the stored procedure
  	    Map resultMap;	    
        if ( "Count only with in the org unit".equalsIgnoreCase( level ) )
        {
    	     resultMap = generateBudgetFileProcessService.callGenerateBudgetFileWDProc(orgUnits, 0 );
        }
        else
        {
    	     resultMap = generateBudgetFileProcessService.callGenerateBudgetFileWDProc(orgUnits, 1 );	
        }	    	    
	    ...
	    logger.info( "processName: " + BEAN_NAME + " - Completed Generate Budget File Work Day Procedure" );
	    addComment( "processName: " + BEAN_NAME + " - Completed Generate Budget File Work Day Procedure" );
	  ...
	  private void sendSummaryMessage( boolean success )
	  {
	    ...
	    // Compose the mailing
	    Mailing mailing = composeMail( MessageService.GENERATE_BUDGET_FILE_WD_PROCESS_MESSAGE_CM_ASSET_CODE, MailingType.PROCESS_EMAIL );

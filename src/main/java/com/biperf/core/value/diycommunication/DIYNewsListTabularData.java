...
public class DIYNewsListTabularData
{
  ...
  public DIYNewsListTabularData()
  {

  }

  public DIYNewsListTabularData( HttpServletRequest request, List<DIYCommunications> communicationList, boolean isActive )
  {
    this.meta = new DIYCommunicationsMetaView( communicationList, "diyCommunications.news.labels.NEWS_CONTENT" );
    ...
    for ( Iterator<DIYCommunications> iter = communicationList.iterator(); iter.hasNext(); )
    {
      ...
      parameterMap.put( "newsId", diyCommunication.getId() );
      String newsEditUrl = ClientStateUtils.generateEncodedLink( "", request.getServletContext().getContextPath() + "/participant/createDiyNews.do?method=display", parameterMap );
      result.setUrl( newsEditUrl );
      results.add( result );
    }
  }

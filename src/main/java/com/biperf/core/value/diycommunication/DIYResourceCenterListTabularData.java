...
public class DIYResourceCenterListTabularData
{
  ...
  public DIYResourceCenterListTabularData()
  {
    meta = new DIYCommunicationsMetaView();
  }

  public DIYResourceCenterListTabularData( List<DIYCommunications> communicationList, HttpServletRequest request, boolean isActive )
  {
    this.meta = new DIYCommunicationsMetaView( communicationList, "diyCommunications.resource.labels.RESOURCE_CENTER_CONTENT" );
    ...
    for ( Iterator<DIYCommunications> iter = communicationList.iterator(); iter.hasNext(); )
    {
      ...
      parameterMap.put( "resourceId", diyCommunication.getId() );
      String viewMatchesUrl = ClientStateUtils.generateEncodedLink( "", request.getServletContext().getContextPath() + "/participant/manageResourceCenterEdit.do?method=display", parameterMap );

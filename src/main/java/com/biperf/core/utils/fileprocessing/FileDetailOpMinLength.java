/*
   File: FileDetailOpMinLength.java
   (c)2001 BI, Inc.
...
public class FileDetailOpMinLength implements Operation
{
  ...
  public String perform( String inputValue, FileDetailInfo fileDetailInfo, OperationInfo operationInfo )
  {
    if ( inputValue != null )
    {
      ...
      // Extract operation length
      int t_opLength = 0;
      try
      {
        t_opLength = Integer.parseInt( operationInfo.getValue() );
      }
      ...
      // Check for length
      if ( inputValue.length() < t_opLength )
      {
        return "false";
      }
      return "true";

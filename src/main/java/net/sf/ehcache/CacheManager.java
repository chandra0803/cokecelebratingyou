...
public class CacheManager
{
  ...
  private void assertNoCacheManagerExistsWithSameName( Configuration configuration )
  {
    ...
    {
      final String name;
      // OK, lets see WHO is calling this method.....
      // Thread.dumpStack();
      ...
      else
      {
        ...
        final String msg = "Another " + ( isNamed ? "CacheManager with same name '" + name + "'" : "unnamed CacheManager" )
        ...
        // System.out.println("\n\n\n\nderp.....skipping the exception in the CacheManager
        // thingy...It already exists...can I just re-use this????\n");
      }
    }
  }
  ...
  protected ClusteredInstanceFactory getClusteredInstanceFactory( Ehcache cache )
  {
    ...
    {
      // adding a cache programmatically when there is no clustered store defined in the
      // configuration
  ...
  private void configure( ConfigurationHelper configurationHelper )
  {
    ...
    int cachesRequiringDiskStores = configurationHelper.numberOfCachesThatOverflowToDisk().intValue() + configurationHelper.numberOfCachesThatAreDiskPersistent().intValue();

    if ( diskStorePath == null && cachesRequiringDiskStores > 0 )
    {
      diskStorePath = DiskStoreConfiguration.getDefaultPath();
      LOG.warn( "One or more caches require a DiskStore but there is no diskStore element configured." + " Using the default disk store path of " + DiskStoreConfiguration.getDefaultPath()
        ...
        for ( CacheManagerPeerListener otherCacheManagerPeerListener : cacheManager.cacheManagerPeerListeners.values() )
        {
          ...
          if ( uniqueResourceIdentifier.equals( otherUniqueResourceIdentifier ) )
          {
            LOG.warn( "Creating a new instance of CacheManager with a CacheManagerPeerListener which " + "has a conflict on a resource that must be unique.\n" + "The resource is "
                ...
                + ".\n" + "To avoid this warning consider using the CacheManager factory methods to create a " + "singleton CacheManager "
                + "or specifying a separate ehcache configuration (ehcache.xml) for each CacheManager instance." );
  ...
  {
    return create( ConfigurationFactory.parseConfiguration( new File( configurationFileName ) ), "Creating new CacheManager with config file: " + configurationFileName );
  }
  ...
  {
    return create( ConfigurationFactory.parseConfiguration( configurationFileURL ), "Creating new CacheManager with config URL: " + configurationFileURL );
  }
  ...
  public void addCache( Ehcache cache ) throws IllegalStateException, ObjectExistsException, CacheException
  {
    ...
    final boolean verifyOffHeapUsage = runtimeCfg.hasOffHeapPool()
        && ( !cacheConfiguration.isOverflowToDisk() && !cacheConfiguration.isOverflowToOffHeapSet() || cacheConfiguration.isOverflowToOffHeap() );

    if ( verifyOffHeapUsage && ( cacheConfiguration.isMaxBytesLocalOffHeapPercentageSet() || cacheConfiguration.getMaxBytesLocalOffHeap() > 0 ) )
    {
  ...
  void initializeEhcache( final Ehcache cache, final boolean registerCacheConfig )
  {
    ...
    if ( runtimeCfg.isTerracottaRejoin() && cache.getCacheConfiguration().isTerracottaClustered() )
    {
      ...
      {
        throw new NonStopCacheException( "Unable to add cache [" + cache.getCacheConfiguration().getName() + "] within " + timeoutMillis + " msecs", e );
      }
  ...
  private Ehcache addCacheNoCheck( final Ehcache cache, final boolean strict ) throws IllegalStateException, ObjectExistsException, CacheException
  {
  ...
  public void setName( String name )
  {
    ...
    {
      throw new CacheException( "Problem in reinitializing MBeanRegistrationProvider - " + mbeanRegistrationProvider.getClass().getName(), e );
    }
  }
  ...
  private void clusterRejoinComplete()
  {
    ...
    transactionController = new TransactionController( createTransactionIDFactory(), runtimeCfg.getConfiguration().getDefaultTransactionTimeoutInSeconds() );
  }
  ...
  SizeOfEngine createSizeOfEngine( final Cache cache )
  {
    ...
    if ( className != null )
    {
      ...
      {
        throw new RuntimeException( "Couldn't load and instantiate custom " + ( cache != null ? "SizeOfEngine for cache '" + cache.getName() + "'" : "default SizeOfEngine" ), exception );
      }
    }
    else
    {

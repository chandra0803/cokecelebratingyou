CREATE OR REPLACE PROCEDURE prc_gq_goal_data_load (p_file_name IN VARCHAR2, p_out_returncode OUT NUMBER) IS

  /*******************************************************************************
  -- Purpose: To load the goal data for GoalQuest
  --
  -- Person        Date       Comments
  -- -----------   --------   -----------------------------------------------------
  -- Percy M.     12/11/06      Initial Creation
  -- iyer,Satish  03/08/2007    Fixed validation issues.
  ...
  *******************************************************************************/
  ...
  rec_stg_gq_goal_data          stage_gq_goal_data_import%ROWTYPE;
  rec_import_file               import_file%ROWTYPE;
  rec_import_record_error       import_record_error%ROWTYPE;

  --gq_goal_data LOAD
  v_import_record_id            stage_gq_goal_data_import.import_record_id%TYPE;
  v_import_file_id              stage_gq_goal_data_import.import_file_id%TYPE;
  v_action_type                 stage_gq_goal_data_import.action_type%TYPE;
  v_import_record_error_id      import_record_error.import_record_error_id%type;

  v_login_id                   stage_gq_goal_data_import.login_id%TYPE;
  v_user_id                    stage_gq_goal_data_import.user_id%TYPE;
  v_first_name                 stage_gq_goal_data_import.first_name%TYPE;
  v_last_name                  stage_gq_goal_data_import.last_name%TYPE;
  v_email_addr                 stage_gq_goal_data_import.email_address%TYPE;
  v_goal_level                 stage_gq_goal_data_import.goal_level%TYPE;
  ...
  v_process_name                execution_log.process_name%type  := 'PRC_GQ_GOAL_DATA_LOAD' ;
...
BEGIN
  ...
  LOOP
    --*** move records from text file into the table stg_gq_goal_data_import_rec.**
    ...
    BEGIN
      ...
      ELSE
        ...
        IF trim(v_goal_level) IS NULL THEN
            rec_import_record_error.item_key := 'admin.fileload.errors.MISSING_PROPERTY';
            rec_import_record_error.param1   := 'goal level';
            rec_import_record_error.param2   := v_goal_level;            
            prc_insert_import_record_error(rec_import_record_error);
        END IF;

        v_action_type := 'add';
        v_stage := 'Initialize the record type';
        rec_stg_gq_goal_data.import_record_id := v_import_record_id;
        rec_stg_gq_goal_data.import_file_id   := v_import_file_id;
        rec_stg_gq_goal_data.action_type      := v_action_type;
        rec_stg_gq_goal_data.login_id         := v_login_id;
        rec_stg_gq_goal_data.USER_id          := v_user_id;
        rec_stg_gq_goal_data.first_name       := v_first_name;
        rec_stg_gq_goal_data.last_name        := v_last_name;
        rec_stg_gq_goal_data.email_address    := v_email_addr;
        rec_stg_gq_goal_data.goal_level       := v_goal_level;
        rec_stg_gq_goal_data.created_by       := v_created_by;
        rec_stg_gq_goal_data.date_created     := v_date_created;


        --Insert into stg_gq_goal_data_import_rec
        v_stage := 'Insert into stg_gq_goal_data_import_rec table';
        prc_insert_stg_gq_goal_data(rec_stg_gq_goal_data, v_return_code);

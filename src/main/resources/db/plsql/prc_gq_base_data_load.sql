CREATE OR REPLACE PROCEDURE prc_gq_base_data_load (p_file_name IN VARCHAR2, p_out_returncode OUT NUMBER) IS

  /*******************************************************************************
  -- Purpose: To load the base data for GoalQuest
  --
  ...
  *******************************************************************************/
  ...
  rec_stg_gq_base_data          stage_gq_base_data_import%ROWTYPE;
  rec_import_file               import_file%ROWTYPE;
  rec_import_record_error       import_record_error%ROWTYPE;

  --gq_base_data LOAD
  v_import_record_id            stage_gq_base_data_import.import_record_id%TYPE;
  v_import_file_id              stage_gq_base_data_import.import_file_id%TYPE;
  v_action_type                 stage_gq_base_data_import.action_type%TYPE;
  v_import_record_error_id      import_record_error.import_record_error_id%type;

  v_login_id                   stage_gq_base_data_import.login_id%TYPE;
  v_user_id                    stage_gq_base_data_import.user_id%TYPE;
  v_first_name                 stage_gq_base_data_import.first_name%TYPE;
  v_last_name                  stage_gq_base_data_import.last_name%TYPE;
  v_email_addr                 stage_gq_base_data_import.email_address%TYPE;
  v_base_objective             stage_gq_base_data_import.base_objective%TYPE;
  ...
  v_process_name                execution_log.process_name%type  := 'PRC_GQ_BASE_DATA_LOAD' ;
...
BEGIN
  ...
  LOOP
    --*** move records from text file into the table stg_gq_base_data_import_rec.**
    ...
    BEGIN
      ...
      ELSE
        ...
        rec_stg_gq_base_data.import_record_id := v_import_record_id;
        rec_stg_gq_base_data.import_file_id   := v_import_file_id;
        rec_stg_gq_base_data.action_type      := v_action_type;
        rec_stg_gq_base_data.login_id         := v_login_id;
        rec_stg_gq_base_data.USER_id          := v_user_id;
        rec_stg_gq_base_data.first_name       := v_first_name;
        rec_stg_gq_base_data.last_name        := v_last_name;
        rec_stg_gq_base_data.email_address    := v_email_addr;
        rec_stg_gq_base_data.base_objective   := v_base_objective;
        rec_stg_gq_base_data.created_by       := v_created_by;
        rec_stg_gq_base_data.date_created     := v_date_created;


        --Insert into stg_gq_base_data_import_rec
        v_stage := 'Insert into stg_gq_base_data_import_rec table';
        prc_insert_stg_gq_base_data(rec_stg_gq_base_data, v_return_code);

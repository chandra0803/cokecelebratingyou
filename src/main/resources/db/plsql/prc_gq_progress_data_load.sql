CREATE OR REPLACE PROCEDURE prc_gq_progress_data_load (p_file_name IN VARCHAR2, p_out_returncode OUT NUMBER) IS
  ...
  -- iyer,Satish  03/09/2007    Fixed validation issues.
  ...
  *******************************************************************************/
  ...
  rec_stg_gq_progress_data          stage_gq_progress_data_import%ROWTYPE;
  rec_import_file               import_file%ROWTYPE;
  rec_import_record_error       import_record_error%ROWTYPE;

  --gq_progress_data LOAD
  v_import_record_id            stage_gq_progress_data_import.import_record_id%TYPE;
  v_import_file_id              stage_gq_progress_data_import.import_file_id%TYPE;
  v_action_type                 stage_gq_progress_data_import.action_type%TYPE;
  v_import_record_error_id      import_record_error.import_record_error_id%type;

  v_login_id                   stage_gq_progress_data_import.login_id%TYPE;
  v_user_id                    stage_gq_progress_data_import.user_id%TYPE;
  v_first_name                 stage_gq_progress_data_import.first_name%TYPE;
  v_last_name                  stage_gq_progress_data_import.last_name%TYPE;
  v_email_addr                 stage_gq_progress_data_import.email_address%TYPE;
  v_total_perf_to_date         stage_gq_progress_data_import.total_perf_to_date%TYPE;
  ...
  v_process_name                execution_log.process_name%type  := 'PRC_GQ_progress_DATA_LOAD' ;
  ...
  LOOP
    ...
    BEGIN
      ...
      ELSE
        ...
        rec_stg_gq_progress_data.import_record_id := v_import_record_id;
        rec_stg_gq_progress_data.import_file_id   := v_import_file_id;
        rec_stg_gq_progress_data.action_type      := v_action_type;
        rec_stg_gq_progress_data.login_id         := v_login_id;
        rec_stg_gq_progress_data.USER_id          := v_user_id;
        rec_stg_gq_progress_data.first_name       := v_first_name;
        rec_stg_gq_progress_data.last_name        := v_last_name;
        rec_stg_gq_progress_data.email_address    := v_email_addr;
        rec_stg_gq_progress_data.total_perf_to_date       := v_total_perf_to_date;
        rec_stg_gq_progress_data.created_by       := v_created_by;
        rec_stg_gq_progress_data.date_created     := v_date_created;


        --Insert into stg_gq_progress_data_import_rec
        v_stage := 'Insert into stg_gq_progress_data_import_rec table';
        prc_insert_stg_gq_progress_dat(rec_stg_gq_progress_data, v_return_code);

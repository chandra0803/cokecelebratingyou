create or replace PACKAGE pkg_query_recog_given
    IS
...
PROCEDURE prc_get_RecogGivenPieresults(
    p_in_parentNodeId       IN VARCHAR,        
    p_in_promotionId        IN VARCHAR2,
    p_in_jobposition        IN VARCHAR2,
    p_in_departments        IN VARCHAR,
...
PROCEDURE prc_get_RecogGivenTimeresults(
    p_in_parentNodeId       IN VARCHAR,        
    p_in_promotionId        IN VARCHAR2,
    p_in_jobposition        IN VARCHAR2,
    p_in_departments        IN VARCHAR,
...
END pkg_query_recog_given;

/

create or replace PACKAGE BODY pkg_query_recog_given
IS
-- package constants
...
PROCEDURE p_stg_given_eligible_counts
( p_in_given_received_type    IN VARCHAR2,
  ...
  p_in_promotion_status    IN VARCHAR2
   ...
   c_process_name       CONSTANT execution_log.process_name%TYPE := 'p_stg_given_eligible_counts';
...
END p_stg_given_eligible_counts;
...
BEGIN
   ...
   p_stg_given_eligible_counts(gc_elg_type_given, gc_promotion_type_recognition, p_in_promotionId, p_in_participantStatus, gc_promotion_status_active);
                  ...
                  FROM ( -- data grouped by org node
                         SELECT org_name, 
                           ...
                           FROM ( -- build full data set
                                  SELECT wn.org_id as node_id,
                                         wn.org_name,
                                         NVL(rec_g.eligible_count, 0) AS eligible_cnt,
                                         COUNT(DISTINCT rd.giver_user_id) actual_cnt,
                                         COUNT(rd.giver_user_id) recognition_cnt, --11/19/2018
                                         --COUNT(rd.giver_activity_id) AS recognition_cnt,  --11/19/2018
                                    ...
                                    FROM ( -- get raw data for org nodes and their child nodes
                                         ...
                                         w_node wn,
                                         gtt_recog_elg_count rec_g    -- eligible giver
                                      -- restrict to just the org nodes
                                   WHERE wn.org_id = wn.node_id --Bug  78849 Soma 11/07/2018 forced to add to avoid duplicate results --11/19/2018 uncommented
                                      -- outer join so all queried org nodes appear in result set
                                     AND 
                                     wn.org_id = rd.org_id (+)
                                     AND wn.org_id = rec_g.node_id (+)
                                     AND gc_elg_type_given = rec_g.record_type (+)
                                   GROUP BY wn.org_id ,
                                         wn.org_name,
                                         NVL(rec_g.eligible_count,0)
                                )
...
/******************************************************************************
  NAME:       prc_get_RecogGivenPieresults
  Author                Date            Description
  ----------           ---------------  ------------------------------------------------
   nagarajs            04/03/2017       Initial Creation - G6.2 Query redesign for perfomance changes                                      
  ******************************************************************************/
  c_process_name        CONSTANT execution_log.process_name%TYPE := 'prc_get_RecogGivenPieresults' ;
  v_stage               execution_log.text_line%TYPE;
  v_parms               execution_log.text_line%TYPE;
  v_from_date           DATE;
  v_to_date             DATE; 
                 ...
                 SELECT node_name, 
                   ...
                   FROM ( -- build full data set
                          SELECT wn.org_id as node_id,
                                 ...
                                 COUNT(rd.giver_activity_id) AS recognition_cnt
                            FROM ( -- get raw data for org nodes and their child nodes
                                   SELECT wn2.org_id,
                                          wn2.node_id,
                                          dtl.giver_activity_id
...
EXCEPTION
   WHEN OTHERS THEN
      ...
      p_out_return_code := 99;
      OPEN p_out_data FOR SELECT NULL FROM dual;
END prc_get_RecogGivenPieresults;
...
/******************************************************************************
  NAME:       prc_get_RecogGivenTimeresults
  Author                Date            Description
  ----------           ---------------  ------------------------------------------------
   nagarajs            04/03/2017       Initial Creation - G6.2 Query redesign for perfomance changes                                      
  ******************************************************************************/
  c_process_name        CONSTANT execution_log.process_name%TYPE := 'prc_get_RecogGivenTimeresults' ;
  v_stage               execution_log.text_line%TYPE;
  v_parms               execution_log.text_line%TYPE;
              ...
              SELECT ts.cms_code||' - '||ml.year_char AS month_name,
                ...
                FROM ( -- get raw data for month and year
                       SELECT TO_CHAR(dtl.date_approved,'MON') AS month_char , 
                              ...
                              COUNT(giver_activity_id) AS recognition_cnt
...
EXCEPTION
   WHEN OTHERS THEN
      ...
      p_out_return_code := 99;
      OPEN p_out_data FOR SELECT NULL FROM dual;
END prc_get_RecogGivenTimeresults;   
PROCEDURE prc_get_RecogRatePieresults(
        p_in_parentNodeId       IN VARCHAR,        
        p_in_promotionId        IN VARCHAR2,
        p_in_jobposition        IN VARCHAR2,
       ...
       SELECT sd.*
         FROM ( -- sequence the data
                SELECT org.*,
                  ...
                  FROM ( SELECT giver_user_id,
                                giver_full_name giver_name, 
                                NVL(COUNT(DISTINCT dtl.giver_activity_id),0) recognitions_cnt, 

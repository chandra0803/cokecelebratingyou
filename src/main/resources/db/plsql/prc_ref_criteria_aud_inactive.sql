CREATE OR REPLACE PROCEDURE prc_ref_criteria_aud_inactive
 (pi_audience_id    IN  VARCHAR2,
  pi_resend_flag    IN  NUMBER,
...
/*----------------------------------------------------------------------------
  ...
  Purpose: Returns Refcursor of User IDs for active and inactive pax based upon termsAndConditions.

  Person        Date         Comments
  -----------   -----------  ---------------------------------------------------
  Suresh J     04/18/2016   
  Ravi Dhanekula 05/12/2017  G6-2304 Changed the lookup from vw_curr_pax_employer to participant table to include pax without employer.
-----------------------------------------------------------------------------*/

     
 CURSOR cur_audience (vi_aud_id VARCHAR2) IS
     SELECT audience_id
       FROM audience
      WHERE list_type   = 'criteria'
        --AND audience_id = NVL(vi_aud_id, audience_id)
          AND audience_id IN  
              (SELECT * FROM TABLE(get_array_varchar(vi_aud_id))) 
      ORDER BY audience_id;

  --Audience criteria cursor
  CURSOR cur_cri_aud (vi_audience_id VARCHAR2) IS
    SELECT 
     ...
     WHERE --audience_id = vi_audience_id
          audience_id IN 
              (SELECT * FROM TABLE(get_array_varchar(vi_audience_id))) 
  ...
  C_process_name            execution_log.process_name%TYPE  := 'prc_ref_criteria_aud_inactive';
...
BEGIN
 ...
 FOR rec_audience IN cur_audience (pi_audience_id) LOOP    
   ...
   DELETE FROM gtt_ex_pax_audience;   
      ...
      ELSE  
       ...
       END IF;
        ...
        IF rec_cri_aud.employer_id     IS NOT NULL OR
          ...
          v_sql_emp := 'SELECT e.user_id
                          FROM vw_curr_pax_employer e
                         WHERE e.employer_id                 = NVL('||v_employer_id||', e.employer_id)
                           AND NVL(e.position_type, ''X'')   = NVL('||v_position_type||', NVL(e.position_type, ''X''))
                           AND NVL(e.department_type, ''X'') = NVL('||v_dep_type||', NVL(e.department_type, ''X'')) 
                           AND (e.termination_date IS NULL OR e.termination_date >= SYSDATE ) 
                           AND EXISTS (SELECT au.user_id FROM application_user au WHERE au.is_active = 0 AND au.user_id = e.user_id)';
        ...
        IF rec_cri_aud.node_id        IS NOT NULL OR
          ...
          IF rec_cri_aud.include_child_nodes = 0 THEN
            ...
            v_sql_node := ' SELECT un.user_id
                             FROM user_node un
                            WHERE un.role    = NVL('||v_user_node_role||', un.role)
                              AND EXISTS (SELECT au.user_id FROM application_user au WHERE au.is_active = 0 AND au.user_id = un.user_id)                            
                              AND un.node_id IN (SELECT n.node_id 
                                                FROM node n
                                               WHERE n.node_id      = NVL('||v_node_id||', n.node_id)
                                                 AND LOWER(n.name)  = LOWER(NVL('||v_node_name||', n.name))
                                                 AND n.node_type_id = NVL('||v_node_type_id||', n.node_type_id)';
            ...
            ELSE
              ...
              FOR rec_node_char IN cur_char ('NT',rec_cri_aud.audience_criteria_id,'include') LOOP
                ...
                v_sql_nc_str := ' SELECT node_id
                                    FROM (SELECT characteristic_id,node_id, REPLACE(REPLACE(characteristic_value,''('') ,'')'') characteristic_value 
        ...
        IF rec_cri_aud.country_id IS NOT NULL THEN
         ...
         v_stage := 'Get User with Country for audience criteria id: '||
                    rec_cri_aud.audience_criteria_id;
         v_sql_cntry := ' SELECT ua.user_id
                            FROM user_address ua 
                           WHERE EXISTS (SELECT au.user_id FROM application_user au WHERE au.is_active = 0 AND au.user_id = ua.user_id) 
                                 AND ua.country_id = '||rec_cri_aud.country_id;
        ...
        IF v_usr_char_cnt > 0 THEN
          ...
          FOR rec_user_char IN cur_char ('USER', rec_cri_aud.audience_criteria_id, 'include') LOOP
            ...
            v_sql_char_str := ' SELECT uc.user_id
                                  FROM (SELECT characteristic_id,user_id, REPLACE(REPLACE(characteristic_value,''('') ,'')'') characteristic_value 
                                           FROM user_characteristic) uc
                                 WHERE uc.characteristic_id = '||rec_user_char.characteristic_id||'
                                   AND EXISTS (SELECT au.user_id FROM application_user au WHERE au.is_active = 0 AND au.user_id = uc.user_id)
                                   AND REGEXP_LIKE (lower(uc.characteristic_value), '''||rec_user_char.criteria_characteristic_value||''') '; 
        ...
        IF rec_cri_aud.exclude_country_id IS NOT NULL THEN
         ...
         v_stage := 'Get users exclude country for audience criteria id: '||
                     rec_cri_aud.audience_criteria_id;
         v_e_sql_cntry :=  ' SELECT ua.user_id
                               FROM user_address ua 
                              WHERE     EXISTS (SELECT au.user_id FROM application_user au WHERE au.is_active = 0 AND au.user_id = ua.user_id) 
                                    AND ua.country_id = '||rec_cri_aud.exclude_country_id;
        ...
        IF rec_cri_aud.exclude_position_type IS NOT NULL OR 
          ...
          v_stage := 'Get users exclude position, department for audience criteria id: '||
                     rec_cri_aud.audience_criteria_id;
          v_e_sql_emp :=   ' SELECT e.user_id
                               FROM vw_curr_pax_employer e
                              WHERE NVL(e.position_type, ''X'')   = NVL('||v_e_position_type||', NVL(e.position_type, ''X''))
                                AND NVL(e.department_type, ''X'') = NVL('||v_e_dep_type||', NVL(e.department_type, ''X'')) 
                                AND EXISTS (SELECT au.user_id FROM application_user au WHERE au.is_active = 0 AND au.user_id = e.user_id)';
        ...
        IF rec_cri_aud.exclude_node_id        IS NOT NULL OR
          ...
          IF rec_cri_aud.exclude_include_child_nodes = 0 THEN
            ...
            v_stage := 'Get users exclude node,node_type,user_node_role,include_child_nodes =0 for audience criteria id: '||
                        rec_cri_aud.audience_criteria_id;
            v_e_sql_node :=' SELECT un.user_id
                               FROM user_node un 
                              WHERE un.role    = NVL('||v_e_user_node_role||', un.role)
                                AND EXISTS (SELECT au.user_id FROM application_user au WHERE au.is_active = 0 AND au.user_id = un.user_id)                               
        ...
        IF v_usr_char_cnt > 0 THEN
          ...
          FOR rec_user_char IN cur_char ('USER', rec_cri_aud.audience_criteria_id, 'exclude') LOOP
            ...
            v_e_sql_char_str :=' SELECT uc.user_id
                                   FROM (SELECT characteristic_id,user_id, REPLACE(REPLACE(characteristic_value,''('') ,'')'') characteristic_value 
                                           FROM user_characteristic) uc
                                  WHERE uc.characteristic_id = '||rec_user_char.characteristic_id||'
                                    AND EXISTS (SELECT au.user_id FROM application_user au WHERE au.is_active = 0 AND au.user_id = uc.user_id)                                  
    ...
    OPEN p_out_refcursor FOR
    SELECT --audience_id,
           user_id 
    FROM gtt_pax_audience gpa
    WHERE EXISTS (SELECT e.user_id 
                    FROM participant e--05/12/2017--changed vw_curr_pax_employer to participant table lookup.G6-2304
                    WHERE e.user_id = gpa.user_id 
                          AND e.termination_date IS NULL) 
         AND ((pi_resend_flag = 0 AND 
                EXISTS (SELECT au.user_id 
                         FROM application_user au
                          WHERE au.is_welcome_email_sent = 0 
                                AND au.last_reset_date IS NULL 
                                AND au.user_id = gpa.user_id))                 
                    OR
                (pi_resend_flag = 1 AND 
                            EXISTS (SELECT au.user_id 
                                     FROM application_user au
                                      WHERE au.user_id = gpa.user_id
                                            AND au.last_reset_date IS NULL                                      
                                      )))
         AND 1 = (SELECT boolean_val
                          FROM os_propertyset
                         WHERE entity_name = 'termsAndConditions.used')
         AND EXISTS (SELECT pax.user_id
                      FROM participant pax 
                       WHERE pax.terms_acceptance = 'notaccepted' 
                             AND pax.user_id = gpa.user_id)
     UNION                             
     SELECT --audience_id,
            user_id 
      FROM participant_audience pa
       WHERE pa.audience_id = pi_audience_id
             AND ((pi_resend_flag = 0 AND 
                EXISTS (SELECT au.user_id 
                         FROM application_user au
                          WHERE au.is_welcome_email_sent = 0 
...
END prc_ref_criteria_aud_inactive;
/

CREATE OR REPLACE PACKAGE pkg_query_recog_list_of_givers
IS
    PROCEDURE prc_getRecogsGivenByPax (        
    ...
    PROCEDURE prc_getPointsGivenByPax (        
        p_in_promotionId         IN     VARCHAR,
        p_in_giverReceiver       IN     VARCHAR,
        p_in_userId              IN     NUMBER,
        p_in_rowNumEnd           IN     NUMBER,
    ...
    PROCEDURE prc_getRecogsGivenByPromo (        
        p_in_promotionId         IN     VARCHAR,
        p_in_giverReceiver       IN     VARCHAR,
        p_in_userId              IN     NUMBER,
        p_in_rowNumEnd           IN     NUMBER,
    ...
    PROCEDURE prc_getPointsGivenByPromo (        
        p_in_promotionId         IN     VARCHAR,
        p_in_giverReceiver       IN     VARCHAR,
        p_in_userId              IN     NUMBER,
        p_in_rowNumEnd           IN     NUMBER,
    ...
    PROCEDURE prc_getSummaryGivenByPax (        
        p_in_promotionId         IN     VARCHAR,
        p_in_giverReceiver       IN     VARCHAR,
        p_in_userId              IN     NUMBER,
        p_in_rowNumEnd           IN     NUMBER,
    ...
    PROCEDURE prc_getActivityGivenByPax (        
        p_in_promotionId         IN     VARCHAR,
        p_in_giverReceiver       IN     VARCHAR,
        p_in_userId              IN     NUMBER,
        p_in_rowNumEnd           IN     NUMBER,
        ...
        p_out_data               OUT    SYS_REFCURSOR,
        p_out_totals_data        OUT    SYS_REFCURSOR);
END pkg_query_recog_list_of_givers;
/
CREATE OR REPLACE PACKAGE BODY pkg_query_recog_list_of_givers
IS
-- package constants
...
gc_elg_type_given                CONSTANT rpt_pax_promo_eligibility.giver_recvr_type%TYPE := pkg_const.gc_elg_type_giver;
...
PROCEDURE p_stg_giver_eligible_counts
( p_in_given_received_type IN VARCHAR2,
  ...
  p_in_promotion_status    IN VARCHAR2
   ...
   c_process_name       CONSTANT execution_log.process_name%TYPE := 'p_stg_giver_eligible_counts';
...
END p_stg_giver_eligible_counts;
...
/******************************************************************************
  ...
  ******************************************************************************/
  c_process_name        CONSTANT execution_log.process_name%TYPE := 'prc_getRecogsGivenByPax' ;
  v_stage               execution_log.text_line%TYPE;
...
BEGIN
   ...
   OPEN p_out_data FOR
   SELECT d.giver_name,             --04/09/2018
           d.recognitions_cnt       --04/09/2018
   FROM (                           --04/09/2018
    SELECT rownum as row_num,       --04/09/2018
           giver_name,              --04/09/2018
           recognitions_cnt         --04/09/2018
   FROM  (                          --04/09/2018
   SELECT giver_name,
          recognitions_cnt
     FROM ( --get sum data
           SELECT dtl.giver_full_name as giver_name,
                  COUNT(dtl.giver_activity_id) AS recognitions_cnt
            ...
            WHERE TRUNC(dtl.date_approved) BETWEEN v_from_date AND v_to_date
              ...
              AND gil_dt.ref_text_1 = gc_ref_text_department_type
              AND (  gil_dt.ref_text_2 = gc_search_all_values
                  OR gil_dt.ref_text_2 = dtl.giver_department )
              AND gil_pt.ref_text_1 = gc_ref_text_position_type
              AND (  gil_pt.ref_text_2 = gc_search_all_values
                  OR gil_pt.ref_text_2 = dtl.giver_job_position )
              AND gil_c.ref_text_1 = gc_ref_text_country_id
              AND (  gil_c.ref_text_2 = gc_search_all_values
...
PROCEDURE prc_getPointsGivenByPax (
    ...
    IS
/******************************************************************************
  NAME:       prc_getPointsGivenByPax
  Author                Date            Description
  ----------           ---------------  ------------------------------------------------
   nagarajs            04/03/2017       Initial Creation - G6.2 Query redesign for perfomance changes                                      
   Gorantla            04/09/2018       G6-3993/Bug 75760 - Report charts errors with long script running error due to many recognitions
  ******************************************************************************/
  c_process_name        CONSTANT execution_log.process_name%TYPE := 'prc_getPointsGivenByPax' ;
  v_stage               execution_log.text_line%TYPE;
  v_parms               execution_log.text_line%TYPE;
  v_from_date           DATE;
  v_to_date             DATE; 
...
BEGIN
   ...
   FROM  (   SELECT giver_name,
    ...
    ORDER BY recognition_points_cnt DESC)                           --04/09/2018 
    ) d WHERE d.row_num < 21 ;      --04/09/2018
...
END prc_getPointsGivenByPax;
    ...
    IS
/******************************************************************************
  NAME:       prc_getRecogsGivenByPromo
  Author                Date            Description
  ----------           ---------------  ------------------------------------------------
   nagarajs            04/03/2017       Initial Creation - G6.2 Query redesign for perfomance changes                                      
  ******************************************************************************/
  c_process_name        CONSTANT execution_log.process_name%TYPE := 'prc_getRecogsGivenByPromo' ;
  v_stage               execution_log.text_line%TYPE;
  v_parms               execution_log.text_line%TYPE;
  v_from_date           DATE;
  v_to_date             DATE; 
...
PROCEDURE prc_getPointsGivenByPromo (
    ...
    IS
/******************************************************************************
  NAME:       prc_getPointsGivenByPromo
  Author                Date            Description
  ----------           ---------------  ------------------------------------------------
   nagarajs            04/03/2017       Initial Creation - G6.2 Query redesign for perfomance changes                                      
  ******************************************************************************/
  c_process_name        CONSTANT execution_log.process_name%TYPE := 'prc_getPointsGivenByPromo' ;
  v_stage               execution_log.text_line%TYPE;
  v_parms               execution_log.text_line%TYPE;
  v_from_date           DATE;
  v_to_date             DATE; 
...
EXCEPTION
   WHEN OTHERS THEN
      ...
      p_out_return_code := 99;
      OPEN p_out_data FOR SELECT NULL FROM dual;
END prc_getPointsGivenByPromo; 

PROCEDURE prc_getMetrics (p_in_promotionid         IN     VARCHAR,
                          p_in_giverReceiver       IN     VARCHAR,
                          p_in_userId              IN     NUMBER,
                          p_in_rowNumEnd           IN     NUMBER,
                          p_in_rowNumStart         IN     NUMBER,
...
/******************************************************************************
  ...
  ----------           ---------------  ------------------------------------------------
   nagarajs            04/03/2017       Initial Creation - G6.2 Query redesign for perfomance changes                                      
  ******************************************************************************/
  c_process_name        CONSTANT execution_log.process_name%TYPE := 'prc_getMetrics' ;
  v_stage               execution_log.text_line%TYPE;
  v_parms               execution_log.text_line%TYPE;
  v_from_date           DATE;
  v_to_date             DATE; 
...
BEGIN
   ...
   p_stg_giver_eligible_counts(gc_elg_type_given, gc_promotion_type_recognition, p_in_promotionId, p_in_participantStatus, gc_promotion_status_active);
                            
   OPEN p_out_data FOR
   SELECT MAX(rd.rec_rank) top_value, 
               COUNT(rd.giver_user_id) total, 
               ROUND(COUNT(rd.giver_user_id) / MIN(rec_g.eligible_count), 2) overall_org_avg 
     FROM ( --get sum data
           SELECT RANK() OVER (PARTITION BY giver_user_id  ORDER BY rpt_recognition_detail_id ASC) AS rec_rank, 
                  giver_user_id
             FROM rpt_recognition_detail dtl,
                  promotion p,
                  gtt_id_list gil_dt, -- department type
           ...
           ( SELECT DECODE(SUM(eligible_count), 0, 1, SUM(eligible_count))  AS eligible_count 
               FROM gtt_recog_elg_count) rec_g;    -- eligible giver 
...
PROCEDURE prc_getSummaryGivenByPax (
 ...
 /******************************************************************************
  NAME:       prc_getSummaryGivenByPax
  Author                Date            Description
  ----------           ---------------  ------------------------------------------------
   nagarajs            04/03/2017       Initial Creation - G6.2 Query redesign for perfomance changes                                      
 ******************************************************************************/
  c_process_name        CONSTANT execution_log.process_name%TYPE := 'prc_getSummaryGivenByPax' ;
  v_stage               execution_log.text_line%TYPE;
  v_parms               execution_log.text_line%TYPE;
  ...
  v_data_sort           VARCHAR2(50);
   ...
   CURSOR cur_query_ref IS
    SELECT CAST(NULL AS NUMBER) AS giver_user_id,
             CAST(NULL AS VARCHAR2(4000)) AS giver_name,
             CAST(NULL AS VARCHAR2(4000)) AS giver_country,
             CAST(NULL AS VARCHAR2(4000)) AS giver_node,
             CAST(NULL AS NUMBER) AS org_id,
             CAST(NULL AS VARCHAR2(4000)) AS giver_department,
             CAST(NULL AS VARCHAR2(4000)) AS giver_job_position,
   ...
   OPEN p_out_data FOR 
     SELECT sd.*
         FROM ( -- sequence the data
                SELECT org.*,
                       COUNT(DISTINCT org.giver_user_id) OVER() AS total_records,
                       -- calc record sort order
                       ROW_NUMBER() OVER (ORDER BY
                         -- sort totals record first
                         DECODE(org.giver_user_id, NULL, 0, 99),
                         -- ascending sorts
                         DECODE(v_data_sort, 'giver_user_id', org.giver_user_id),
                         DECODE(v_data_sort, 'giver_name', LOWER(org.giver_name)),
                         DECODE(v_data_sort, 'giver_country', org.giver_country),
                         DECODE(v_data_sort, 'giver_node', org.giver_node),
                         DECODE(v_data_sort, 'giver_department', org.giver_department),
                         DECODE(v_data_sort, 'giver_job_position', org.giver_job_position),
                         ...
                         -- descending sorts
                         DECODE(v_data_sort, 'desc/giver_user_id', org.giver_user_id) DESC,
                         DECODE(v_data_sort, 'desc/giver_name', LOWER(org.giver_name)) DESC,
                         DECODE(v_data_sort, 'desc/giver_country', org.giver_country) DESC,
                         DECODE(v_data_sort, 'desc/giver_node', org.giver_node) DESC,
                         DECODE(v_data_sort, 'desc/giver_department', org.giver_department) DESC,
                         DECODE(v_data_sort, 'desc/giver_job_position', org.giver_job_position) DESC,
                  ...
                  FROM ( -- data grouped by org node
                      SELECT giver_user_id,
                        ...
                        FROM (  SELECT dtl.giver_user_id        AS giver_user_id,
                                       dtl.giver_full_name      AS giver_name,
                                       dtl.giver_node_name      AS giver_node,
                                       dtl.giver_node_id        AS organization_id, 
                                       NVL(tts_c.cms_name, ' ') AS giver_country,
                                       NVL(tts_p.cms_name, ' ') AS giver_job_position,
                                       NVL(tts_d.cms_name, ' ') AS giver_department,
                                       COUNT(dtl.giver_activity_id) AS recognitions_cnt,
                        ...
                        GROUP BY GROUPING SETS ((),(giver_user_id,
                                                    giver_name,
                                                    giver_country,
                                                    giver_node,
                                                    organization_id,
...
PROCEDURE prc_getActivityGivenByPax (
    ...
    IS
/******************************************************************************
  NAME:       prc_getActivityGivenByPax
  Author                Date            Description
  ----------           ---------------  ------------------------------------------------
   nagarajs            04/03/2017       Initial Creation - G6.2 Query redesign for perfomance changes                                      
 ******************************************************************************/
  c_process_name        CONSTANT execution_log.process_name%TYPE := 'prc_getActivityGivenByPax' ;
  v_stage               execution_log.text_line%TYPE;
  v_parms               execution_log.text_line%TYPE;
  v_default_locale      os_propertyset.string_val%TYPE;
  v_from_date           DATE;
  ...
  v_data_sort           VARCHAR2(50);
   ...
   CURSOR cur_query_ref IS
    SELECT CAST(NULL AS DATE) AS date_approved,
             CAST(NULL AS VARCHAR2(4000)) AS giver_name,
             CAST(NULL AS VARCHAR2(4000)) AS giver_country,
             CAST(NULL AS VARCHAR2(4000)) AS giver_node_name,
     ...
     SELECT sd.*
         FROM ( -- sequence the data
                SELECT TRUNC(org.date_approved) AS date_approved,
                       ...
                       org.claim_id, 
                       COUNT( org.giver_name) OVER() AS total_records,
                       -- calc record sort order
                       ROW_NUMBER() OVER (ORDER BY
                         -- sort totals record first
                         DECODE(org.giver_name, NULL, 0, 99),
                         -- ascending sorts
                         DECODE(v_data_sort, 'date_approved', org.date_approved),
                         DECODE(v_data_sort, 'giver_name', LOWER(org.giver_name)),
                         DECODE(v_data_sort, 'giver_country', org.giver_country),
                         DECODE(v_data_sort, 'giver_node_name', org.giver_node_name),
                         ...
                         DECODE(v_data_sort, 'desc/giver_name', LOWER(org.giver_name)) DESC,
                         DECODE(v_data_sort, 'desc/giver_country', org.giver_country) DESC,
                         DECODE(v_data_sort, 'desc/giver_node_name', org.giver_node_name) DESC,
                  ...
                  FROM ( -- data grouped by org node
                      SELECT claim_id,
                        ...
                        FROM (  SELECT dtl.claim_id,
                                       ...
                                       dtl.giver_full_name      AS giver_name,
                                       NVL(tts_c.cms_name, ' ') AS giver_country,
                                       dtl.giver_node_name      AS giver_node_name,
                                  ...
                                  FROM rpt_recognition_detail dtl,
                                       ...
                                       gtt_id_list gil_c    -- country
                                      -- restrict by required fields
                                WHERE dtl.giver_user_id = p_in_userId

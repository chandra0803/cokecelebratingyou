CREATE OR REPLACE PACKAGE pkg_ssi_stack_rank_load IS
  ...
  *******************************************************************************/
  ...
  PROCEDURE p_stage_ssi_stack_rank 
    (p_file_name        IN VARCHAR2, 
     p_out_returncode   OUT NUMBER);

  PROCEDURE p_ssi_stack_rank_verify_import
  ...
  PROCEDURE p_ssi_stack_stg_verify_import
...
END pkg_ssi_stack_rank_load; -- Package spec
/
CREATE OR REPLACE PACKAGE BODY pkg_ssi_stack_rank_load
IS
  /*******************************************************************************
...
PROCEDURE p_stage_ssi_stack_rank
  ...
  /*******************************************************************************
  -- Purpose: To load the STACK_RANK contest for pax/manager/superviewer
  --
  -- Person        Date          Comments
  -- -----------   --------      -----------------------------------------------------
  ...
  *******************************************************************************/
  ...
  v_process_name                execution_log.process_name%type  := 'p_stage_ssi_stack_rank' ;
  ...
  rec_ssi_st_rnk                stage_ssi_stack_rank_import%ROWTYPE;

  --stack_rank LOAD
  v_import_record_id            stage_ssi_stack_rank_import.import_record_id%TYPE;
  v_import_file_id              stage_ssi_stack_rank_import.import_file_id%TYPE;
  ...
  v_created_by                  stage_ssi_stack_rank_import.created_by%TYPE := 0;
  v_date_created                stage_ssi_stack_rank_import.date_created%TYPE := SYSDATE;
  ...
  PROCEDURE prc_insert_stg_ssi_stack_rank (p_import_record     IN  stage_ssi_stack_rank_import%ROWTYPE,
                                         p_out_return_code   OUT NUMBER)
   IS
      -- EXECUTION LOG VARIABLES
      v_process_name    execution_log.process_name%TYPE := 'PRC_INSERT_STG_SSI_STACK_RANK';
      v_release_level   execution_log.release_level%TYPE := '1';
   BEGIN
      INSERT INTO stage_ssi_stack_rank_import 
           (import_record_id,
            import_file_id,
            user_name,
            user_id,
   ...
   EXCEPTION
      ...
      THEN
         ...
         COMMIT;
   END prc_insert_stg_ssi_stack_rank;
...
BEGIN
  ...
  LOOP
    --*** move records from text file into the table stage_ssi_stack_rank_import**
    ...
    BEGIN
        ...
        rec_ssi_st_rnk.user_name  := REPLACE(TRIM(FNC_PIPE_PARSE(v_text, 1, c_delimiter)),CHR(13));

        v_field_cnt  := v_field_cnt + 1;
        rec_ssi_st_rnk.first_name := REPLACE(TRIM(FNC_PIPE_PARSE(v_text, 2, c_delimiter)),CHR(13));
      
        v_field_cnt := v_field_cnt + 1;
        rec_ssi_st_rnk.last_name := REPLACE(TRIM(FNC_PIPE_PARSE(v_text, 3, c_delimiter)),CHR(13));

        v_field_cnt  := v_field_cnt + 1;
        rec_ssi_st_rnk.role := REPLACE(TRIM(FNC_PIPE_PARSE(v_text, 4, c_delimiter)),CHR(13));
        ...
        IF trim(rec_ssi_st_rnk.role) IS NULL THEN
            rec_import_record_error.item_key := 'admin.fileload.errors.MISSING_PROPERTY';
            rec_import_record_error.param1   := 'Role';
            rec_import_record_error.param2   := rec_ssi_st_rnk.role;              
            prc_insert_import_record_error(rec_import_record_error);
            
        ELSE
        
            IF lower(trim(rec_ssi_st_rnk.role)) NOT IN ('participant','manager','superviewer')  THEN
        ...
        rec_ssi_st_rnk.import_record_id := v_import_record_id;
        rec_ssi_st_rnk.import_file_id   := v_import_file_id;
        rec_ssi_st_rnk.created_by       := v_created_by;
        rec_ssi_st_rnk.date_created     := v_date_created;


        --Insert into stage_ssi_stack_rank_import
        v_stage := 'Insert into stage_ssi_stack_rank_import table';
        prc_insert_stg_ssi_stack_rank(rec_ssi_st_rnk, v_return_code);
    ...
    END; -- Get record
  END LOOP; -- *** end of loop to move data to stage_ssi_stack_rank_import table. ***
  COMMIT;
...
PROCEDURE p_verify_ssi_stack_rank
    ( p_in_import_file_id   IN  import_file.import_file_id%TYPE,
  ...
  *******************************************************************************/
  ...
  -- EXECUTION LOG VARIABLES
  lc_proc_name                 CONSTANT execution_log.process_name%TYPE := UPPER('p_verify_ssi_stack_rank');
  l_proc_step                  execution_log.text_line%TYPE;
  v_release_level               execution_log.release_level%type := '1';
  v_execution_log_msg           execution_log.text_line%TYPE;
  v_stage                       VARCHAR2(500);
       ...
       (  SELECT import_record_error_pk_sq.nextval AS import_record_error_id,
            ...
            FROM ( -- get errant records
                   ...
                   (  SELECT s.*
                        ...
                        FROM stage_ssi_stack_rank_import s
...
END p_verify_ssi_stack_rank;
 
---------------------
-- Reports any errors in the staged file records
PROCEDURE p_import_ssi_stack_rank
    ( p_in_import_file_id   IN  import_file.import_file_id%TYPE,
      p_in_ssi_contest_id   IN  promotion.promotion_id%TYPE,
      p_out_returncode      OUT NUMBER
    ) 
  ...
  *******************************************************************************/
  -- EXECUTION LOG VARIABLES
    ...
    lc_proc_name             CONSTANT execution_log.process_name%TYPE := UPPER('p_import_ssi_stack_rank');
    l_proc_step              execution_log.text_line%TYPE;
    l_rec_cnt                INTEGER;
...
/*******************************************************************************
  -- Purpose: To load the stack  contest for pax/manager/superviewer from bi-admin screen
  --
  -- Person        Date          Comments
  -- -----------   --------      -----------------------------------------------------
  ...
  *******************************************************************************/
   ...
   c_process_name          CONSTANT execution_log.process_name%type := 'P_SSI_STACK_RANK_VERIFY_IMPORT';
   c_release_level         CONSTANT execution_log.release_level%type := '1';
...
BEGIN
   ...
   IF v_valid_ssi_contest_id THEN
     ...
     SELECT import_record_error_pk_sq.NEXTVAL AS import_record_error_id,
      ...
      FROM stage_ssi_stack_rank_import sr
     WHERE import_file_id = p_import_file_id;
     
     RAISE e_pgm_exit;  
   END IF;
   -- report validation errors
   v_msg := 'Call p_verify_ssi_stack_rank';
   p_verify_ssi_stack_rank(p_import_file_id, p_ssi_contest_id, v_total_err_count,v_out_returncode);
   ...
   IF (p_load_type = 'L') THEN
      v_msg := 'Call p_import_ssi_stack_rank';
      p_import_ssi_stack_rank(p_import_file_id,p_ssi_contest_id,v_out_returncode);     
      COMMIT; 
...
EXCEPTION
   ...
   WHEN OTHERS THEN
      ...
      p_total_error_rec := v_import_record_count;
      p_out_returncode := 99;
END p_ssi_stack_rank_verify_import;
...
/*******************************************************************************
  -- Purpose: To load the stack rank contest for pax/manager/superviewer from SSI Contest screen
  --
  -- Person        Date          Comments
  -- -----------   --------      -----------------------------------------------------
  -- nagarajs     11/17/2016     Initial Creation 
  *******************************************************************************/
   ...
   c_process_name          CONSTANT execution_log.process_name%type := 'P_SSI_STACK_STG_VERIFY_IMPORT';
   c_release_level         CONSTANT execution_log.release_level%type := '1';
...
BEGIN
   ...
   v_msg := 'Call p_stage_ssi_stack_rank';
   p_stage_ssi_stack_rank(p_file_name, v_out_returncode);
   ...
   IF v_out_returncode = 0 THEN
       ...
       -- report validation errors
       v_msg := 'Call p_verify_ssi_stack_rank';
       p_verify_ssi_stack_rank(v_import_file_id, p_ssi_contest_id, v_total_err_count,v_out_returncode);
       ...
       IF (v_out_returncode = 0) THEN
          ...
          g_load_type := 'L';
          v_msg := 'Call p_import_ssi_stack_rank';
          p_import_ssi_stack_rank(v_import_file_id,p_ssi_contest_id,v_out_returncode);     
          COMMIT; 
...
END p_ssi_stack_stg_verify_import;

END pkg_ssi_stack_rank_load; -- End of Package
/

CREATE OR REPLACE PROCEDURE prc_stage_badge_load(p_file_name IN VARCHAR2,p_out_returncode OUT NUMBER) IS
  /*******************************************************************************
  -- Purpose: BI Admin can load a file to stage and verify list of users and their badge names
  ...
  *******************************************************************************/
  ...
  rec_badge           stage_badge_load%ROWTYPE;
  rec_import_record_error   import_record_error%ROWTYPE;

 --BADGE LOAD
   v_user_name               stage_badge_load.user_name%TYPE;
  v_import_record_id        stage_badge_load.import_record_id%TYPE;
  v_import_file_id          stage_badge_load.import_file_id%TYPE;
  v_badge_name             stage_badge_load.badge_name%TYPE;
   v_created_by              stage_badge_load.created_by%TYPE := 0;
  

  -- EXECUTION LOG VARIABLES
  v_process_name            execution_log.process_name%type  := 'prc_stage_badge_load';
  ...
  LOOP
    ...
    BEGIN
      ...
      ELSE
        ...
        v_field_cnt := v_field_cnt + 1;
        v_badge_name       := REPLACE(TRIM(fnc_pipe_parse(v_text, 2, c_delimiter)),CHR(13));

        v_stage := 'Participant Validations';
        
    -- User_name and badge name are Required
        ...
        IF LTRIM (RTRIM (v_badge_name)) IS NULL THEN
           rec_import_record_error.item_key := 'admin.fileload.errors.BADGE_IS_INVALID';
           rec_import_record_error.param1 := 'Badge Name';
           rec_import_record_error.created_by := v_created_by;
           prc_insert_import_record_error (rec_import_record_error);
        END IF;
    ...
    rec_import_record_error.item_key := NULL;
       ...
       rec_badge.import_record_id := v_import_record_id;
       rec_badge.import_file_id   := v_import_file_id;
       rec_badge.user_name             := v_user_name;
       rec_badge.user_id             := v_user_id;
       rec_badge.badge_name           := v_badge_name;
       rec_badge.insert_or_update           := null;
       rec_badge.date_modified           := null;
       rec_badge.modified_by           := null;
       rec_badge.created_by            := v_created_by;
       rec_badge.date_created          := SYSDATE;
       rec_badge.version            := 1;
          
               
        --Insert into stage_badge_load
        v_stage := 'Insert into stage_badge_load table';
        prc_stg_badge_insert(rec_badge, v_return_code);
    ...
    END;
  END LOOP; -- *** end of loop to move data to stage_badge_load table. ***
  COMMIT;
...
END; -- Procedure prc_stage_badge_load
/
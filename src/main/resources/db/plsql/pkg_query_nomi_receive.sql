CREATE OR REPLACE PACKAGE pkg_query_nomi_receive
IS
/*********************************************************************
Purpose: Provides nomination nominee query utilities.
...
PROCEDURE prc_nomination_nominee
...
END pkg_query_nomi_receive;
/
CREATE OR REPLACE PACKAGE BODY pkg_query_nomi_receive
IS
/*********************************************************************
Purpose: Provides nomination nominee query utilities.
...
gc_pkg_name                      CONSTANT VARCHAR2(30) := UPPER('pkg_query_nomi_receive');
...
-----------------------
-- Queries nomination nominatee based on input parameters and summerizes the results by nominator
PROCEDURE prc_summary_nominee
( p_in_parent_node_id_list IN VARCHAR2,
  p_in_department_list     IN VARCHAR2,
  p_in_job_type_list       IN VARCHAR2,
...
--*******************************************************************************
   ...
   c_process_name       CONSTANT execution_log.process_name%TYPE := 'prc_summary_nominee';
...
--prc_execution_log_entry(c_process_name, gc_release_level, gc_error, v_stage || v_parm_list || SQLCODE || ':' || SQLERRM, NULL);
   ...
   SELECT sd.*
     FROM ( -- sequence the data
            SELECT rd.*,
                   COUNT(rd.recvr_user_id) OVER() AS total_records,
                   -- calc record sort order
                   ROW_NUMBER() OVER (ORDER BY
                     -- sort totals record first
                     DECODE(rd.recvr_user_id, NULL, 0, 99),
                     -- ascending sorts
                     DECODE(v_data_sort, 'recvr_user_id',      rd.recvr_user_id),
                     ...
                     DECODE(v_data_sort, 'nominated_cnt',      rd.nominated_cnt),
                     DECODE(v_data_sort, 'points_received',    rd.points_received),
                     DECODE(v_data_sort, 'cash_received',      rd.cash_received),
                     DECODE(v_data_sort, 'other_qty_received', rd.other_qty_received),
                     DECODE(v_data_sort, 'other_amt_received', rd.other_amt_received),
                     -- descending sorts
                     DECODE(v_data_sort, 'desc/recvr_user_id',      rd.recvr_user_id) DESC,
                     ...
                     DECODE(v_data_sort, 'desc/nominated_cnt',      rd.nominated_cnt) DESC,
                     DECODE(v_data_sort, 'desc/points_received',    rd.points_received) DESC,
                     DECODE(v_data_sort, 'desc/cash_received',      rd.cash_received) DESC,
                     DECODE(v_data_sort, 'desc/other_qty_received', rd.other_qty_received) DESC,
                     DECODE(v_data_sort, 'desc/other_amt_received', rd.other_amt_received) DESC,
                     -- default sort fields
                     LOWER(rd.nominee),
                     rd.recvr_user_id
                   ) -1 AS rec_seq
              FROM ( -- get raw data
                     SELECT dtl.recvr_user_id,
                            fnc_format_user_name(dtl.recvr_last_name, dtl.recvr_first_name) AS nominee,
                            cav_cn.cms_value AS recvr_country_name,
                            dtl.recvr_node_name,
                            COUNT(dtl.ROWID) AS nominated_cnt,
                            --SUM(dtl.points_award_amt) AS points_received,   --04/09/2019
                            SUM(ROUND(dtl.points_award_amt * v_budget_media_value,2)) AS points_received,  --04/09/2019 
                            --SUM(dtl.cash_award_amt ) AS cash_received,--04/09/2019
                            SUM(ROUND(dtl.cash_award_amt * v_cash_currency_value,2)) AS cash_received, --04/09/2019
                            SUM(
                            ...
                            ) AS other_qty_received,
                            --SUM(dtl.other_award_amt) AS other_amt_received, --04/09/2019
                            SUM(ROUND(dtl.other_award_amt * v_cash_currency_value,2)) AS other_amt_received, --04/09/2019
                            dtl.promotion_id
                      ...
                      WHERE dtl.recvr_country_id = c.country_id
                        ...
                        AND NVL(p_in_participant_status, gc_pax_status_active) = dtl.recvr_pax_status
   ...
   SELECT rec_query.nominated_cnt      AS nominated_cnt,
          ...
          rec_query.other_amt_received AS other_amt_received
     FROM dual
...
( p_in_user_id             IN NUMBER,
  ...
  p_out_totals_data        OUT SYS_REFCURSOR
   ...
   c_process_name       CONSTANT execution_log.process_name%TYPE := 'prc_nomination_nominee';
   v_stage              execution_log.text_line%TYPE;
   v_parm_list          execution_log.text_line%TYPE;
   v_locale             os_propertyset.string_val%TYPE;
   v_data_sort          VARCHAR2(50);
   ...
   SELECT CAST(NULL AS NUMBER) AS claim_id,
          ...
          CAST(NULL AS NUMBER) AS promotion_id,
          CAST(NULL AS NUMBER) AS recvr_user_id,
          CAST(NULL AS NUMBER) AS total_records,
          CAST(NULL AS NUMBER) AS rec_seq
     FROM dual;
            ...
            SELECT rd.*,
              ...
              FROM ( -- get raw data
                     SELECT dtl.claim_id,
                            ...
                            SUM(dtl.points_award_amt) AS points_award_amt,
                            SUM(dtl.cash_award_amt) AS cash_award_amt,
                            dtl.other_award_desc,
                            SUM(dtl.other_award_amt) AS other_award_amt,
                            p.promotion_id,
                            dtl.recvr_user_id
                       FROM rpt_nomination_detail dtl,
                      ...
                      WHERE dtl.giver_country_id = c_g.country_id
                        ...
                        AND v_locale                    = ccv_as.locale (+)
                         -- restrict by required fields
                        AND dtl.recvr_user_id = p_in_user_id
                        AND dtl.date_submitted BETWEEN v_from_date AND v_to_date
                        AND dtl.claim_id IS NOT NULL
                        --AND dtl.promotion_status = gc_promotion_status_active --02/22/2017
                      GROUP BY GROUPING SETS
...
END prc_nomination_nominee;

-----------------------
-- Queries the top N nominees based on input parameters
PROCEDURE prc_top_nominee
( p_in_user_id             IN NUMBER, --03/14/2017
  p_in_parent_node_id_list IN VARCHAR2,
  p_in_department_list     IN VARCHAR2,
...
**********************************************************************/
   ...
   c_process_name       CONSTANT execution_log.process_name%TYPE := 'prc_top_nominee';
   v_stage              execution_log.text_line%TYPE;
   v_parm_list          execution_log.text_line%TYPE;
   v_from_date          DATE;
   v_to_date            DATE;
BEGIN
   ...
   SELECT sd.nominatee,
     ...
     FROM ( -- sequence the data
            SELECT rd.*,
                   -- calc record sort order
                   ROW_NUMBER() OVER (ORDER BY rd.nominated_cnt DESC, LOWER(rd.nominatee)) AS rec_seq
              FROM ( -- get raw data
                     SELECT fnc_format_user_name(dtl.recvr_last_name, dtl.recvr_first_name) AS nominatee,
                            COUNT(dtl.ROWID) AS nominated_cnt
                       FROM rpt_nomination_detail dtl,
                            rpt_hierarchy_rollup hr,
                            gtt_id_list gil_hr, -- hierarchy rollup
                      ...
                      WHERE dtl.date_submitted BETWEEN v_from_date AND v_to_date
                        ...
                        AND gil_hr.id = hr.node_id
                        AND hr.child_node_id = dtl.recvr_node_id
                        AND dtl.recvr_user_id = NVL(p_in_user_id, dtl.recvr_user_id) --03/14/2017
                        AND dtl.claim_id IS NOT NULL
                       -- AND dtl.promotion_status = gc_promotion_status_active --02/22/2017
                         -- restrict by optional fields
                        AND NVL(p_in_participant_status, gc_pax_status_active) = dtl.recvr_pax_status

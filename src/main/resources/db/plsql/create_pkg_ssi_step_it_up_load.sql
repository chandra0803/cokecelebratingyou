CREATE OR REPLACE PACKAGE pkg_ssi_step_it_up_load IS
  ...
  *******************************************************************************/
  ...
  PROCEDURE p_stage_ssi_step_it_up 
    (p_file_name        IN VARCHAR2, 
     p_out_returncode   OUT NUMBER);

  PROCEDURE p_ssi_step_it_up_verify_import
  ...
  PROCEDURE p_ssi_stepit_stg_verify_import
...
END pkg_ssi_step_it_up_load; -- Package spec
/
CREATE OR REPLACE PACKAGE BODY pkg_ssi_step_it_up_load
IS
  /*******************************************************************************
  ...
  -- -----------   --------      -----------------------------------------------------
  -- Suresh J     11/11/2016     Initial Creation 
  -- chidamba     03/22/2018     G6-3947 -Step it up contest pax file load is failed for who is not in creator audience.
  *******************************************************************************/
...
PROCEDURE p_stage_ssi_step_it_up
  ...
  /*******************************************************************************
  -- Purpose: To load the STEP_IT_UP contest for pax/manager/superviewer
  --
  -- Person        Date          Comments
  -- -----------   --------      -----------------------------------------------------
  ...
  *******************************************************************************/
  ...
  rec_ssi_step_it_up            stage_ssi_step_it_up_import%ROWTYPE;

  --step_it_up LOAD
  v_import_record_id            stage_ssi_step_it_up_import.import_record_id%TYPE;
  v_import_file_id              stage_ssi_step_it_up_import.import_file_id%TYPE;
  v_import_record_error_id      import_record_error.import_record_error_id%type;

  v_created_by                  stage_ssi_step_it_up_import.created_by%TYPE := 0;
  v_date_created                stage_ssi_step_it_up_import.date_created%TYPE := SYSDATE;
  v_is_active                   application_user.is_active%TYPE; --03/03/2017

  -- EXECUTION LOG VARIABLES
  v_process_name                execution_log.process_name%type  := 'PRC_SSI_STEP_IT_UP_LOAD' ;
  v_release_level               execution_log.release_level%type := '1';
  v_execution_log_msg           execution_log.text_line%TYPE;
  
  v_baseline                    stage_ssi_step_it_up_import.baseline%TYPE;
  ...
  PROCEDURE prc_insert_stg_ssi_step_it_up (p_import_record     IN  stage_ssi_step_it_up_import%ROWTYPE,
                                         p_out_return_code   OUT NUMBER)
   IS
      -- EXECUTION LOG VARIABLES
      v_process_name    execution_log.process_name%TYPE := 'PRC_INSERT_STG_SSI_STEP_IT_UP';
      v_release_level   execution_log.release_level%TYPE := '1';
   BEGIN
      INSERT INTO stage_ssi_step_it_up_import 
           (import_record_id,
            import_file_id,
            user_name,
            user_id,
   ...
   EXCEPTION
      ...
      THEN
         ...
         COMMIT;
   END prc_insert_stg_ssi_step_it_up;
...
BEGIN
  ...
  LOOP
    --*** move records from text file into the table stage_ssi_step_it_up_import**
    ...
    BEGIN
        ...
        rec_ssi_step_it_up.user_name  := REPLACE(TRIM(FNC_PIPE_PARSE(v_text, 1, c_delimiter)),CHR(13));

        v_field_cnt  := v_field_cnt + 1;
        rec_ssi_step_it_up.first_name := REPLACE(TRIM(FNC_PIPE_PARSE(v_text, 2, c_delimiter)),CHR(13));
      
        v_field_cnt := v_field_cnt + 1;
        rec_ssi_step_it_up.last_name := REPLACE(TRIM(FNC_PIPE_PARSE(v_text, 3, c_delimiter)),CHR(13));

        v_field_cnt  := v_field_cnt + 1;
        rec_ssi_step_it_up.role := REPLACE(TRIM(FNC_PIPE_PARSE(v_text, 4, c_delimiter)),CHR(13));
        
        v_field_cnt  := v_field_cnt + 1;
        rec_ssi_step_it_up.baseline := REPLACE(TRIM(FNC_PIPE_PARSE(v_text, 5, c_delimiter)),CHR(13));
        ...
        IF trim(rec_ssi_step_it_up.role) IS NULL THEN
            rec_import_record_error.item_key := 'admin.fileload.errors.MISSING_PROPERTY';
            rec_import_record_error.param1   := 'Role';
            rec_import_record_error.param2   := rec_ssi_step_it_up.role;              
            prc_insert_import_record_error(rec_import_record_error);

        ELSE
        
            IF lower(trim(rec_ssi_step_it_up.role)) NOT IN ('participant','manager','superviewer')  THEN
        ...
        IF rec_ssi_step_it_up.baseline IS NOT NULL THEN
        
          BEGIN
            v_baseline := TO_NUMBER(rec_ssi_step_it_up.baseline);
          EXCEPTION
            WHEN OTHERS THEN
              rec_import_record_error.item_key := 'admin.fileload.errors.MUST_BE_NUMBER_ERROR';
              rec_import_record_error.param1   := 'Baseline';
              rec_import_record_error.param2   := rec_ssi_step_it_up.baseline;              
              prc_insert_import_record_error(rec_import_record_error);
          END;
          
          IF rec_ssi_step_it_up.baseline > c_amount_allow THEN   
          ...
          IF INSTR (TO_CHAR (rec_ssi_step_it_up.baseline), '.') > 0  AND
             REGEXP_COUNT ( 
                         TO_CHAR (rec_ssi_step_it_up.baseline),
                         '[0-9]',
                         INSTR (TO_CHAR (rec_ssi_step_it_up.baseline), '.') + 1) NOT BETWEEN 2 AND 4  THEN 
        ...
        rec_ssi_step_it_up.import_record_id := v_import_record_id;
        rec_ssi_step_it_up.import_file_id   := v_import_file_id;
        rec_ssi_step_it_up.created_by       := v_created_by;
        rec_ssi_step_it_up.date_created     := v_date_created;


        --Insert into stage_ssi_step_it_up_import
        v_stage := 'Insert into stage_ssi_step_it_up_import table';
        prc_insert_stg_ssi_step_it_up(rec_ssi_step_it_up, v_return_code);
    ...
    END; -- Get record
  END LOOP; -- *** end of loop to move data to stage_ssi_step_it_up_import table. ***
  COMMIT;
...
-- Reports any errors in the staged file records
 PROCEDURE p_verify_ssi_step_it_up
    ( p_in_import_file_id   IN  import_file.import_file_id%TYPE,
  ...
  *******************************************************************************/
  ...
  -- EXECUTION LOG VARIABLES
  lc_proc_name                 CONSTANT execution_log.process_name%TYPE := UPPER('p_verify_ssi_step_it_up');
  l_proc_step                  execution_log.text_line%TYPE;
  v_release_level               execution_log.release_level%type := '1';
  v_execution_log_msg           execution_log.text_line%TYPE;
  v_stage                       VARCHAR2(500);
       ...
       (  SELECT import_record_error_pk_sq.nextval AS import_record_error_id,
            ...
            FROM ( -- get errant records
                   ...
                   (  SELECT s.*
                        ...
                        FROM stage_ssi_step_it_up_import s
                                  ...
                                  WHERE       pax_aud.user_id          = sr.user_id  and
                                              ...
                                              SELECT pax.user_id 
                                                                ...
                                                                WHERE p.promotion_id           = s.promotion_id and 
                                                                      --p.primary_audience_type = 'allactivepaxaudience' and --03/22/2018
                                                                      p.secondary_audience_type = 'allactivepaxaudience' and --03/22/2018
                   ...
                   SELECT sr.import_file_id,
                          sr.import_record_id,
                          gc_sys_err_not_mgr_or_own  AS item_key,
                          'USER_ID'                  AS field_name,
                          TO_CHAR(sr.user_id)        AS field_value,
                          NULL                       AS ref_value
                     FROM stg_rec sr,
...
END p_verify_ssi_step_it_up;
 
---------------------
-- Reports any errors in the staged file records
PROCEDURE p_import_ssi_step_it_up
    ( p_in_import_file_id   IN  import_file.import_file_id%TYPE,
      p_in_ssi_contest_id   IN  promotion.promotion_id%TYPE,
      p_out_returncode      OUT NUMBER
    ) 
    IS
  /*******************************************************************************
  -- Purpose: To load the Step It Up contest for pax/manager/superviewer
  --
  -- Person        Date          Comments
  -- -----------   --------      -----------------------------------------------------
  -- Suresh J     11/11/2016     Initial Creation 
  *******************************************************************************/
  -- EXECUTION LOG VARIABLES
    ...
    lc_proc_name             CONSTANT execution_log.process_name%TYPE := UPPER('p_import_ssi_step_it_up');
    l_proc_step              execution_log.text_line%TYPE;
    l_rec_cnt                INTEGER;
...
/*******************************************************************************
  -- Purpose: To load the step it up contest for pax/manager/superviewer from bi-admin screen
  --
  -- Person        Date          Comments
  -- -----------   --------      -----------------------------------------------------
  ...
  *******************************************************************************/
   ...
   c_process_name          CONSTANT execution_log.process_name%type := 'P_SSI_STEP_IT_UP_VERIFY_IMPORT';
   c_release_level         CONSTANT execution_log.release_level%type := '1';
...
BEGIN
   ...
   IF v_valid_ssi_contest_id THEN
     ...
     SELECT import_record_error_pk_sq.NEXTVAL AS import_record_error_id,
      ...
      FROM stage_ssi_step_it_up_import sr
     WHERE import_file_id = p_import_file_id;
     
     RAISE e_pgm_exit;  
   END IF;
   -- report validation errors
   v_msg := 'Call p_verify_ssi_step_it_up';
   p_verify_ssi_step_it_up(p_import_file_id, p_ssi_contest_id, v_total_err_count, v_out_returncode);
   ...
   IF (p_load_type = 'L') THEN
      v_msg := 'Call p_import_ssi_step_it_up';
      p_import_ssi_step_it_up(p_import_file_id,p_ssi_contest_id,v_out_returncode);     
      COMMIT; 
...
EXCEPTION
   ...
   WHEN OTHERS THEN
      ...
      p_total_error_rec := v_import_record_count;
      p_out_returncode := 99;
END p_ssi_step_it_up_verify_import;
...
/*******************************************************************************
  -- Purpose: To load the step it up contest for pax/manager/superviewer from SSI Contest screen
  --
  -- Person        Date          Comments
  -- -----------   --------      -----------------------------------------------------
  -- nagarajs     11/17/2016     Initial Creation 
  *******************************************************************************/
   ...
   c_process_name          CONSTANT execution_log.process_name%type := 'P_SSI_STEPIT_STG_VERIFY_IMPORT';
   c_release_level         CONSTANT execution_log.release_level%type := '1';
...
BEGIN
   ...
   v_msg := 'Call p_stage_ssi_step_it_up';
   p_stage_ssi_step_it_up(p_file_name, v_out_returncode);
   ...
   IF v_out_returncode = 0 THEN
       ...
       -- report validation errors
       v_msg := 'Call p_verify_ssi_step_it_up';
       p_verify_ssi_step_it_up(v_import_file_id, p_ssi_contest_id, v_total_err_count,v_out_returncode);
       ...
       IF (v_out_returncode = 0) THEN
         ...
         p_upd_import_file
          ...
          g_load_type := 'L';
          v_msg := 'Call p_import_ssi_step_it_up';
          p_import_ssi_step_it_up(v_import_file_id,p_ssi_contest_id,v_out_returncode);     
          COMMIT; 
...
END p_ssi_stepit_stg_verify_import;

END pkg_ssi_step_it_up_load; -- End of Package
/

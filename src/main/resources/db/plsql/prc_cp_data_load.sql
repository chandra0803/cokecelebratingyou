CREATE OR REPLACE PROCEDURE prc_cp_data_load (p_file_name IN VARCHAR2, p_out_returncode OUT NUMBER) IS
  ...
  *******************************************************************************/
  ...
  rec_stg_cp_data               stage_cp_data_import%ROWTYPE;
  rec_import_file               import_file%ROWTYPE;
  rec_import_record_error       import_record_error%ROWTYPE;

  --cp_data LOAD
  v_import_record_id            stage_cp_data_import.import_record_id%TYPE;
  v_import_file_id              stage_cp_data_import.import_file_id%TYPE;
  v_action_type                 stage_cp_data_import.action_type%TYPE;
  v_import_record_error_id      import_record_error.import_record_error_id%type;

  v_login_id                   stage_cp_data_import.login_id%TYPE;
  v_user_id                    stage_cp_data_import.user_id%TYPE;
  v_first_name                 stage_cp_data_import.first_name%TYPE;
  v_last_name                  stage_cp_data_import.last_name%TYPE;
  v_email_addr                 stage_cp_data_import.email_address%TYPE;
  v_cp_level                   stage_cp_data_import.cp_level%TYPE;
  ...
  v_process_name                execution_log.process_name%type  := 'PRC_cp_DATA_LOAD' ;
...
BEGIN
  ...
  LOOP
    --*** move records from text file into the table stg_cp_data_import_rec.**
    ...
    BEGIN
      ...
      ELSE
        ...
        IF trim(v_cp_level) IS NULL THEN
            rec_import_record_error.item_key := 'admin.fileload.errors.MISSING_PROPERTY';
            rec_import_record_error.param1   := 'cp level';
            rec_import_record_error.param2   := v_cp_level;            
            prc_insert_import_record_error(rec_import_record_error);
        END IF;

        v_action_type := 'add';
        v_stage := 'Initialize the record type';
        rec_stg_cp_data.import_record_id := v_import_record_id;
        rec_stg_cp_data.import_file_id   := v_import_file_id;
        rec_stg_cp_data.action_type      := v_action_type;
        rec_stg_cp_data.login_id         := v_login_id;
        rec_stg_cp_data.USER_id          := v_user_id;
        rec_stg_cp_data.first_name       := v_first_name;
        rec_stg_cp_data.last_name        := v_last_name;
        rec_stg_cp_data.email_address    := v_email_addr;
        rec_stg_cp_data.cp_level       := v_cp_level;
        rec_stg_cp_data.created_by       := v_created_by;
        rec_stg_cp_data.date_created     := v_date_created;


        --Insert into stg_cp_data_import_rec
        v_stage := 'Insert into stg_cp_data_import_rec table';
        prc_insert_stg_cp_data(rec_stg_cp_data, v_return_code);

CREATE OR REPLACE PROCEDURE PRC_RA_OD_REMINDER (p_in_user_id      IN  application_user.user_id%TYPE, 
 ...
 IS
  PRAGMA AUTONOMOUS_TRANSACTION;
   ...
   -- Loganathan    08/06/2019 	 Bug 79087 - RA overdue notifications shows large 
                                 overdue days when pax moved from One Manager to Other Manager
   *******************************************************************************/
  c_process_name          CONSTANT execution_log.process_name%TYPE:='PRC_RA_OD_REMINDER';
...
IF v_live_cnt_promotion >= 1 THEN 
  IF v_cnt_all_active_promo >= 1 THEN
        ...
        FROM
          (WITH rec_user as 
             (SELECT au.user_id,
                     ...
                     pe.position_type,
                     p.avatar_small,
                     --trunc(sysdate+1) - NVL(trunc(hire_Date),NVL(trunc(enrollment_date),trunc(au.date_created))) days_since_hired,  --08/06/2019 Bug 79087--Commented
                     trunc(sysdate) - NVL(trunc(rct_date),NVL(trunc(hire_Date),NVL(trunc(enrollment_date),trunc(au.date_created)))) days_since_hired, --08/06/2019 Bug 79087
                     pe.hire_Date,
                     au.enrollment_date,
                     au.date_created,                     
                ...
                FROM application_user au,
                     ...
                     --gtt_ra_receiver gr	--08/06/2019 Bug 79087  --Commented
         ...
         rec_pend as 
          ...
          SELECT user_id,
            ...
            FROM (SELECT *
                    FROM (SELECT *
                            FROM (SELECT gr.user_id,
                                         ...
                                         gr.date_created,
                                         ci.date_approved,
                                         --trunc(sysdate+1) - trunc(ci.date_approved) since_last_recog,--08/06/2019 Bug 79087 Commented
                                   ...
                                   WHERE c.claim_id = ci.claim_id
                                     ...
                                     AND c.submitter_id = p_in_user_id)
                           WHERE ranks = 1)
                   WHERE since_last_recog > v_days_emp_reminder) rec,
                (SELECT *
                   FROM (SELECT pr.*,
                                RANK () OVER (PARTITION BY PR.participant_id ORDER BY pr.date_created DESC) ranks
                           FROM rec_pax pr
           ...
           SELECT user_id,
                  ...
                  position_type,
                  avatar_small,
                  days_since_hired since_last_recog
             FROM (SELECT *
                     FROM rec_user
    ...
    ORDER BY since_last_recog desc,
             upper(first_name),
             upper(last_name),
             user_id
            ...
            FROM (SELECT *
                    FROM (SELECT *
                            FROM (SELECT gr.user_id,
                                         ...
                                         gr.date_created,
                                         ci.date_approved,
                                         --trunc(sysdate+1) - trunc(ci.date_approved) since_last_recog,    --08/06/2019 Bug 79087
                                         trunc(sysdate) - greatest(trunc(rct_date),trunc(ci.date_approved)) since_last_recog,	--08/06/2019 Bug 79087
                                         a.quantity,
                                         rc.behavior, 
                                         0 by_others,

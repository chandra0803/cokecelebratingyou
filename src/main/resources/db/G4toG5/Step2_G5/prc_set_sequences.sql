...
DECLARE
   ...
   (  -- get max sequence values
      ...
      UNION ALL SELECT 'BUDGET_TX_HISTORY_PK_SQ', CAST(MAX(budget_reallocation_history_id) AS NUMBER) FROM budget_reallocation_history
      UNION ALL SELECT 'BUDGET_PK_SQ', CAST(MAX(budget_id) AS NUMBER) FROM budget
      UNION ALL SELECT 'BUDGET_HISTORY_PK_SQ', CAST(MAX(budget_history_id) AS NUMBER) FROM budget_history
      ...
      UNION ALL SELECT 'PROMO_SWEEPSTAKE_WINNER_PK_SQ', CAST(MAX(winner_id) AS NUMBER) FROM promo_sweepstake_winners
      ...
      UNION ALL SELECT 'PROCESS_INVOCATION_PK_SQ', CAST(MAX(process_invocation_id) AS NUMBER) FROM process_invocation      
      UNION ALL SELECT 'PROCESS_INVOC_PARAM_PK_SQ', CAST(MAX(process_invocation_param_id) AS NUMBER) FROM process_invocation_parameter
      UNION ALL SELECT 'PROCESS_INVOC_PARAM_VALU_PK_SQ', CAST(MAX(process_invoc_param_value_id) AS NUMBER) FROM process_invoc_param_value
      UNION ALL SELECT 'PROCESS_INVOC_COMMENT_PK_SQ', CAST(MAX(process_invocation_comment_id) AS NUMBER) FROM process_invocation_comment   
      UNION ALL SELECT 'COMM_LOG_PK_SQ', CAST(MAX(comm_log_id) AS NUMBER) FROM comm_log      
      UNION ALL SELECT 'PARTICIPANT_ABOUTME_PK_SQ', CAST(MAX(PARTICIPANT_ABOUTME_ID) AS NUMBER) FROM participant_aboutme
      UNION ALL SELECT 'MAILING_PK_SQ', CAST(MAX(mailing_id) AS NUMBER) FROM mailing
      UNION ALL SELECT 'MAILING_RECIPIENT_PK_SQ', CAST(MAX(mailing_recipient_id) AS NUMBER) FROM mailing_recipient      
      UNION ALL SELECT 'MAILING_MESSAGE_LOCALE_PK_SQ', CAST(MAX(mailing_message_locale_id) AS NUMBER) FROM mailing_message_locale
      UNION ALL SELECT 'MAILING_RECIPIENT_DATA_PK_SQ', CAST(MAX(mailing_recipient_data_id) AS NUMBER) FROM mailing_recipient_data
      UNION ALL SELECT 'PROCESS_ROLE_PK_SQ', CAST(MAX(process_role_id) AS NUMBER) FROM process_role      
      UNION ALL SELECT 'REPORT_DASHBOARD_PK_SQ',CAST(MAX(report_dashboard_id) AS NUMBER) AS max_seq_id  FROM report_dashboard
      UNION ALL SELECT 'REPORT_DASHBOARD_ITEM_PK_SQ',CAST(MAX(report_dashboard_item_id) AS NUMBER)     FROM report_dashboard_item
      UNION ALL SELECT 'RPT_DASHBOARD_ITEM_PARAM_PK_SQ',CAST(MAX(rpt_dashboard_item_param_id) AS NUMBER)      FROM report_dashboard_item_param
      UNION ALL SELECT 'PARTICIPANT_FOLLOWER_PK_SQ' ,CAST(MAX(participant_follower_id) AS NUMBER)      FROM participant_followers 
      UNION ALL SELECT 'BUDGET_TX_HISTORY_PK_SQ' ,CAST(MAX(BUDGET_REALLOCATION_HISTORY_ID) AS NUMBER)      FROM BUDGET_REALLOCATION_HISTORY
     UNION ALL SELECT 'C_MANAGER_MESSAGE_PK_SQ' ,CAST(MAX(CELEBRATION_MANAGER_MESSAGE_ID) AS NUMBER)      FROM CELEBRATION_MANAGER_MESSAGE
     UNION ALL SELECT 'MAILING_MSG_ATTACH_INFO_PK_SQ' ,CAST(MAX(MAILING_ATTACHMENT_INFO_ID) AS NUMBER)      FROM MAILING_ATTACHMENT_INFO
     UNION ALL SELECT 'PAX_SURVEY_RES_PK_SQ' ,CAST(MAX(PARTICIPANT_SURVEY_RESPONSE_ID) AS NUMBER)      FROM PARTICIPANT_SURVEY_RESPONSE
     UNION ALL SELECT 'PUBLIC_RECOGNITION_COMMENT_SQ' ,CAST(MAX(PUBLIC_RECOGNITION_COMMENT_ID) AS NUMBER)      FROM PUBLIC_RECOGNITION_COMMENT
     UNION ALL SELECT 'PUBLIC_RECOGNITION_LIKE_SQ' ,CAST(MAX(PUBLIC_RECOGNITION_LIKE_ID) AS NUMBER)      FROM PUBLIC_RECOGNITION_LIKE
     UNION ALL SELECT 'SURVEY_QUS_RES_PK_SQ' ,CAST(MAX(SURVEY_QUESTION_RESPONSE_ID) AS NUMBER)      FROM SURVEY_QUESTION_RESPONSE
  )

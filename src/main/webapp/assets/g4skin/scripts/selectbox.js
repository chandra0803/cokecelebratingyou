...
function moveSelectedOptions(from,to){if(arguments.length>3){var regex = arguments[3];if(regex != ""){unSelectMatchingOptions(from,regex);}}var l = from.selectedIndex;    if(l>-1){for(var i=l;i<from.options.length;){var o = from.options[i];if(o.selected){to.options[to.options.length] = new Option( o.text, o.value, false, false);}from.options[i] = null;i = from.selectedIndex;if(i==-1){break;}}if((arguments.length<3) ||(arguments[2]==true)){sortSelect(from);sortSelect(to);}from.selectedIndex = -1;to.selectedIndex = -1;}}
...
function addOptions(text, value, to){var newOption = new Option( text, value, false, false);var found = false;for(var i=(to.options.length-1);i>=0;i--){var o = to.options[i];if(o.value == newOption.value){found = true;break;}}if(!found){to.options[to.options.length] = newOption;}if((arguments.length<3) ||(arguments[2]==true)){sortSelect(to);}to.selectedIndex = -1;}
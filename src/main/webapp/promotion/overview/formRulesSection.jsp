...
<%@ include file="/include/taglib.jspf"%>
	<%	Map paramMap = new HashMap();
	    ...
	    String url = "/" + (String)request.getAttribute( "promotionStrutsModulePath" ) + "/promotionFormRules.do?method=display";
    ...
    <c:choose>
      ...
      <c:when test="${ promotionOverviewForm.hasParent == 'true' }">
			...
			<c:when test="${ (promotionOverviewForm.promotionTypeCode == 'recognition') and (promotionOverviewForm.promotionStatus == 'live') }">
				<a class="content-link" href="<c:out value="${viewUrl}"/>">
        	<cms:contentText code="system.link" key="VIEW" />
				</a>
			</c:when>
...
<c:forEach items="${formRules}" var="ruleItem">
  <tr>
    <td>&nbsp;</td>
    <td colspan="2" valign="top" class="content-field-label" nowrap style="white-space:pre-wrap ;"><cms:contentText code="${ruleItem.claimForm.cmAssetCode}" key="${ruleItem.cmKeyForName}" /></td>
  </tr>
</c:forEach>

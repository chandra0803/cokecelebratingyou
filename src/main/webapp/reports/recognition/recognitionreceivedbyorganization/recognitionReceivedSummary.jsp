...
<%@page import="com.biperf.core.value.recognition.RecognitionReceivedSummaryValue"%>
...
"tabularData": [
  {
    "meta": {
      "columns": [
        ...
        {
          "id": "3",
          "name": "<cms:contentText key="ELIGIBLE_RECIPIENTS" code="${report.cmAssetCode}"/>",
          "description": "<cms:contentText key="ELIGIBLE_RECIPIENTS_DESC" code="${report.cmAssetCode}"/>",
          "type": "number",
          "alignment": "",
          "nameId": "",
          "sortable": true,
        ...
        {
          "id": "4",
          "name": "<cms:contentText key="ACTUAL_RECIPIENTS" code="${report.cmAssetCode}"/>",
          "description": "<cms:contentText key="ACTUAL_RECIPIENTS_DESC" code="${report.cmAssetCode}"/>",
          "type": "number",
          "alignment": "",
          "nameId": "",
          "sortable": true,
        ...
        {
          "id": "5",
          "name": "<cms:contentText key="PERCENT_RECEIVED" code="${report.cmAssetCode}"/>",
          "description": "<cms:contentText key="PERCENT_RECEIVED_DESC" code="${report.cmAssetCode}"/>",
          "type": "number",
          "alignment": "",
          "nameId": "",
          "sortable": true,
        ...
        <c:choose>
			<c:when test="${ fn:contains(  reportDataId.nodeId ,reportParametersForm.parentNodeId ) }">
				<c:choose>
					<c:when test="${reportDataId.actualCnt > 0 || reportDataId.totalRecognition > 0 || reportDataId.points > 0 || reportDataId.plateauEarnedCnt > 0}">
						<%
						    RecognitionReceivedSummaryValue reportValue = (RecognitionReceivedSummaryValue) pageContext.getAttribute( "reportDataId" );
						    Map<String, Object> paramMap = new HashMap<String, Object>();
						    paramMap.put( "parentNodeId", reportValue.getNodeId(  ) );
						    paramMap.put( "nodeAndBelow", 0 );
						    String urlToUse = "/reports/displayRecognitionReceivedByOrgReport.do?method=displayDetailReport";
			...
			<c:otherwise>
				<%
				    ...
				    String urlToUse = "/reports/displayRecognitionReceivedByOrgReport.do?method=displaySummaryReport";
				    String drilldownUrl = ClientStateUtils.generateEncodedLink( RequestUtils.getBaseURI(request), urlToUse, paramMap );
				%>
			    "<%=drilldownUrl %>",
			</c:otherwise>
